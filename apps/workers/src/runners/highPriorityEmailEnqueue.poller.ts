...
// Extracted so a single tick can be unit-tested directly against an
// injected fake queue instead of only through the never-resolving
// while(true) loop below (which also always constructs a real BullMQ
// Queue) — same pattern already used for claimFollowUpDueTick and
// reportExport.cleanup.ts/materialSpecExport.cleanup.ts.
export async function highPriorityEmailEnqueueTick(
  ...
  if (deliveries.length === 0) return { succeeded: 0, failed: 0 };
...
export function startHighPriorityEmailEnqueuePoller(opts?: {
  ...
  const loop = async () => {
    while (!stopped) {
      try {
        await highPriorityEmailEnqueueTick(queue, batchSize);
      } catch (e: any) {
