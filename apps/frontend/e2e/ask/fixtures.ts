      ...
      const response = {
        ...
        captureRequests: [{
          ...
          inputSchema: { type: 'GROUP', fields: [
            ...
            { key: 'priority', label: 'Priority', prompt: 'How important is it?', required: true, inputSchema: { type: 'SINGLE_SELECT', options: [{ label: 'Low', value: 'LOW' }, { label: 'Medium', value: 'MEDIUM' }, { label: 'High', value: 'HIGH' }, { label: 'Urgent', value: 'URGENT' }] } },
            { key: 'nextDueDate', label: 'Due date', prompt: 'When is it due? (YYYY-MM-DD)', required: false, inputSchema: { type: 'SHORT_TEXT', maxLength: 10 } },
            ...
            { key: 'frequency', label: 'Repeat', prompt: 'How often?', required: true, when: { fieldKey: 'isRecurring', operator: 'EQUALS', value: true }, inputSchema: { type: 'SINGLE_SELECT', options: [{ label: 'Monthly', value: 'MONTHLY' }, { label: 'Annually', value: 'ANNUALLY' }] } },
            { key: 'estimatedCostUsd', label: 'Estimated cost', prompt: 'Roughly what will it cost?', required: false, inputSchema: { type: 'DECIMAL', min: 0, unit: 'USD' } },
