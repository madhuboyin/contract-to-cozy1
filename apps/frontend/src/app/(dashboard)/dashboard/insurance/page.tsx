...
import { FileText, Plus, Loader2, Shield, Trash2, Edit, X, Save, ExternalLink } from 'lucide-react';
...
import { InsurancePolicy, CreateInsurancePolicyInput, UpdateInsurancePolicyInput, Property, APIResponse } from '@/types';
...
export default function InsurancePage() {
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isModalOpen} onOpenChange={closeModal}>
          <DialogTrigger asChild>
            <Button onClick={() => { setEditingPolicy(undefined); setIsModalOpen(true); }}>
      ...
      <p className="text-muted-foreground">Keep track of your current homeowner, flood, or other property insurance policies and renewal dates.</p>
      ...
      {!isLoading && sortedPolicies.length === 0 && (
        <Card className="text-center py-10">
          ...
          <CardDescription>Click "Add Policy" to record your first insurance policy.</CardDescription>
        </Card>
      )}
          ...
          {sortedPolicies.map(policy => {
            ...
            return (
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-xs font-semibold px-2 py-1 rounded-full text-white"
                      style={{ backgroundColor: isExpired ? 'rgb(220 38 38)' : 'rgb(34 197 94)' }}
                    >
                      {isExpired ? 'EXPIRED' : format(parseISO(policy.expiryDate), 'MMM dd, yyyy')}
                    </div>
                  </div>
                  <CardDescription>
                    Policy No: {policy.policyNumber} | {policy.coverageType}
                    {property && ` | Property: ${property.name || property.address}`}
                  </CardDescription>
                </CardHeader>
                <CardContent className="flex-1 space-y-3 pt-3 text-sm">
                    <p className="font-medium text-gray-700">Premium: ${policy.premiumAmount.toFixed(2)} / Annually</p>
                ...
                <Dialog open={editingPolicy?.id === policy.id && isModalOpen} onOpenChange={closeModal}>
                  <DialogTrigger asChild>
                    <div className="flex border-t">
                      <Button variant="ghost" className="w-1/2 rounded-none rounded-bl-lg text-blue-600" onClick={() => openEditModal(policy)}>
