...
export const BUYER_PLAN_WORKSPACES: Array<{
  ...
  { key: 'CONTRACT', eyebrow: 'Phase 1', label: 'Contract', description: 'Dates, terms and contingencies', icon: FileCheck2 },
  { key: 'DUE_DILIGENCE', eyebrow: 'Phase 2', label: 'Due diligence', description: 'Inspection and decisions', icon: ClipboardCheck },
  { key: 'FINANCING_PROTECTION', eyebrow: 'Phase 3', label: 'Financing & protection', description: 'Loan, title and insurance', icon: Landmark },
  { key: 'CLOSING_PREP', eyebrow: 'Phase 4', label: 'Closing preparation', description: 'Disclosure and walkthrough', icon: ShieldCheck },
  { key: 'CLOSE_MOVE_IN', eyebrow: 'Phase 5', label: 'Close & move in', description: 'Closing day and possession', icon: Flag },
];
              ...
              >
                <span className="flex items-center justify-between gap-2">
                  ...
                  {phaseTasks.length > 0 && resolvedCount === phaseTasks.length
                    ...
                    : <span className={`text-xs ${selected ? 'text-white/70' : 'text-slate-400'}`}>{resolvedCount}/{phaseTasks.length}</span>}
                </span>
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.45fr_0.85fr]">
      <section className="relative overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl shadow-slate-950/10 sm:p-8">
        <div aria-hidden className="absolute -right-20 -top-24 h-64 w-64 rounded-full bg-teal-400/20 blur-3xl" />
        <div className="relative">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-200">Your next move</p>
              <h2 className="mt-3 max-w-2xl text-2xl font-semibold tracking-tight sm:text-3xl">{nextAction?.title ?? 'Your closing plan is ready to review'}</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">{nextAction?.description ?? 'Open the current phase to confirm the next important dates and decisions.'}</p>
            </div>
            {blockedCount > 0 && <Badge className="border border-amber-300/40 bg-amber-300/15 text-amber-100">{blockedCount} blocked</Badge>}
          </div>
          <div className="mt-7 flex flex-wrap items-center gap-3">
            ...
            <Button type="button" variant="ghost" className="text-white hover:bg-white/10 hover:text-white" onClick={() => onOpenWorkspace(currentWorkspace)}>View current phase</Button>
          </div>
          <div className="mt-8 grid gap-4 border-t border-white/10 pt-5 sm:grid-cols-3">
            <div><p className="text-xs text-slate-400">Overall progress</p><p className="mt-1 text-xl font-semibold">{progressPercent}%</p><p className="text-xs text-slate-400">{completed} of {total} resolved</p></div>
            <div><p className="text-xs text-slate-400">Target closing</p><p className="mt-1 text-xl font-semibold">{closeDate ? closeDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : 'Not set'}</p><p className="text-xs text-slate-400">{daysToClose === null ? 'Add when known' : daysToClose >= 0 ? `${daysToClose} days remaining` : `${Math.abs(daysToClose)} days past target`}</p></div>
            <div><p className="text-xs text-slate-400">Current phase</p><p className="mt-1 text-xl font-semibold">{BUYER_PLAN_WORKSPACES.find((item) => item.key === currentWorkspace)?.label}</p><p className="text-xs text-slate-400">Guided, never locked</p></div>
          </div>
        </div>
      </section>
