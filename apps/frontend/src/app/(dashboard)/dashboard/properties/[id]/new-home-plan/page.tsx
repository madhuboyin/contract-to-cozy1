...
export default function NewHomePlanPage() {
  ...
  const builderResponseMutation = useMutation({ mutationFn: async ({ itemId, close }: { itemId: string; close: boolean }) => { const response = await api.addNewHomeBuilderResponse(propertyId, itemId, { responseType: 'RESOLVED', verifyClosure: close, message: close ? 'Resolution verified by homeowner.' : 'Builder reported resolution.' }); if (!response.success) throw new Error(response.message); }, onSuccess: async () => { await refresh(); } });
