// apps/frontend/src/app/(dashboard)/dashboard/properties/[id]/tools/cost-growth/MultiLineChart.tsx
'use client';
...
export default function MultiLineChart(props: {
  ...
  const safe = useMemo(() => {
    const n = Math.max(2, props.xLabels.length);
    ...
    return { xLabels: xLabels.slice(0, n), series };
  ...
  const { minY, maxY, yTicks } = useMemo(() => {
    ...
    // If range is tiny, pad so slopes are readable
    let min = rawMin;
    let max = rawMax;
  ...
  // Tick behavior:
  // - If <= 5 points: label all
  // - If more: first/mid/last (fallback)
  const xTicks =
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={props.ariaLabel || 'Home cost growth trend chart'}
    >
      {/* axes */}
      ...
      {safe.series.map((s, idx) => {
        ...
        // subtle style differences per series
        const dash = s.dash ?? (idx === 0 ? undefined : idx === 1 ? '6 5' : idx === 2 ? '2 4' : '8 6');
