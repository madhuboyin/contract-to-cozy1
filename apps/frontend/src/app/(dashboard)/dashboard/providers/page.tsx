  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // This call is authenticated and *should* return segment-specific categories.
        // The bug you found means the backend is not filtering this correctly.
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      try {
        ...
        // --- START: FIX ---
        // Replaced manual fetch with api client.
        // The api client automatically sends auth and handles token refresh.
        // This request now relies on the *backend* to correctly
        // filter providers based on the user's segment.
        const response = await api.searchProviders({
          category: serviceCategory || undefined, // Pass category if it exists
        });

        if (response.success) {
          // --- FIX: Use a stronger type assertion to resolve the conflict ---
    ...
    // Only fetch providers if a category is selected
    if (serviceCategory) {
      fetchProviders();
    } else {
      // No category selected, just stop loading.
      setLoading(false);
    }
  }, [serviceCategory]);
