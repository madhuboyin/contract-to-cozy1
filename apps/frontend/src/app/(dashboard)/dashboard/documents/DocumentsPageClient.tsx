// Property-scoped document center client.
'use client';
...
function useCanonicalPropertyId(): string | undefined {
  const params = useParams<{ id?: string | string[] }>();
  const searchParams = useSearchParams();
  const { selectedPropertyId } = usePropertyContext();
  const routePropertyId = Array.isArray(params.id) ? params.id[0] : params.id;
  return routePropertyId || searchParams.get('propertyId') || selectedPropertyId || undefined;
}
