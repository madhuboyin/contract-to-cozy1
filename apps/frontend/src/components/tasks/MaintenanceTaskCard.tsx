              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                {task.serviceCategory && (
                  <Badge variant="outline" className="text-[11px] sm:text-xs bg-gray-50 px-1.5 py-0">
                    ...
                    {humanizeActionType(task.serviceCategory)}
                  </Badge>
                )}
              </div>
            </div>
            ...
            {task.nextDueDate && (
              <div className={`flex items-center gap-2 ${dueDateStatus?.className}`}>
                ...
                {dueDateStatus && dueDateStatus.type !== 'upcoming' && (
                  <span className="text-[11px] px-1 bg-current/10 rounded font-bold">{dueDateStatus.label}</span>
                )}
              </div>
            )}
                ...
                <div className="mt-2 p-3 bg-gray-50/50 rounded-lg text-sm space-y-2 border border-gray-100 animate-in fade-in slide-in-from-top-1">
                  {task.assetType && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{humanizeActionType(task.assetType)}</span>
                    </div>
                  )}
                  {task.actualCost && (
                    <div className="flex justify-between">
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-blue-900">
                  You just maintained {humanizeActionType(task.title)}. Want to verify its details?
                </p>
