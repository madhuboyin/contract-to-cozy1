...
export function InventoryResultList({ block, propertyId, onFilter, onPage, onAccessLost, link }: {
  ...
  const detailItemId = controls?.view.detailTaskId ?? localDetailItemId;
          ...
          {section.items.slice(0, controls ? visible : section.items.length).map((item) => {
            ...
            return <li key={item.id} data-ask-task-id={item.id} tabIndex={-1} className={cn('rounded-xl border p-3 outline-offset-2', selected ? 'border-teal-600 bg-teal-50' : 'border-transparent bg-slate-50')}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <button type="button" data-inventory-detail-trigger={item.id} aria-expanded={detailItemId === item.id} aria-controls={`inventory-detail-${item.id}`} onClick={() => openDetail(item)} className="min-h-10 text-left font-medium text-slate-950 underline-offset-4 hover:text-teal-800 hover:underline">{item.title}</button>
