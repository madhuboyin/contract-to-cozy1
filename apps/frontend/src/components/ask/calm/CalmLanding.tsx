    ...
    <div className="mt-4" data-calm-landing="">
      <section aria-label="Your home today" data-calm-state-strip="">
        ...
        {strip && !loading && <>
          ...
          {strip.urgent && <button type="button" data-strip-urgent="" onClick={() => onAsk(strip.urgent!.prompt, 'ATTENTION')} className="mt-3 flex w-full items-center gap-2 rounded-lg py-1 text-left text-[15px] text-slate-700 transition hover:text-slate-950">
            <span className="shrink-0 text-slate-500">Top priority</span>
      ...
      {shownStarters.length > 0 && <ul className={cn(ROW, 'mt-4')} aria-label="Things you can ask">
