...
export function WinCard({
  ...
  return (
    <Card className={cn(
      ...
      <CardHeader className={cn("pb-2 pt-4", compactActionLayout && "pb-1 pt-3")}>
        <div
          className={cn(
            "flex items-center gap-2 text-xs font-medium text-gray-400",
            compactActionLayout &&
              "inline-flex w-fit rounded-full bg-teal-50/65 px-2.5 py-1 text-[0.8rem] font-semibold text-teal-800",
          )}
        >
          {icon || <Sparkles className={cn("h-3.5 w-3.5 text-brand-600", compactActionLayout && "h-3 w-3 text-teal-700")} />}
          {title}
        </div>
      </CardHeader>
      
      <CardContent className={cn("space-y-3 pb-4", compactActionLayout && "space-y-2.5 pb-3")}>
        <div>
          <h3 className={cn("text-2xl font-bold text-slate-900 leading-tight", compactActionLayout && "max-w-[30ch] text-[1.28rem] tracking-[-0.02em]")}>
            {value}
          </h3>
          {!hasExplicitTrust && (
          ...
          {description && (
            <p className={cn("mt-1 text-sm text-slate-600", compactActionLayout && "mt-1.5 max-w-3xl text-[0.86rem] leading-5 text-slate-500")}>
              {description}
            </p>
          )}
        </div>
            ...
            <div className="flex flex-col gap-2.5 border-t border-slate-100 pt-3">
              <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                <div className="flex min-w-0 items-start gap-2.5">
                  <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-emerald-100 bg-teal-50/50">
                  ...
                  {actionMetaLabel ? (
                    <p className="text-[0.68rem] font-semibold uppercase tracking-[0.11em] text-teal-800/60">
                      {actionMetaLabel}
                    </p>
                  ) : null}
                  {compactActionHeadline ? (
                    <p className="mt-1 text-[0.88rem] font-medium leading-5 text-slate-900 sm:text-[0.92rem]">
                      {compactActionHeadline}
                    </p>
                  ) : null}
              ...
              {(normalizedProgressPercent !== null || actionProgressValue || actionProgressLabel) ? (
                <div className="flex flex-col gap-1 pt-0.5">
                  {normalizedProgressPercent !== null ? (
                    <div className="h-1 w-full max-w-[24rem] overflow-hidden rounded-full bg-teal-50/80">
                      <div
      ...
      <CardFooter className={cn("bg-slate-50/50 border-t border-slate-100 p-0", compactActionLayout && "bg-slate-50/35")}>
        <p className={cn("w-full px-4 py-3 text-[11px] text-slate-500", compactActionLayout && "px-4 py-2 text-[0.76rem] text-slate-500")}>
