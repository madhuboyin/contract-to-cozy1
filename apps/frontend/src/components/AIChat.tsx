...
export function AIChat() {
  ...
  // ("100%"/h-full here) commonly does not shrink when the on-screen
  // keyboard opens, so a fixed-height sheet can end up with its composer
  // covered by the keyboard instead of resized above it. Track the actual
  // visual viewport height and drive the sheet's height from it; falls
  // back to 100% (the existing h-full behavior) when unsupported or closed.
  useEffect(() => {
  ...
  useEffect(() => {
    const open = (event: Event) => {
      ...
      const requestedBackTo = typeof detail?.backTo === 'string' ? detail.backTo : currentLocation;
      setInitialQuestion(question);
      setLaunchSurface(surface);
      setLaunchCapabilityId(capabilityId);
      setBackTo(resolveDashboardBackHref(requestedBackTo, currentLocation));
      setIsOpen(true);
    };
    ...
    <>
      {!isOpen && (
        <button
          type="button"
          ref={triggerRef}
          onClick={() => { setInitialQuestion(''); setLaunchSurface(''); setLaunchCapabilityId(''); setBackTo(`${window.location.pathname}${window.location.search}${window.location.hash}`); setIsOpen(true); }}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/30 backdrop-blur-[2px] lg:flex lg:items-end lg:justify-end lg:p-6" role="dialog" aria-modal="true" aria-label="Ask Cozy">
          ...
          >
            ...
            <AskWorkspace mode="panel" onClose={requestClose} onPendingStateChange={setHasPendingWork} initialQuestion={initialQuestion} initialBackTo={backTo} launchSurface={launchSurface} launchCapabilityId={launchCapabilityId} />
