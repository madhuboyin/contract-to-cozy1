  ...
  return <fieldset className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-200/40">
    ...
    <div className="flex flex-wrap gap-2.5">
      ...
      {schema.type === 'MULTI_SELECT' ? schema.options.map((option) => {
        ...
        }} className={`rounded-xl border px-3.5 py-2 text-sm font-semibold transition-colors ${selected ? 'border-teal-700 bg-teal-700 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-teal-300 hover:bg-teal-50'}`}>{option.label}</button>;
      }) : null}
      {schema.type === 'INTEGER' || schema.type === 'DECIMAL' ? <input aria-label={field.label} type="number" step={schema.type === 'INTEGER' ? 1 : 'any'} min={schema.min} max={schema.max} value={typeof value === 'number' ? value : ''} disabled={disabled} onChange={(event) => onChange(event.target.value === '' ? undefined : Number(event.target.value))} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15 sm:text-sm" /> : null}
      {schema.type === 'SHORT_TEXT' ? <input aria-label={field.label} value={typeof value === 'string' ? value : ''} maxLength={schema.maxLength} disabled={disabled} onChange={(event) => onChange(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15 sm:text-sm" /> : null}
      {schema.type === 'TIME' ? <input aria-label={field.label} type="time" value={typeof value === 'string' ? value : ''} disabled={disabled} onChange={(event) => onChange(event.target.value || undefined)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15 sm:text-sm" /> : null}
      {schema.type === 'APPROXIMATE_DATE' && approximate ? <div className="w-full space-y-3">
        <div className="flex flex-wrap gap-2" aria-label={`${field.label} precision`} role="group">
          {DATE_PRECISION_OPTIONS.filter((option) => permittedPrecisions.includes(option.value)).map((option) => <button key={option.value} type="button" disabled={disabled} aria-pressed={approximate.precision === option.value} onClick={() => onChange({ precision: option.value })} className={`rounded-xl border px-3.5 py-2 text-sm font-semibold transition-colors ${approximate.precision === option.value ? 'border-teal-700 bg-teal-700 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-teal-300 hover:bg-teal-50'}`}>{option.label}</button>)}
        </div>
        {approximate.precision !== 'UNKNOWN' ? <div className="flex flex-wrap gap-2">
          <input aria-label={`${field.label} ${approximate.precision === 'RANGE' ? 'start' : 'value'}`} type={approximate.precision === 'YEAR' ? 'number' : approximate.precision === 'MONTH' ? 'month' : 'date'} min={approximate.precision === 'YEAR' ? 1600 : undefined} max={approximate.precision === 'YEAR' ? new Date().getFullYear() : schema.allowFuture ? undefined : new Date().toISOString().slice(0, approximate.precision === 'MONTH' ? 7 : 10)} value={approximate.value ?? ''} disabled={disabled} onChange={(event) => onChange({ ...approximate, value: event.target.value || undefined })} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15 sm:text-sm" />
          {approximate.precision === 'RANGE' ? <input aria-label={`${field.label} end`} type="date" max={schema.allowFuture ? undefined : new Date().toISOString().slice(0, 10)} value={approximate.rangeEnd ?? ''} disabled={disabled} onChange={(event) => onChange({ ...approximate, rangeEnd: event.target.value || undefined })} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-base outline-none transition-colors focus:border-teal-600 focus:ring-2 focus:ring-teal-600/15 sm:text-sm" /> : null}
        </div> : <p className="text-[13px] leading-5 text-slate-500">We’ll keep the date unknown and avoid inventing precision.</p>}
      </div> : null}
