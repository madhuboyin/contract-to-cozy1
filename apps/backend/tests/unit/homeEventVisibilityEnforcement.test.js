...
test('the visibility route requires CONTRIBUTOR+ and restricts to the two household-internal choices', () => {
  ...
  assert.match(schemaBlock, /z\.enum\(\['PRIVATE', 'HOUSEHOLD'\]\)/);
});

test("Property Brief's VERIFIED_HISTORY assembly excludes PRIVATE events, leaving other visibility levels unaffected", () => {
  ...
  assert.match(queryBlock, /visibility:\s*\{\s*not:\s*'PRIVATE'\s*\}/);
});
