...
test('every represented Skill operation resolves to one registered immutable adapter', () => {
  ...
  // warranty.correct, room.rename and room.create (each a MUTATION_PREPARATION adapter under the property-record skill).
  assert.equal(Object.keys(SKILL_ADAPTER_DEFINITIONS).length, 72);
