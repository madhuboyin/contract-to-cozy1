...
test('adapter failure degrades or removes only operations that depend on that adapter', () => {
  ...
  const allDisabled = readAskOperationalControls({
    ...
    // The Inline Workspace correction and add adapters belong to the same skill, so "every adapter disabled" includes them;
    // with them enabled the skill is correctly only DEGRADED.
    ASK_ADAPTER_INVENTORY_ITEM_CORRECT_KILL_SWITCH: 'true',
    ...
    ASK_ADAPTER_INVENTORY_CREATE_KILL_SWITCH: 'true',
    ASK_ADAPTER_PROPERTY_CONTEXT_AREA_CAPTURE_KILL_SWITCH: 'true',
    ASK_ADAPTER_HOME_EVENT_VISIBILITY_KILL_SWITCH: 'true',
  });
