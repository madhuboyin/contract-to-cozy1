...
// Side-effect import: askOrchestrator.service.ts registers its 35 confirm
...
test('every one of the 35 confirmation-required Ask commands resolves to a registered confirm capability handler', () => {
  ...
  assert.equal(commandIds.length, 35);
...
test('each of the 35 extracted confirm handler functions is registered exactly once, by its command\'s own declared adapterKey', () => {
  const orchestrator = readFileSync(resolve(__dirname, '../../src/services/ask/askOrchestrator.service.ts'), 'utf8');
