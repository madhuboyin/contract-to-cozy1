...
test('every Ask operation has a complete governed definition', () => {
  ...
  // write slice) + HOME_EVENT_CORRECT (write slice 2) + WARRANTY_CORRECT (write slice 3).
  assert.equal(Object.keys(ASK_OPERATION_DEFINITIONS).length, 80);
...
test('every material Ask command has governed confirmation, authorization, cancellation, and correction metadata', () => {
  ...
  assert.equal(Object.keys(ASK_DOMAIN_COMMAND_REGISTRY).length, 33);
