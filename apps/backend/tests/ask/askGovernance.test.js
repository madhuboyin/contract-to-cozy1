...
test('every Ask operation has a complete governed definition', () => {
  ...
  // write slice) + HOME_EVENT_CORRECT (write slice 2) + WARRANTY_CORRECT (write slice 3) + ROOM_RENAME (write slice 4) + ROOM_CREATE (add slice) + INVENTORY_ITEM_CREATE (add slice 4).
  assert.equal(Object.keys(ASK_OPERATION_DEFINITIONS).length, 83);
...
test('every material Ask command has governed confirmation, authorization, cancellation, and correction metadata', () => {
  ...
  assert.equal(Object.keys(ASK_DOMAIN_COMMAND_REGISTRY).length, 36);
...
test('golden and negative prompts route before remote generation', () => {
  const cases = [
    ...
    ['Is this home record complete?', 'PROPERTY_SUMMARY'],
    ['Are all details complete for this property?', 'PROPERTY_SUMMARY'],
    ['What home details are left to fill?', 'PROPERTY_SUMMARY'],
    ...
    // Phase 10A: outcome observation.
    ['I already replaced my furnace, it cost $6,200', 'HVAC_DECISION_OUTCOME_REPORT'],
    ['My heat pump was repaired last week', 'HVAC_DECISION_OUTCOME_REPORT'],
