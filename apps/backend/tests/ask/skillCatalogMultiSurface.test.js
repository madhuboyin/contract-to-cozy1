...
test('the full representative taxonomy is registered and validates without semantic conflicts', () => {
  assert.equal(Object.keys(SKILL_DEFINITIONS).length, 44);
...
test('consumer-specific discovery returns only explicitly permitted operations', () => {
  ...
  assert.deepEqual(ask.operations.map(({ id }) => id), ['HOME_CHANGE_SUMMARY', 'HOME_EVENT_CORRECT', 'HOME_EVENT_VISIBILITY', 'INVENTORY_ITEM_CORRECT', 'INVENTORY_ITEM_CREATE', 'INVENTORY_LOOKUP', 'PROPERTY_CONTEXT_AREA_CAPTURE', 'PROPERTY_SUMMARY', 'ROOM_CREATE', 'ROOM_RENAME', 'WARRANTY_CORRECT']);
...
test('catalog projection applies Skill and operation controls independently', () => {
  ...
  assert.deepEqual(
    ...
    ['HOME_CHANGE_SUMMARY', 'HOME_EVENT_CORRECT', 'HOME_EVENT_VISIBILITY', 'INVENTORY_ITEM_CORRECT', 'INVENTORY_ITEM_CREATE', 'PROPERTY_CONTEXT_AREA_CAPTURE', 'PROPERTY_SUMMARY', 'ROOM_CREATE', 'ROOM_RENAME', 'WARRANTY_CORRECT'],
  );
