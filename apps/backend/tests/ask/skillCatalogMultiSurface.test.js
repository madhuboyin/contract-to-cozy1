...
test('consumer-specific discovery returns only explicitly permitted operations', () => {
  ...
  assert.deepEqual(ask.operations.map(({ id }) => id), ['HOME_CHANGE_SUMMARY', 'HOME_EVENT_CORRECT', 'INVENTORY_ITEM_CORRECT', 'INVENTORY_ITEM_CREATE', 'INVENTORY_LOOKUP', 'PROPERTY_SUMMARY', 'ROOM_CREATE', 'ROOM_RENAME', 'WARRANTY_CORRECT']);
...
test('catalog projection applies Skill and operation controls independently', () => {
  ...
  assert.deepEqual(
    ...
    ['HOME_CHANGE_SUMMARY', 'HOME_EVENT_CORRECT', 'INVENTORY_ITEM_CORRECT', 'INVENTORY_ITEM_CREATE', 'PROPERTY_SUMMARY', 'ROOM_CREATE', 'ROOM_RENAME', 'WARRANTY_CORRECT'],
  );
