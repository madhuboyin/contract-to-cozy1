...
# Run the reviewed Property Tax source/rule seed through a one-off Kubernetes
# Job. The backend image already contains the compiled JavaScript entry point,
# so the machine invoking this script needs kubectl, not Node.js or npm.
#
# This script intentionally runs:
#   node dist/scripts/seedReviewedTaxPilots.js
#
# It must never run `prisma db seed`: the comprehensive Prisma seed truncates
# users, properties, and related data.
#
# Usage:
#   ./run-property-tax-seed-job.sh
#
# Render the Job locally without contacting the cluster:
#   DRY_RUN=true ./run-property-tax-seed-job.sh
#
# Optional overrides:
...
JOB_NAME="${JOB_NAME:-property-tax-seed-$(date +%s)-${RANDOM}}"
...
metadata:
  ...
  namespace: ${NAMESPACE}
  labels:
    app.kubernetes.io/name: property-tax-seed
    ...
    spec:
      ...
      containers:
        - name: seed
          ...
          command:
            - node
            - dist/scripts/seedReviewedTaxPilots.js
          securityContext:
...
if [[ "${DRY_RUN}" == "true" ]]; then
  echo "Rendering the Property Tax seed Job; no cluster resources will be created." >&2
  render_job
  exit 0
fi
...
echo "Creating Property Tax seed Job ${NAMESPACE}/${JOB_NAME}..."
...
if "${KUBECTL[@]}" wait \
  ...
  echo "Property Tax seed completed successfully: ${NAMESPACE}/${JOB_NAME}"
  exit 0
fi

echo "Property Tax seed failed or timed out: ${NAMESPACE}/${JOB_NAME}" >&2
diagnose_job
exit 1
