# Signal & Action Audit (Normalized) — Final

**Generated:** 2026-03-17
**Source of truth:** `signal-action-audit-final.json`
...
| Metric | Count |
|--------|-------|
| Total normalized signals | 64 |
| Total signal families | 18 |
| Signals with no action (`isNoAction=true`) | 13 |
| Generic CTA count (`isGenericCta=true`) | 25 |
| Premature booking count (`isPrematureBooking=true`) | 8 |
| Backend exists / frontend weak gap | 61 |
...
|-------|-------|-------------|
| awareness | 21 | User learns about issue — no action path yet |
| diagnosis | 11 | User investigating severity |
| decision | 18 | User choosing action path |
...
- **`resilience_gap`**: Missing hardware or systems for risk mitigation.
- **`energy_inefficiency`**: Utility waste or missing efficiency data.
...
### 3.2 Missing Actions
**Critical Pattern**: 13 signals (including OPEN recalls, Coverage Lapses, and heavy rain forecasts) have no frontend action. The backend correctly identifies these risks, but the user is left stranded.

### 3.3 Generic CTAs
**Critical Pattern**: 25 signals use vague CTAs like "View Details" or "View Analysis". The Guidance Engine must replace these with specific, score-band or outcome-specific next steps.
...
5.  **Gamification → Disconnected**: Streak risks are detected but no "Streak Saver" micro-tasks are prominently pushed to the user.
...
| `resilience_gap` | Link to backup hardware DIY guide | Risk education bridge |
...
5.  **Gamification Loop**: No tight integration between "Streak Risk" and "Action Center" to prioritize streak-saving micro-tasks.

---
*End of Signal & Action Audit — Final Version*
