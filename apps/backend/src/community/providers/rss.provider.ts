...
// Minimal RSS parser (works for many CivicPlus / typical RSS feeds)
export async function fetchRssItems(feedUrl: string, limit = 20): Promise<RssItem[]> {
  const resp = await fetch(feedUrl, { headers: { 'User-Agent': 'contracttocozy/1.0' } });
  if (!resp.ok) throw new Error(`RSS fetch failed ${resp.status} for ${feedUrl}`);
