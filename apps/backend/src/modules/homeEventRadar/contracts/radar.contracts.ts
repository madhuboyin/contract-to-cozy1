...
export const radarDetailResponseSchema = radarFeedItemSchema.extend({
  ...
  relatedIncident: z.object({
    ...
    resolutionState: z.enum(['not_started', 'in_progress', 'resolved', 'closed']),
    title: z.string().min(1),
    summary: z.string().nullable(),
    resolvedAt: z.iso.datetime({ offset: true }).nullable(),
    expiredAt: z.iso.datetime({ offset: true }).nullable(),
    updatedAt: z.iso.datetime({ offset: true }),
    href: z.string().min(1),
  }).nullable(),
  ...
  resolutionContinuity: z.object({
    state: z.enum(['not_started', 'in_progress', 'resolved', 'closed']),
    incidentState: z.enum(['not_started', 'in_progress', 'resolved', 'closed']).nullable(),
    guidanceState: z.enum(['not_started', 'in_progress', 'resolved', 'closed']).nullable(),
    continueResolution: z.object({
      label: z.enum(['Start resolution', 'Continue resolution']),
      href: z.string().min(1),
    }).nullable(),
  }),
