...
export async function materializePilotRecommendationsForProperty(
  ...
  for (const definition of PILOT_DEFINITIONS) {
    const evaluation = await evaluateDefinitionForProperty(propertyId, definition.code, 'PILOT_READ');
    ...
    await upsertRecommendation({
      ...
      reasonCodes: [{ code: definition.reasonCode, templateKey: definition.reasonTemplateKey }],
