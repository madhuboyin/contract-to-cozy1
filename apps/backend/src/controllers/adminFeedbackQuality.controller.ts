...
// feedbackQualityAggregates.service.ts for what this does and does not
// cover yet.
...
export async function getFeedbackQualityHandler(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const byTargetType = await getFeedbackQualityAggregates({ since });
    res.json({ success: true, data: { byTargetType } });
  } catch (err) {
