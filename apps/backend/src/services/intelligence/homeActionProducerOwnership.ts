...
export const HOME_ACTION_PRODUCER_OWNERSHIP: readonly HomeActionProducerOwnershipEntry[] = [
  ...
  },
  {
    ...
    decisionLineagePolicy: {
      kind: 'VARIES_BY_INSTANCE',
      rationale: 'C2C Intelligence & Agentic Evolution Phase 4A (architecture §12.7): this one producer emits repair/replace actions for every ReplaceRepairAnalysis category. An HVAC item resolves to HVAC_REPAIR_REPLACE (`repair-replace:` lineage prefix); every other category resolves to APPLIANCE_REPAIR_REPLACE (`appliance-repair-replace:` prefix). Both are DECISION_REQUIRED (fixed MATERIAL_FINANCIAL governance); the per-action decisionDefinitionId is resolved at runtime by resolveActionDecisionLineagePolicy from the lineageId prefix. Each family fails closed per-item through its adapter eligibility gate (HVAC: composeHvacDecisionContext; appliance: non-HVAC ReplaceRepairAnalysis presence).',
    },
    notes: 'ACKNOWLEDGE only. Added Phase 1 Slice 2 (ReplaceRepairAnalysis) — the producer this Phase 0 pass adds a row for after it was missing from the original registry report. Governance is always fixed MATERIAL_FINANCIAL. Phase 4A (architecture §12.7) makes this producer category-aware: HVAC → HVAC_REPAIR_REPLACE, non-HVAC → APPLIANCE_REPAIR_REPLACE, hence VARIES_BY_INSTANCE above (was DECISION_REQUIRED/HVAC_REPAIR_REPLACE). The IPD-006-gated GENERIC_APPLIANCE agent profile is NOT part of Phase 4A — the appliance decision thread exists, the Specialist conversation on top of it does not yet. Home Intelligence FRD Phase 5 work item 2, rule 6 of 7 (HI-CMP-002): also batch-checks HomeEvent REPAIR/MAINTENANCE history per inventoryItemId (countRecentRepairEventsByInventoryItem, 30-month lookback, 2+ events) and enriches priority/evidence/whyItMatters when a recurring-failure pattern exists — id/lineageId/decisionLineagePolicy are unaffected.',
  },
  {
  ...
  },
  {
    ...
    outcomeAdapterOwner: null,
    endToEndOutcomeAdapters: [{
      owner: 'recordDecisionRecordOutcome (services/decisionPlatform/outcomeObservationService.ts)',
      completionPath: 'recordRefinanceDecision (refinanceRadar/refinanceDecision.service.ts)',
      conditions: 'A snapshot-backed refinance decision records a supported selection, start, deferral, or terminal outcome transition.',
    }],
  ...
  },
  {
    ...
    endToEndOutcomeAdapters: [{
      owner: 'recordDecisionRecordOutcome (services/decisionPlatform/outcomeObservationService.ts)',
      completionPath: 'createCanonicalAction/updateCanonicalAction and savingsOutcome.service.ts',
      conditions: 'A HIGH-confidence benefit match with locked recommendation lineage starts or completes a canonical action, or records a supported benefit outcome stage.',
    }],
    decisionLineagePolicy: {
      kind: 'VARIES_BY_INSTANCE',
  ...
  },
  {
    ...
    outcomeAdapterOwner: 'recordDecisionRecordOutcome (services/decisionPlatform/outcomeObservationService.ts), called transactionally by ownershipCostDecisionService.record on RESOLVE',
