...
export const HOME_ACTION_PRODUCER_OWNERSHIP: readonly HomeActionProducerOwnershipEntry[] = [
  ...
  },
  {
    ...
    decisionLineagePolicy: {
      kind: 'VARIES_BY_INSTANCE',
      rationale: 'C2C Intelligence & Agentic Evolution Phase 4A (architecture §12.7): this producer admits HVAC and APPLIANCE analyses only. HVAC resolves to HVAC_REPAIR_REPLACE (`repair-replace:`); APPLIANCE resolves to APPLIANCE_REPAIR_REPLACE (`appliance-repair-replace:`). Higher-risk categories abstain at ingress. Both admitted families are DECISION_REQUIRED and resolve per-action through the lineage prefix.',
    },
    notes: 'ACKNOWLEDGE only. Governance is fixed MATERIAL_FINANCIAL. Phase 4A routes HVAC → HVAC_REPAIR_REPLACE and APPLIANCE → APPLIANCE_REPAIR_REPLACE; the IPD-006-approved GENERIC_APPLIANCE profile supplies the shared Specialist conversation. PLUMBING, ELECTRICAL, ROOF_EXTERIOR, and STRUCTURAL do not fall back to this producer. HI-CMP-002 recurring-failure enrichment leaves lineage unaffected.',
  },
  {
