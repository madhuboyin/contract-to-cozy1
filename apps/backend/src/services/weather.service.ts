...
export class WeatherService {
  ...
  async getLocalSignals(zipCode: string): Promise<SignalType[]> {
    ...
    if (!zip) {
      console.warn('[WEATHER] getLocalSignals called with empty zipCode');
      return [];
    }
    ...
    if (cached) {
      console.log(`[WEATHER] Cache hit for zip=${zip}: [${cached.join(', ')}]`);
      return cached;
    }
    ...
    try {
      ...
      // 3. Map to internal signals
      const signals = this.mapResponseToSignals(data);
      console.log(`[WEATHER] Fetched signals for zip=${zip}: [${signals.join(', ')}]`);

      // 4. Store in cache
      this.setCache(zip, signals);
  ...
  private getFromCache(zip: string): SignalType[] | null {
  ...
  private setCache(zip: string, signals: SignalType[]): void {
    this.cache.set(zip, {
      signals,
