...
- **A frontend nav/UX audit found three independent, non-shared backend implementations** all answering a similar "what does this property need" question (the Home tab's canonical feed, the Fix hub's separate `resolution-center` service, and a narrower orchestration-summary fallback) — see [Known implementation alignment issues](00-introduction.md#known-implementation-alignment-issues) on the Introduction page for this and related findings (three onboarding entry points, Buyer Plan/Claims missing from the tool-discovery registry).
