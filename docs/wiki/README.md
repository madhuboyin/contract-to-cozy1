...
- **A frontend nav/UX audit found a homeowner-visible split**: the "Home" tab and the "Plan & Projects" nav tab render two differently-ranked "what needs attention" lists off two different Home Actions APIs — see [Known implementation alignment issues](00-introduction.md#known-implementation-alignment-issues) on the Introduction page for this and related findings (three onboarding entry points, Buyer Plan/Claims missing from the tool-discovery registry).
