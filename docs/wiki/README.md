...
This is a code-grounded technical snapshot, originally assembled by reading backend and frontend code. It is a map for finding workflows, not a standing certification of current behavior. Before using a feature page to plan a change, confirm the current route, service, schema, and reachable UI or job. Historical planning docs in `docs/functional/` and `docs/product/` remain sources of intent, but may describe superseded designs.
...
Each page below describes its cluster for both audiences at once — what the feature does for the user, and the implementation found when that page was researched.
...
- **Naming collisions to watch for**: "Home Event Radar" (hazard feed) vs. `homeEvents.routes.ts` (a property history timeline) are unrelated; `/dashboard/risk-radar` currently redirects through Protect to the property overview's risk-protection tab, while Home Event Radar has its own property tool; two separate quote-comparison surfaces (service quotes vs. insurance quotes) share similar names but different backends.
...
- **Home Actions is shared by the Home tab and Resolution Center:** the live Home Actions service calls `getOrchestrationSummary()` internally. The old standalone `/api/orchestration/*` route and decision-trace UI are absent. Resolution Center renders a projection of Home Actions and offers action reasons and source chips; see the [guidance page](features/03-guidance-ai-concierge-and-personalization.md#orchestration-next-best-action--home-actions).
- **A "Vault / Save / Protect" route consolidation retired roughly a dozen top-level `/dashboard/*` URLs** (`insurance`, `budget`, `appreciation`, `tax-appeal`, `inventory`, `documents`, `climate`, `risk-radar`, and others) into three redirect groups, some going two or three hops deep before landing on their real destination. Several feature pages previously described these old URLs as still-standalone; all known instances are now corrected.
- See [Known implementation alignment issues](00-introduction.md#known-implementation-alignment-issues) on the Introduction page for these and related findings (three onboarding entry points, Buyer Plan/Claims missing from the tool-discovery registry).

---
Originally compiled from the codebase in August 2026. The Home Actions, Resolution Center, and orchestration descriptions were rechecked against source paths on 2026-09-22. Other feature descriptions remain snapshots; trace the current route, service, and render path before using one to scope an enhancement.
