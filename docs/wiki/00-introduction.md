...
- **HIGH — A "Vault / Save / Protect" route consolidation happened after most of this wiki's feature pages were first written, and left over a dozen claims describing pre-consolidation URLs as still-standalone.** A fresh audit of `next.config.js` found three new redirect groups ("Vault consolidation," "Save consolidation," "Protect consolidation") retiring roughly a dozen top-level `/dashboard/*` URLs — `insurance`, `coverage-intelligence`, `risk-radar`, `climate` → `/dashboard/protect`; `home-savings`, `appreciation`, `expenses`, `budget`, `tax-appeal` → `/dashboard/save`; `inventory`, `documents`, `warranties` → `/dashboard/vault` — each of which is itself a further `JobHubRedirectPage` hop, and at least one (`/dashboard/properties/:id/vault`) redirects a *third* time, to the property's Inventory page. Every old `page.tsx` still exists on disk but is unreachable at its original URL — the same dead-at-old-URL pattern as `ActionsClient.tsx` below, just far more of it. [Home Health](features/02-home-health-inventory-and-maintenance.md), [Coverage, Risk & Financial Tools](features/04-coverage-risk-and-financial-tools.md), and [Home Events, Environment & Community](features/06-home-events-environment-and-community.md) have all been corrected at each specific claim this affects. Where a redirect chain goes more than one hop deep, this wiki traced the confirmed hops and explicitly flagged what wasn't independently re-verified beyond that, rather than guessing at the final destination.
