...
> **Correction, 2026-08-22:** this section's first two items originally claimed the "Plan & Projects" nav tab renders `/dashboard/actions`'s own page (`ActionsClient.tsx`), which calls `getOrchestrationSummary()`, competing with the Home tab's canonical `/home` feed — and that this page fails to show decisions/projects/major-moments. Both claims were wrong: `/dashboard/actions` is permanently redirected (`next.config.js`) to `/dashboard/resolution-center` before its own page ever mounts, which itself resolves to the Fix hub. The items below are corrected accordingly, and the actual finding is more interesting than the original one.

- **HIGH — Three independent implementations answer a similar "what does this property need" question, not two, and none share ranking logic.** Tracing the real reachable route chain (not just what each page's own code calls) found:
  1. **`GET /api/properties/:propertyId/home`** (`homeActions.service.ts`, canonical) powers the Home tab's "What needs attention" section (`UnifiedHomeSurface.tsx`).
  2. **`GET /api/properties/:id/resolution-center`** (`resolutionCenter.service.ts`, a separate ~1,100-line service with its own direct Prisma queries — it imports neither `homeActions.service.ts` nor `orchestration.service.ts`) powers the Fix hub, which is the *actual* destination of the "Plan & Projects" nav tab: sidebar link → `/dashboard/actions` → (server-side redirect in `next.config.js`) → `/dashboard/resolution-center` → (`JobHubRedirectPage`, resolves the active property client-side) → `/dashboard/properties/:id/fix`.
  3. **`GET /api/orchestration/summary/:propertyId`** (`orchestration.service.ts`) is live only as a narrow, lower-priority fallback signal on the top-level dashboard page wrapper (one candidate in a "next best move" waterfall, surfaced only when higher-priority local signals are absent) and in a few cross-tool components (`PropertyOrchestrationStrip`, `HomePulse`, `PriorityAlertBanner`) — not a second full ranked list competing with the Home tab.

  A homeowner does **not** see two different rankings on two different primary nav tabs (the original claim). The real issue is that three unrelated backend implementations independently compute similar "what needs attention" views with no shared logic and no reconciliation between them, which is still worth resolving before any of the three change in ways that make them diverge further. See [Guidance, Ask Cozy & Personalization](features/03-guidance-ai-concierge-and-personalization.md#orchestration-next-best-action--home-actions).
- **MEDIUM — The "Plan & Projects" nav tab's description overstates what its actual destination shows.** `jobsNavigation.ts` describes it as "Actions, decisions, projects, and major moments." Its real destination (the Fix hub, via the redirect chain above) genuinely covers actions and decisions — confirmed via `resolutionCenter.service.ts`'s `decisionInsights`/`counts.decisionsReady`/`counts.openCases` fields and the Fix hub's "Decisions ready" KPI and "Intelligence & Decisions" section — but that service has zero references to projects, renovations, or life-event context; "Active major moment" and the Project Tracker live only on the Home tab and a separate `/dashboard/properties/:id/projects` route, not here.
...
- **LOW — Stale/dead references left behind by the same nav-consolidation effort.** `/dashboard/actions`'s own page (`ActionsClient.tsx`, which calls `getOrchestrationSummary`) is permanently unreachable — redirected away before it ever mounts (see above). `resolution-center/ResolutionCenterClient.tsx` is likewise dead: zero live importers; its route now renders `JobHubRedirectPage` instead. `(dashboard)/layout.tsx` still has `job.key === 'fix'` conditional blocks (adding Maintenance/Seasonal sub-links) that can never fire — no entry in `PRIMARY_JOBS` has `key: 'fix'` anymore. `jobsNavigation.ts`'s "Home" job still lists `'home-gazette'` as a path-matching `engines` key even though Gazette is fully retired. `components/layout/RightSidebar.tsx` (586 lines, also calls `getOrchestrationSummary`) has zero importers anywhere in the frontend, consistent with `AppShell.tsx`'s own audit comment that it was removed. None of these are user-visible; they're dead code sharing one theme — several files from before the 5-tab nav consolidation were never deleted.
