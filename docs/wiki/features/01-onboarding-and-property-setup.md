...
- **User flow:**
  1. `/onboarding/address` — user picks their situation (own it / buying / new build / exploring), then either a purchase-stage sub-form (if buying) or a "what brought you here" trigger list (repair, replacement, contractor quote, maintenance backlog, insurance/warranty question, project planning, anticipated cost, or "just exploring"). Established owners enter only the address tuple; buyer and new-home branches retain their compact home-basics questions (dwelling type, year built, bedrooms, bathrooms, basement, pool/spa).
  ...
  3. `/onboarding/confirm` — shows the resolved address (editable before creation) and, for buyer/new-home branches, the home-basics fields for final review. On "Add home and see first action," it calls `POST /api/properties`, durably retains the committed Property ID in the short-lived onboarding session, then calls `PUT /properties/:propertyId/onboarding/entry-context` (via `captureEntryContext`). Activation retries and refreshes reuse that ID instead of attempting another create.
