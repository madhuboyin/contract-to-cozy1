...
- **Frontend:** MFA challenge UI is inlined directly into `app/(auth)/login/page.tsx` (toggles between password form and code form based on whether `mfaToken` state is set); there's no dedicated MFA settings page under `app/onboarding`, but MFA setup does have a confirmed home — **`MfaSettingsPanel`** is rendered on both `app/(dashboard)/dashboard/profile/page.tsx` (homeowner) and `app/providers/(dashboard)/profile/page.tsx` (provider). A prior version of this line hedged this as merely "assumed"; it's now confirmed directly.
...
- **User flow:**
  1. `/onboarding/address` — user picks their situation (own it / buying / new build / exploring), then either a purchase-stage sub-form (if buying) or a "what brought you here" trigger list (repair, replacement, contractor quote, maintenance backlog, insurance/warranty question, project planning, anticipated cost, or the exploration-only "just understand my home"). Established owners enter only the address tuple; buyer and new-home branches retain their compact home-basics questions (dwelling type, year built, bedrooms, bathrooms, basement, pool/spa). The frontend builder and short-lived session reject `NONE_EXPLORING` outside the `EXPLORATION` entry path before Property creation.
  ...
  3. `/onboarding/confirm` — shows the resolved address (editable before creation) and, for buyer/new-home branches, the home-basics fields for final review. On "Add home and see first action," it calls `POST /api/properties`, durably retains the committed Property ID in the short-lived onboarding session, then calls `PUT /properties/:propertyId/onboarding/entry-context` (via `captureEntryContext`). Activation retries and refreshes reuse that ID instead of attempting another create. Legacy sessions containing an incompatible exploration-only trigger present an inline goal correction; saving the correction retains any committed Property ID and resumes the second write without creating a duplicate.
  4. Redirects to `/onboarding/first-value?propertyId=...` independently of best-effort lookup-session cleanup, then calls `GET /properties/:propertyId/onboarding/first-value` to render one recommended action tied to the stated trigger. The user can attach evidence (free text, a document/photo/quote upload via `uploadDocument`, etc.) and resolve the action as completed / deferred (auto-snoozes 7 days) / not relevant, via `POST .../first-action-resolution`.
...
- **Notes:**
  ...
  - For the established-owner branch, home type and the other home-basics are deferred to Property Details and are not shown during setup. The address tuple plus the journey trigger is sufficient to create the first Property; buyer and new-home branches retain their specialized requirements.
...
- **Notes:** This is a genuinely different flow from `/onboarding/*` (the trigger-first wizard) despite similar naming — don't conflate the two when reading code or filing bugs. `/dashboard/properties/new` is the ordinary additional-property entry point. It collects only the structured address and an optional primary-home choice; autocomplete is an accelerator and manual entry remains available. Details, systems, photos, and records are collected progressively after creation. The zero-property welcome experience still uses trigger-first onboarding rather than redirecting to this route.
...
- **Frontend:** `app/(dashboard)/dashboard/properties/[id]/household/page.tsx`, `.../household/activity/page.tsx`, `app/invite/[token]/page.tsx`; components `components/features/household/MemberList.tsx`, `InviteMemberSheet.tsx`, `HouseholdUtils.ts` (role labels/descriptions).
