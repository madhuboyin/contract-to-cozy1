...
*   ✅ **Initiative 4: The Resolution Hub (`/fix`).** (Product) Unifed Decision (Replace/Repair), Search (Providers), and Execution (Bookings) into a single concierge surface.
*   **Success Metric:** Transition from fragmented tools to a unified "Surface vs. Engine" architecture.

---

### Phase 2: Hero Flows & Activation (Days 30–60) — ⏳ UP NEXT
*Goal: Perfect the "First 3 Minutes" and the core "Resolution" loops.*

*   **Initiative 1: Address-First Onboarding.** (Product/Eng) Implement the lookup-to-claim flow using public data APIs to populate the Command Center instantly.
*   **Initiative 2: The "Unexpected Repair" Flow.** (Product/Design) Build the world-class flow from "Issue" -> "Estimate" -> "Compare" -> "Book" -> "Track".
*   **Initiative 3: Error Boundary Hardening.** (Eng) Ensure frontend crashes in hidden background engines do not take down the primary Command Surfaces.
*   **Initiative 4: Zod Schema Sync.** (Eng) Ensure frontend and backend types are perfectly aligned to prevent data corruption during complex AI extractions.
*   **Success Metric:** Average Time to First Value (TTFV) < 180 seconds. Outcome Density > 1 (Every user experiences at least one "Win").
...
4.  ✅ **The Resolution Hub (`/fix`):** Moving from passive directories to active concierge management.
5.  **Address Lookup Onboarding:** Eliminates the "Data Entry Wall" that kills conversion.
...
*   **Dependency:** Reliable public data APIs (Zillow/Attom) for the address-lookup flow in Phase 2.
