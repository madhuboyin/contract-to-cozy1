...
| First/last frost-date normals | NOAA monthly temperature and degree-day normals are integrated, but the annual freeze percentile datatype identifiers and product-selected probability threshold have not been verified. | Keep `firstFrostDate` and `lastFrostDate` unavailable; do not derive dates from monthly averages. | A reviewed NOAA datatype/threshold contract and fixture-backed adapter are approved. |
| County and polygon Radar matching | Property has cached coordinates, but no canonical resolved county identifier. Radar polygon events have no documented GeoJSON/SRID/antimeridian contract or spatial index. | County and polygon events remain unmatched rather than falling back to city/ZIP or unsafe coordinate inference. | A canonical jurisdiction resolver and reviewed polygon storage/matching contract are implemented with boundary fixtures. |
| Cost-volatility climate shock years | Tax and insurance histories are canonical, but no reviewed FEMA/NOAA event-to-cost attribution dataset exists. | Do not manufacture climate shock years from regional sensitivity; keep the climate event list empty. | A sourced, versioned event history and attribution policy are approved. |
