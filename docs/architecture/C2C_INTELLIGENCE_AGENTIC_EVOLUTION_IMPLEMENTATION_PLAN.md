...
**Implementation status (2026-08-29): decision-platform family + category-aware ingress implemented (uncommitted); `GENERIC_APPLIANCE` agent profile deliberately NOT added (IPD-006).** Tasks 1–6 below are done: `APPLIANCE_REPAIR_REPLACE` is added to the `DecisionDefinitionId` union with a snapshot-style `DECISION_DEFINITIONS` entry and a thin (`composesFromPropertyContext: false`) `DECISION_CONTEXT_CONTRACTS` entry; `applianceDecisionFamilyAdapter.ts` is a `createSnapshotDecisionFamilyAdapter` config that projects the already-authoritative non-HVAC `ReplaceRepairAnalysis` (never recomputes a verdict), excludes HVAC at its eligibility boundary (`APPLIANCE_REPAIR_REPLACE_ELIGIBLE_CATEGORIES` = every inventory category except `HVAC`, chosen to line up exactly with the ingress prefix rule), and carries an explicit exhaustive `ReplaceRepairVerdict → REPAIR|REPLACE` table (`REPLACE_NOW`/`REPLACE_SOON → REPLACE`, `REPAIR_AND_MONITOR`/`REPAIR_ONLY → REPAIR`) plus `ReplaceRepairAnalysis.id` provenance via `canonicalFactReferences`; the `decisionFamilyAdapterRegistry.ts` entry is registered. Category-aware ingress: `loadRepairReplaceDecisionActions` selects the existing `inventoryItem.category` and picks the `lineageId` prefix per analysis (`repair-replace:` for HVAC, new `appliance-repair-replace:` otherwise; `id` untouched); `PREFIX_TO_DECISION_DEFINITION` gains the `appliance-repair-replace:` → `APPLIANCE_REPAIR_REPLACE` entry; `resolveWorkItemDecisionFamilyRefs`'s `GUIDANCE` branch selects `inventoryItem.category` and picks the family the same way. The `loadRepairReplaceDecisionActions` producer's `homeActionProducerOwnership` row moves from `DECISION_REQUIRED`/HVAC to `VARIES_BY_INSTANCE` with a rationale. Envelope wiring: a new closed `APPLIANCE_REPAIR_REPLACE_VERDICT` qualified-claim proposition type + `envelopeMappingRegistry` decision mapping (domain `ASSET_LIFECYCLE`), and a `REPAIR|REPLACE` domain-owned incompatibility entry in `qualifiedClaimCompatibilityRegistry`. No schema change (the adapter reads existing `ReplaceRepairAnalysis`); `npx tsc --noEmit` clean. Focused tests: `applianceDecisionFamilyAdapter.test.js` (verdict table, eligible-category boundary, registry wiring, proposition type, conflict detection) plus updated routing/lineage/promotion coverage. Task 7 (`GENERIC_APPLIANCE` in `repairReplaceProfileRegistry.ts`) and the agent-specialist conversation on top of the thread remain gated on `IPD-006` (evaluation contract + reviewed verdict mapping). Remaining: resolve `IPD-006`, then add the profile; commit; live-verify. Pre-existing unrelated red tests noted: two decision-platform emitter governance tests, one Ask non-HVAC routing test, one `WORK_ITEM_ELIGIBLE_SOURCE_KINDS` `SALE_PREP` test.
...
| 13 | Generic-appliance Decision Platform family and category-aware ingress (implemented; `GENERIC_APPLIANCE` agent profile still `IPD-006`-gated) | PR 10; `IPD-006` for the profile |
