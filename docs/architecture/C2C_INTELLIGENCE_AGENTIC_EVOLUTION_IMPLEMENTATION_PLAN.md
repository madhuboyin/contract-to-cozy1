...
**Implementation status (2026-08-29): Complete.** The implemented service is `intelligenceEnvelopeQuery.service.ts`. It authorizes through the canonical property-access resolver before invoking any producer reader; applies the closed producer/filter contract, stable ordering, query-bound cursor, total/per-producer latency budgets, and isolated diagnostics; and exposes a bounded Ask presentation. Producer reads now page iteratively until the requested filtered page is provably complete (including same-timestamp ties) or the producer is exhausted; a budget exhaustion is diagnostic rather than a silent truncated result. Coverage reads traverse every native page within the same explicit budgets. The `query-envelope` Skill, read-only adapter, dependency contract, evaluation package, and operation are registered. Every Skill manifest now declares autonomy explicitly (`0` observe, `1` material read/recommendation, `2` confirmation-gated mutation preparation), and startup validation rejects autonomy below declared materiality/effects or above the platform ceiling. This does not broaden the Envelope to ordinary domain records or create another Home Action ranking path.
...
**Implementation status (2026-08-29): Complete.** HVAC-category `ReplaceRepairAnalysis` records act only as review triggers and supporting evidence. `loadRepairReplaceDecisionActions` fails closed unless exactly one active HVAC Decision Thread has current context and a supported current snapshot verdict; missing, stale, invalid, or ambiguous lineage produces neutral review copy. Verdict-dependent copy, option recommendation, confidence, priority, published-verdict evidence, and source revision come only from that snapshot; neutral urgency may still come from canonical non-verdict evidence such as recurring failures. The same authority boundary is enforced outside Home Action promotion: homeowner generic-analysis reads suppress HVAC rows, generic HVAC run requests fail with `HVAC_DECISION_PLATFORM_REQUIRED`, focused Ask inventory requests route to the HVAC Decision Platform operation, Resolution Center excludes generic HVAC analyses, and direct Inventory/Guidance surfaces route to the tracked HVAC decision without displaying or branching from the generic verdict. Non-HVAC analysis behavior and the canonical Home Action ranking, eligibility, and delivery owners are unchanged.
...
**Implementation status (2026-08-30): Functionally complete except for the owner decision in `IPD-002`; scheduled execution is hard-gated and not live-verified.** Delivery increments 5–7 are implemented. The hand-authored `COVERAGE_MANIFEST`, explicit non-actionable registry, startup validation, semantic digest, declared/authorized-observed union projection, evidence-basis classification, and exact declaration-drift certification are implemented. `CoverageAuditFinding` persists the coarse natural key and observation lifecycle; the audit reads properties in stable ID pages, resolves the real homeowner principal, and drains each authorized producer once per property through the internal coverage view rather than replaying all native rows for every public Envelope display page. Unresolved owners, adapter/time-budget failures, and unmapped native values make the run partial and prohibit retirement. The audit has no imports from promotion, ranking, eligibility, or delivery owners. No migration script was created. The `IPD-009` durable `CoverageAuditRun` model and repository provide idempotent per-attempt creation, atomic CAS finalization/reconciliation, CAS failure, and explicit terminalization of interrupted attempts; BullMQ retries use a fresh attempt identity and restart globally. Runs require a concrete deployment revision. Exact declaration-drift details are persisted separately from coarse findings and returned by the admin API/UI. Worker startup validates both intelligence-owner and Envelope registries, and worker build/install regenerates and synchronizes the Prisma client. The typed `envelopeCoverageEvaluationContract.ts` seam enumerates every required `IPD-002` decision; because no approved values exist, it reports `NOT_MEASURED` and installs a non-overridable scheduled-execution block that a per-job environment flag cannot bypass. Manual admin inspection remains available through the shared renewable-lease handler. Remaining: the user applies the Prisma schema change (`CoverageAuditRun.declarationDriftDetails`), the accountable owner resolves `IPD-002`, and the workflow is live-verified. A migration script is intentionally not included.
...
**Implementation status (2026-08-30): complete, including the comprehensive Phase 2 closure audit.** The code-owned multi-version `AgentDefinition` registry, canonical digest baseline, active-version lookup, startup parity checks, referenced-version deployment-readiness check, HVAC profile, bounded tool loop, typed claims, persistence/retention, authenticated API, and engagement UI are implemented. Fresh HVAC runs use definition `1.2.0`, bound to evaluation contract `agent-hvac-repair-replace-eval@1.1.0`; prior versions remain registered for pinned continuations. No `AgentDefinition` database model was added and HVAC scoring remains owned by the Decision Platform engine.
...
**Closure audit (2026-08-30):** the authenticated controller/route returns only the bounded status projection. The four-state panel now uses the canonical query cache after every continuation, supports upload → inventory-item attachment → CAS resume for `NEEDS_DOCUMENT`, and exposes `DISPUTE_INPUT` for both paused and terminal runs. Fact and HVAC document changes synchronously mark the Decision Thread stale before the request completes, and the engine recognizes a linked inspection report or estimate as technician evidence. Runtime persistence commits the terminal run, reservation fence, consumed prior state, optional next pause, and invocation audits in one transaction. A reclaimed lease is correlation-fenced; the minimum lease exceeds the execution budget; abandoned `RESUME_IN_PROGRESS` claims recover after lease expiry; and missing/`UNSPECIFIED` deployment revisions fail closed. The v1.1 evaluation corpus contains 11 cases, genuinely executes pause → resume, enforces the declared low-confidence homeowner escalation, counts only recommendation-ready outcomes as deterministic completion, and is an explicit CI step. Actual LLM cost overrun abstains rather than publishing a recommendation.

### 7.5 Phase 2 verification and acceptance

**Verification (2026-08-30):** backend and frontend TypeScript checks pass. Agent-focused coverage includes immutable definition/evaluation parity, reservation correlation fencing, atomic episode persistence, expired in-progress recovery, deployment provenance, bounded dispute audit, actual-cost enforcement, low-confidence escalation, and true two-invocation evaluation. No development/test services or live database were started; full Prisma orchestration remains integration-only per §12.
...
**2026-08-30 comprehensive functional follow-up:** Phase 3's homeowner workflow is now continuous rather than status-only. Ask submits bounded HVAC fact answers and disputes through the shared Specialist runtime with CAS, sends document recovery to the focused canonical panel, resolves multi-action follow-ups deterministically, and uses valid `home-operations` destinations. Envelope “show more” consumes the prior opaque query cursor. General no-property questions use only published or code-owned approved guidance. Remote fact retrieval includes common homeowner-to-canonical vocabulary, and cost comparison candidates are admitted only when currency representation and monthly/annual/one-time/balance basis are compatible; percentages and cross-basis values abstain. These changes preserve the single `getHomeActionFeed()` ranking owner and the no-free-form-remote-prose boundary.
...
- `SpecialistAdmissionGate` (7 gates) + `REQUIRED_GATES_BY_CLASSIFICATION` — every classification must explicitly clear all seven reviews. Reuse can make a review short, but never optional, and any `FAIL` revokes admission.
...
- `validateSpecialistAdmissionRegistry` — wired into `index.ts` startup and the `test:phase4b` CI gate alongside the executable `validateAgentDefinitionRegistry` and `validateRepairReplaceProfiles` suites. It requires reviewer identity plus an approval reference, rejects future-dated reviews, binds exact profile metadata, DecisionDefinition and context-contract content digests, adapter authority/version metadata, evaluation-corpus digests, canonical AgentDefinition version digests, and the runtime activation/lineage manifest. The shared EXPLAIN path also renders the admitted profile's professional boundary as a mandatory deterministic claim.
- `repairReplaceProfileCatalog.ts` is the immutable activation contract consumed by promotion, Decision Platform lineage, the Specialist runtime, dispute controls, and the Home UI projection. Admission status is re-derived at runtime; a declared `ADMITTED` value is never trusted on its own.

Net effect: `GENERIC_APPLIANCE` is admitted only because all seven gates and every bound executable artifact pass. A later candidate fails CI/startup if its review, artifacts, activation binding, or profile metadata is incomplete, and an invalidated admission is not selectable by the runtime. Tests: `tests/agents/specialistAdmissionRegistry.test.js` plus the composite `test:phase4b` command.
