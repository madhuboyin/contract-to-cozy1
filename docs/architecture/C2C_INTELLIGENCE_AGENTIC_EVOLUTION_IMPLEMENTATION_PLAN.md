...
- **§7.3.5 audit:** the loop accumulates one bounded `PendingToolInvocation` per tool call (`REQUEST_CONTEXT` / `REQUEST_DOCUMENT` / `SCORE` / `EXPLAIN`) and the runtime persists them as `ToolInvocation` rows against the run id (a resumed invocation's calls join the original run's trail under a fresh correlation id).
...
- **§7.3.9 / §7.3.11 narration:** `agentLlmPurpose.contract.ts` defines the closed `AGENT_LLM_PURPOSES` set and `narrateTypedClaims` — the single `EXPLAIN` entry point. Deterministic typed claims are authoritative; a provider (dormant behind `AGENT_HVAC_NARRATION_LLM_ENABLED`, default off, no route registered yet) may only return a subset/reorder of the closed set, with structured-output validation discarding any invented claim or over-cap result. `LlmInvocation` rows are written only when it runs.
...
**Test coverage (2026-08-29):** `tests/agents/` — `agentDefinitionRegistry` (key/version/digest immutability, missing-reference, DEV-boundary, deployment-readiness), `agentRuntimePersistence` (reservation claim/idempotency/P2002 race, write-once terminal run, `AgentState` CAS advance + stale-writer rejection, resolve-once), `agentRetention` (90/30/7 clamps, batch order, immediate erasure), `agentRuntimeGuards` (auth fail-closed, CAS conflict, intake-key rejection, trigger-handler parity, bounded hash), `hvacSpecialistLoop` + `hvacSpecialistEvaluation` (`IPD-005` fixture contract), `hvacSpecialistContextIntake`, and `agentAuditAndBoundary` (per-tool `ToolInvocation` accumulation, `CONTEXT_UNAUTHORIZED` gate, `narrateTypedClaims` closed-set validation, pause-expiry fail-closed, the static "no Prisma for property facts" boundary + "runtime touches only its own tables" assertions). Follow-up (`IPD-004`) tests are N/A. Full-orchestration happy-path idempotency (`invokeAgentRuntime` against live Prisma) remains integration-only per §12. HVAC engine / Decision Platform / Ask regression suites continue to pass.
