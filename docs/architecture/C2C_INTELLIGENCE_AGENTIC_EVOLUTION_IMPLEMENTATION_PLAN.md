...
- `AgentRunReservation` (`IPD-007`): unique `idempotencyKey`, pinned agent/version, principal/property/thread references, `leaseExpiresAt`, nullable unique `resultRunId`, own `expiresAt` for the stale-lease sweep;
- `AgentRun`: immutable single terminal insert — pinned agent/version/`definitionDigest`/`deploymentRevision`, trigger + unique `idempotencyKey`, principal/property/thread and origin Home Action/Ask references, budget-usage counters, terminal `outcome`, correlation ID, `startedAt`/`finishedAt`, and a creation-stamped `expiresAt`; no `RUNNING` state, no `updatedAt`;
...
**Implementation status (2026-08-29): PR 10 implemented (uncommitted); LLM narration + real HVAC context-intake writes are documented seams.** `IPD-004` is resolved (follow-up excluded from v1), so the runtime and the four-tool read loop are built. The typed operations `START_OR_RESUME`, `SUBMIT_CONTEXT`, `DISPUTE_INPUT`, and `GET_STATUS` are implemented (no `CONFIRM_FOLLOW_UP` / `CANCEL_FOLLOW_UP` in v1); every operation re-authorizes property access through the canonical resolver and verifies the expected `AgentState` `casVersion`; a resume pins the originating definition version. The bounded `selectNextTool` loop enforces per-tool attempts, total iterations, and time/context/LLM/cost budgets from the definition, clears its own state flags, and abstains as a first-class outcome. `hvacRepairReplaceSpecialist.service.ts` drives the canonical `hvacDecisionFamilyAdapter` (create/resume `DecisionThread` + immutable `RecommendationSnapshot`) and reads the verdict only from a `contextStatus = CURRENT` snapshot — it never recomputes HVAC scoring or reads a generic verdict. `EXPLAIN` composes a deterministic explanation from a closed typed-claim registry keyed off the snapshot's own `reasonCodes` / `limitationCodes`; an injected `narrationProvider` seam (default: deterministic) is where a governed LLM call attaches later, and `LlmInvocation` audit rows are written only when it runs. `agentPropertyContext.service.ts` wraps `getPropertyContext` with a typed `AgentContextRequest` and the real resolved-owner user ID (`requestingAgentId` is attribution only). `SUBMIT_CONTEXT` validates the homeowner's structured intake and delegates the actual inventory/quote/fact persistence to an injected `contextIntakeHandler` seam, then resumes. Bounded `ToolInvocation` audit rows and per-run metrics are written. No parallel learning store — the `DecisionThread` / `RecommendationSnapshot` chain remains the only outcome-attribution surface, and existing `OutcomeObservation` paths are unchanged. PR 11 adds the Home Action engagement UI and closes the `IPD-005` evaluation.
