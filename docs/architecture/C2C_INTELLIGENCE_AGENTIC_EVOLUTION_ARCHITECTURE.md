...
**Implementation status (2026-08-29): the Phase 4B admission process is an enforced code artifact.** `HVAC` and the IPD-006-approved `GENERIC_APPLIANCE` profile are `ADMITTED`; electrical, plumbing, roofing, and structural remain explicit `NOT_ADMITTED` scaffolds. Startup/CI validation continues to fail closed on missing admission evidence, contradictory status, or an improperly admitted higher-risk category.
