...
**Date:** 2026-08-26
**Status:** Approved target architecture — implementation in progress; owner inputs remain phase-gated where documented.
...
**Scope:** This document is the approved Stage 3 target architecture and phased evolution plan. Implementation is tracked separately in `C2C_INTELLIGENCE_AGENTIC_EVOLUTION_IMPLEMENTATION_PLAN.md`.
...
### 1.2.1 `IPD-008` adapter-mapping resolution

Approved on 2026-08-28: Envelope mappings are immutable, versioned, and code-owned. Closed native registries map exhaustively. Open-ended `IntelligenceObservation.observationType` and Radar event type/subtype values enter through explicit admission lists; a new value produces `UNMAPPED_NATIVE_VALUE` and fails certification rather than being inferred from a label, source family, affected asset, or fallback `OTHER`.

The approved Signal primary domains are `MAINT_ADHERENCE→MAINTENANCE`, `COVERAGE_GAP→INSURANCE`, `SAVINGS_REALIZATION→FINANCIAL`, `RISK_SPIKE→SAFETY`, `COST_ANOMALY→PRICING`, `RISK_ACCUMULATION→MAINTENANCE`, `SYSTEM_DEGRADATION→ASSET_LIFECYCLE`, `COST_PRESSURE_PATTERN→FINANCIAL`, and `FINANCIAL_DISCIPLINE→FINANCIAL`. Guidance uses identity mapping from the shared issue taxonomy. Decision and personalization definitions, admitted observation types, Radar event types, and Radar compound-rule codes each have reviewed explicit mappings.
...
**What generalizes across canonical `APPLIANCE` items (major kitchen and laundry appliances and similar-shaped repair/replace decisions)? Two separate registries, not one — and one of them may already exist.** **[verified]** `replaceRepairAnalysis.service.ts`'s `ReplaceRepairService` already computes the four native verdicts. **IPD-006 narrows this agentic family to the canonical `APPLIANCE` category. Water heaters are canonically `PLUMBING` and remain excluded with the other higher-risk systems even if the underlying service can score them.** This document does not build a second appliance-classification catalog. The generalization is:
...
└── RepairReplaceProfileRegistry (NEW, agent-internal — not decisionPlatform, not a classification catalog)
    ...
    └── GENERIC_APPLIANCE profile — decisionDefinitionId: APPLIANCE_REPAIR_REPLACE
                                     ...
                                     (unchanged) — this profile covers canonical APPLIANCE items such as
                                     dishwashers, refrigerators, and washers/dryers; it does not admit
                                     PLUMBING through name-based matching

decisionFamilyAdapterRegistry.ts (existing, decisionPlatform — unchanged mechanism)
├── HVAC_REPAIR_REPLACE       (existing, unchanged)
└── APPLIANCE_REPAIR_REPLACE  (new entry, one DecisionThread-lineage adapter for admitted APPLIANCE items)
```
...
`createSnapshotDecisionFamilyAdapter` supplies `isEligiblePrimaryEntity`, digest-based supersession, no-op resume, and thread create/resume/ambiguity handling. IPD-006 approved `agent-generic-appliance-repair-replace-eval@1.0.0`: a 10-case checked-in corpus, zero LLM calls, 100% deterministic completion for non-abstention cases, and a 20–50% abstention band.
...
1. **`loadRepairReplaceDecisionActions`** (`homeActionSourcePromotion.service.ts`) admits only `HVAC` and `APPLIANCE`; it uses `repair-replace:` for HVAC and `appliance-repair-replace:` for APPLIANCE. Higher-risk categories emit no fallback decision action. `id` remains untouched.
...
3. **`resolveWorkItemDecisionFamilyRefs`'s `GUIDANCE` branch** selects the category and returns a decision ref only for `HVAC` or `APPLIANCE`; every other category abstains rather than falling through.
...
**Functional remediation (2026-08-30):** the Ask Specialist adapter now carries a paused conversation through the runtime's existing `SUBMIT_CONTEXT` and `DISPUTE_INPUT` operations with the current CAS version; fact replies are parsed only for the closed HVAC intake keys, documents and corrections deep-link to the focused canonical Home Operations Specialist panel, and multi-action clarification offers routable prompts plus deterministic unique entity matching. Envelope pagination remains server-side and opaque: “Show more intelligence” reuses the prior execution's query-bound cursor instead of restarting page one. Property-less `GENERAL_HOME_GUIDANCE` now reads published Knowledge Hub material with a bounded code-owned educational catalog as the storage-unavailable fallback. Property fact selection recognizes homeowner HVAC, urgency, deadline, and cost vocabulary. Typed cost comparisons normalize cents/dollars, require the same temporal basis, reject percentages, and render currency in code; incomparable values never become candidates.
...
| **Exit criteria** | A homeowner engaging with a delivered canonical `APPLIANCE` Home Action receives the same decision-support conversation shape HVAC already gets, backed by `ReplaceRepairService` and a real `APPLIANCE_REPAIR_REPLACE` `DecisionThread`; higher-risk categories abstain |
