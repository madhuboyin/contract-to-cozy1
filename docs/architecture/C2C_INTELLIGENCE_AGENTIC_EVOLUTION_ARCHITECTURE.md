...
**Implementation status (2026-08-29): Phase 4A decision-platform family + category-aware ingress implemented (uncommitted); `GENERIC_APPLIANCE` agent profile IPD-006-gated.** `APPLIANCE_REPAIR_REPLACE` is a fully registered Decision Platform family — `DecisionDefinitionId` union entry, snapshot-style `DECISION_DEFINITIONS` entry, thin `DecisionContextContract`, `applianceDecisionFamilyAdapter.ts` (a `createSnapshotDecisionFamilyAdapter` config projecting the authoritative non-HVAC `ReplaceRepairAnalysis` with an explicit exhaustive 4-value verdict table and `ReplaceRepairAnalysis.id` provenance; HVAC excluded at the eligibility boundary), and the `decisionFamilyAdapterRegistry.ts` entry. Category-aware ingress is wired through `loadRepairReplaceDecisionActions`' `lineageId` prefix selection, `PREFIX_TO_DECISION_DEFINITION`, and `resolveWorkItemDecisionFamilyRefs`. `qualifiedClaim` support: a closed `APPLIANCE_REPAIR_REPLACE_VERDICT` proposition type + envelope mapping + `REPAIR|REPLACE` incompatibility entry. No schema change. The `GENERIC_APPLIANCE` `RepairReplaceProfile` entry and the Specialist conversation on top of the thread are deliberately not added — they remain gated on IPD-006's evaluation contract and reviewed verdict mapping. A non-HVAC repair/replace Home Action now reaches a real `DecisionThread` instead of failing closed under the HVAC family; the agent layer follows once IPD-006 lands.
...
**Implementation status (2026-08-29): the Phase 4B admission process is an enforced code artifact.** `apps/backend/src/services/agents/specialistAdmissionRegistry.ts` encodes §12.6 / §9.2 as: a closed three-way `SpecialistAdmissionClassification`; a 7-gate review checklist with per-classification required subsets (`EVALUATION_SUITE` always required — the "same bar as an AgentDefinition" rule); `HIGHER_RISK_INVENTORY_CATEGORIES` that force the safety-tier and autonomy-ceiling gates onto any candidate touching electrical / plumbing / roofing / structural regardless of classification; and one `SpecialistAdmissionRecord` per candidate with per-gate `PASS`/`FAIL`/`NOT_REVIEWED` + evidence. `HVAC` is the `ADMITTED` reference; `GENERIC_APPLIANCE` is `PENDING_REVIEW` (blocked on IPD-006's evaluation-suite gate); the four higher-risk families are explicit `NOT_ADMITTED` scaffolds. `validateSpecialistAdmissionRegistry` runs at startup / CI (`index.ts`, beside `validateAgentDefinitionRegistry`) and fails closed if a registered `RepairReplaceProfile` or non-`DEV` `AgentDefinition` has no `ADMITTED` record, if a declared status contradicts its gates, or if a higher-risk category is `ADMITTED` without its forced gates cleared. No new specialist is admitted — that was never Phase 4B's deliverable.
