...
**Not yet built:** the bidirectional `linkedExecutionId` pairing and `ASK_CAPTURE_LINK_RECONCILE` consumer for the event+warranty case (the schema fields and the `DomainEventType` member exist; no writer or worker `case` branch uses them yet — that's this phase's one remaining item).
