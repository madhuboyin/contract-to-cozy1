...
**Status (2026-09-13): shipped and code-review-clean, behind `askConversationalCaptureEnabled` (default OFF) — but NOT complete against this section's own Work list.** All findings from four external review rounds are fixed and verified (see below); two of this phase's own requirements remain genuinely unfinished, not merely deferred polish: **edit-before-confirm for capture candidates**, and **the warranty capture writer + the paired-confirmation acceptance scenario** (§9's own "event+warranty pairing" example is still unbuildable end-to-end). Neither is a corner case -- both are named directly in this phase's Work list and acceptance criteria. "Code-review-clean" and "phase complete" are different claims; this phase is the former, not yet the latter. Eight commits, each independently tsc-clean and targeted-test-verified:
...
**What remains genuinely unfinished (not deferred polish) as of 2026-09-13:**
- **Edit-before-confirm for capture candidates is not built.** No mechanism lets a homeowner correct a proposed candidate's value before confirming it; an invalid FACT value is now dropped pre-proposal (round 3's fix) rather than editable, which narrows the harm but does not satisfy this phase's own edit-before-confirm requirement.
- **The warranty capture writer does not exist, and the paired-confirmation acceptance scenario cannot run end-to-end.** `linkSiblingCaptureExecutions`/`ASK_CAPTURE_LINK_RECONCILE` are wired and reachable (Phase 2 review fix), but nothing produces a Warranty-side capture to pair with a HomeEvent -- there is no `CAPTURE_WARRANTY_CONFIRM`-equivalent operation, no extraction candidate type for it, and no writer setting `Warranty.sourceExecutionId`. §9's own event+warranty pairing example remains unbuildable today.
