...
**External review, second round, 2026-09-13: 1 more [P1], the residual half of round one's own finding, deliberately left open at the time — now closed.** The reviewer re-flagged that a plain typed question with no `launchContext` (the common case — most Ask turns are not launched from a Home Action or Journey card) still got fully property-wide, un-differentiated ranking: "the current question and answer never affect relevance... unrelated questions about the same property... still receive largely identical recommendations, aside from exclusions." True, and already named as a residual gap in round one's own fix comment ("deliberately does NOT thread literal question/answer text into ranking... separate, larger work, not attempted here") — but, per this program's own standing lesson from Phase 4's first gap (§10 above, `a643a11b`) and Phase 3's review history, a documented residual gap is not the same as it being acceptable to leave unfixed once a cheap, correctly-scoped fix exists. Re-read FRD §27's actual input list before designing the fix ("current request, current response... logical decision path") and its own cited mechanism ("reuse `capabilityCandidateMatcher`/`capabilityRanking` machinery — `baseScore`, `recommendation.triggerFamilies`") — neither asks for free-text NLP scoring; both ask for the matcher's existing *structured* relationship data to drive ranking, which round one's fix never touched for the no-launch-context case. `ASK_OPERATION_CAPABILITY[operationId]` (`currentCapabilityId`, already computed every turn for self-exclusion) already names, in fully structured form, what the current response was about, independent of launch context. `capability.recommendation.explicitRelatedCapabilityIds` is a real, populated, registry-validated relationship table (`RELATED_CAPABILITIES` in `capabilityDefinitionFactory.ts`, ~30 capabilities with real, distinct related sets; every id is validated against the live registry at construction time — no self-reference, no unknown id, `capabilityRegistry.ts:82-92`) — the same field the matcher's own `COMPLETION_OUTPUT_RELATIONSHIP` match kind reads and the sibling "System A" `capabilityRelatedResolver.ts` also reads directly, so this is reusing real, already-load-bearing data, not inventing a new relationship table. Fixed with a new pure function, `explicitlyRelatedCapabilityIds(currentCapabilityId)`, and a stable-partition promotion (`prioritizeExplicitlyRelated`) applied inside `selectAskNextActionCapabilities` after exclusion and before the 5-item slice — related suggestions move to the front, each partition keeping its incoming `baseScore`-ranked order, so this is a local, post-fetch reordering of an already-governed/ranked list, not a change to `capabilityRecommendation.service.ts`'s shared pipeline or a new `sourceContext` kind. `buildAskNextActionsBlock`'s description now also reflects "ranked for what you just did" whenever either signal (launch-context `sourceContext` OR a non-empty `relatedCapabilityIds`) is present, not only the former. A turn whose operation has no `ASK_OPERATION_CAPABILITY` entry (`GROUNDED_GUIDANCE`) still falls back to unpromoted, property-wide ranking — a real, separate, explicitly-named residual case (no capability-of-record for that turn at all), not silently absorbed into this fix. `tsc --noEmit` clean; 9 new tests in `askNextActions.test.js` (29 total for this file, all passing), including a direct test of the reviewer's own stated verification — two different just-answered capabilities (`sell-hold-rent` vs. `ownership-costs`) promoting genuinely different top suggestions from the identical candidate list.
...
**External review round 2, 2026-09-13: 2 more findings (1 P1, 1 P2), both verified true against code before fixing, both closed.**

**(1) [P1] Proactive follow-ups lost the triggering signal's context.** Radar's own continuation card suggests `['What should I do about this?', 'How urgent is this?']` (`radarNotificationDelivery.service.ts`), but `askFollowUpContext.ts`'s top-level gate matched a message against four fixed patterns (entity/filter/pagination/specialist) and returned the message UNCHANGED, without even querying the prior execution, for anything matching none of them — both Radar suggestions matched none. Maintenance's "Now complete it" fared differently but no better: it DID match `ENTITY_CONTINUATION_PATTERN` and DID read the prior execution, but `extractSingularEntityTitle` (built for the `WORKFLOW_PROGRESS`/`GROUPED_LIST`/`TABLE` shapes a *routed* operation's own result produces) found nothing, because a proactive continuation card's own `WORKFLOW_PROGRESS` block uses generic `details` labels ("What changed", "Why it matters") a producer supplies freely — never `label: 'Task'` — so it always fell through to the unresolved fallback for every maintenance-deadline notification, silently, since the function's whole contract is "return the message unchanged when nothing resolves," not an error. Fixed with a genuinely structured mechanism rather than smarter text-matching: `CapabilityInvocationEnvelope.suppliedInput` — declared on the contract already but, confirmed by grep before touching anything, populated and read NOWHERE in the normal propose-time dispatch path (the only existing consumer, `maintenanceTaskCompleteResult`'s own `suppliedInput.taskId` lookup, was reachable only through `submitAskCapture`'s capture-edit flow calling the function directly, bypassing the registry entirely). `askFollowUpContext.ts` gained: (a) a `MONITOR_VAGUE_FOLLOWUP_PATTERN` recognizing Radar's own vague-referent chip text, gated on the prior execution actually being a proactive continuation card (`launchContextJson.surface === 'MONITOR_NOTIFICATION'`, the same marker `createAskNotificationContinuation` already stamps on every producer's card) — resolves to `suppliedInput: {radarMatchId, radarEventId}` read straight from the card's own `parametersJson`; (b) inside the existing entity-continuation branch, a fallback for the Maintenance case specifically (`complete|finish` verb + `MAINTENANCE_STATUS` operationId + monitor card) resolving to `suppliedInput: {taskId}` — read from the same `parametersJson.taskId` `maintenanceReminder.service.ts` already writes. `suppliedInput` is threaded end-to-end through `executeOperation`/`executeOperationCore`/`dispatchOperationAdapter(Result)`/`buildCapabilityInvocationEnvelope` (mirroring `continuationCursor`'s existing threading, mechanically, one more optional field on each of the same five shared inline-typed signatures) into the envelope, and the two registrations (`maintenance.complete`, `intelligence-envelope.query`) now read `envelope.suppliedInput`. `intelligenceEnvelopeQueryResult` uses a supplied `radarMatchId` to post-filter `queryIntelligenceEnvelope`'s returned items down to the ones whose `source.sourceRecordId` equals it — real and precise (`PropertyRadarMatch`'s own envelope adapter sets `sourceRecordId: row.id`, confirmed by reading `intelligenceEnvelopeQuery.service.ts` directly, i.e. this IS the match id, not an approximation), and sidesteps the separate, still-open Phase 7-scoped entityRef gap (§4.6/§31 item 2 — Radar producers never set `entityRef`) entirely, since this filter never touches `entityRef`. Falls back to the unfiltered page when the originating match's item didn't come back (aged out/deleted) rather than presenting a false "nothing found." 9 new tests in `askFollowUpContext.test.js` (19 total), 3 new source-governance tests in `askEnvelopeAndRankingBoundary.test.js`, all green; `tsc --noEmit` clean on backend.

**(2) [P2] Home Event Radar's required DomainEvent migration was never done.** FRD §29's own line is explicit: "Home Event Radar migrates its direct `Notification` write onto this same `DomainEvent` rail (Stage 2's explicit decision)" — Stage 2 itself: "Migrate its notification path to publish a `DomainEvent` at the decision/materialize boundary instead of writing `Notification` directly — bringing it onto the same standard rail as every other producer... the single highest-leverage proactive-intelligence fix identified." Confirmed still entirely undone by reading the code fresh: `homeEventRadarMatcher.service.ts` calls `radarNotificationDecisionService.evaluateMatch()` synchronously in-process, which itself called `RadarNotificationDeliveryService.materialize()` synchronously, in-process, at both its call sites — zero `DomainEvent` involvement anywhere in Radar's decision→materialize path (the *only* existing Radar↔DomainEvent wiring, `RADAR_PROPERTY_RECONCILIATION_REQUESTED`, is for a structurally different concern — re-matching a property's active events after a property-fact change — not notification materialization). Phase 5's own original work explicitly scoped this out ("this phase's Radar work is confined to the notification-delivery layer... not the event-ingestion/matching layer"), which was true of §4.5/§4.6 but left the DomainEvent-rail requirement itself unaddressed without saying so directly. Fixed as a real migration, mirroring this directory's own sibling precedent (`radarPropertyReconciliation.service.ts`) exactly: new module `radarNotificationMaterializationReconciliation.service.ts` exports `requestRadarNotificationMaterialization` (upserts a new `RADAR_NOTIFICATION_MATERIALIZE_REQUESTED` `DomainEvent`, idempotent on `decisionId`, payload bounded to `{decisionId, propertyId}` — a durable id reference, not a copy of mutable decision/match/event state, same rationale as the sibling module's own payload design) and `processRadarNotificationMaterializeEvent` (the consumer: re-hydrates the full `RadarNotificationMaterializationInput` from just `decisionId` via `propertyRadarNotificationDecision.findUnique` with its existing `propertyRadarMatch`→`radarEvent`→`sourceDefinition` relations, then calls `materialize()`). `RadarNotificationDecisionService`'s constructor DI swapped from a `deliveryService: {materialize}` dependency to `requestMaterialization: (input) => Promise<unknown>`, defaulting to the new emitter — both of `evaluateMatch`'s call sites now request materialization instead of calling it directly. New `DomainEventType` enum member added to `schema.prisma` (additive, `npx prisma generate` run in both backend and workers per this repo's established convention — **`npx prisma db push` is still the user's own manual step**); new consumer registered in `processDomainEvents.job.ts`'s per-type switch (`ProcessDomainEventsDeps.radarNotificationMaterialize`, `defaultDeps`, the switch case), mirroring `RADAR_PROPERTY_RECONCILIATION_REQUESTED`'s own registration exactly. `materialize()` itself is already idempotent (dedup via the decision's own `notificationId` / the deterministic `deduplicationKey`), so this rail's at-least-once redelivery semantics are safe without further change there. Updated the one existing test whose constructor-arg shape changed (`homeEventRadarNotificationPolicy.test.js`'s "decision persistence is idempotent..." test); added a new 6-test file for the reconciliation module (`homeEventRadarNotificationMaterializationReconciliation.test.js`) and 2 new tests in workers' `processDomainEventsJob.test.js` for the new switch case (happy path + retry/dead-letter path). `tsc --noEmit` clean on both backend and workers (workers needed its own `npm run prisma:generate`, per this repo's established separate-client-copy convention). Verified Radar's own weather acceptance harness (`homeEventRadarWeather.acceptance.test.js`) is unaffected — it's a self-contained fixture simulation calling the pure `evaluateRadarNotificationPolicy` function directly, never `RadarNotificationDecisionService`/`RadarNotificationDeliveryService`.

**Not done, stated plainly:** §4.5 (Radar runtime population/seed data) and §4.6 (the weather-domain `entityRef` gap on Radar producers, needed for Scenario 8.4) remain untouched, exactly as scoped into Phase 7 already — this round's `sourceRecordId`-based filtering closes a *different*, narrower need (scoping a follow-up to the one match that triggered it) and does not substitute for giving Radar producers a real `entityRef`. Not deployed/live-verified, same standing caveat as every phase.
...
**External review round, 2026-09-13: 3 more findings (2 P1, 1 P2), all verified true against code before fixing, all closed before Phase 6 can be considered ready for confirmation.**

**(1) [P1] Low-confidence goals created threads automatically.** FRD §21's own text: "a GOAL candidate only creates/attaches a thread when the pre-filter+extraction combination reaches its normal confidence bar for the GOAL category." Verified no such bar existed anywhere: `GoalExtractionCandidateSchema` only bounds `extractionConfidence` to `[0, 1]`, and `filterValidCandidates` (the one gate every candidate passes through before `persistCandidates`'s transaction) only checked FACT candidates — a GOAL candidate with `extractionConfidence: 0` reached `processGoalCandidate`'s real `DecisionThread` write untouched. No numeric confidence bar existed anywhere else in this codebase to reuse either — `extractionConfidence` is captured and stored for every category (`parametersJson`, audit-only) but was never gated against, even for FACT/EVENT/WARRANTY (those categories don't need one, since a low-confidence candidate still reaches a homeowner-reviewed Save card before anything is written; GOAL is exempt from that gate entirely per §21's own "reversible at zero cost" materiality carve-out, making this bar the ONLY safeguard standing between a casual mention and a real write). Fixed with a new, explicitly-documented constant, `MIN_GOAL_EXTRACTION_CONFIDENCE = 0.6` (conversationalCapture.ts) — a deliberate choice, not derived from an existing evaluated number since none exists — and `isValidGoalCandidate`, wired into `filterValidCandidates` alongside the existing FACT check, using the same `filterCandidatesPreservingWarrantyLinks` index-remap machinery so dropping a low-confidence GOAL can never silently break a later WARRANTY's `linkedEventCandidateIndex` (the same bug class Phase 3 already fixed once for exactly this shape of removal).

**(2) [P1] Saved goals weren't consumed by subsequent conversations.** Confirmed by grep: `AskSession.activeDecisionThreadId` (written once, by `processGoalCandidate`) had ZERO readers anywhere in the backend. FRD §8.5's own scenario is explicit that this should matter: "next-action scan biased toward the active thread surfaces Seller Prep/sell-hold-rent capabilities... without the homeowner needing to know either exists" — and this exact gap was already named and deferred by Phase 4's own header comment in `askNextActions.ts` ("no existing DecisionThread-aware sourceContext kind... left as explicit, undone follow-up (candidate for Phase 6)"). Fixed in `askNextActions.ts`: a new `activeSellHoldRentGoalRelatedCapabilityIds(propertyId)` calls `sellHoldRentDecisionFamilyAdapter.selectThread` (the SAME canonical, property-scoped `activeIdentityKey` lookup `createOrResumeThread` already resolves against, confirmed genuinely read-only before reusing it for a ranking signal — no risk of next-action ranking creating a thread as a side effect) and, when an active thread exists, promotes `['sell-hold-rent', 'seller-prep']` directly into `buildAskNextActionsBlock`'s existing `relatedCapabilityIds` promotion (the same mechanism the Phase 4 review-round-3 fix built, fetched concurrently via `Promise.all` alongside the existing suggestion/related-capability fetches). Deliberately does NOT read `AskSession.activeDecisionThreadId` — that field is session-scoped, and the acceptance criterion this fix serves ("resumes correctly across a new session") requires exactly the opposite: a brand-new session has no cached value yet, so relying on it would fail the one case this fix most needs to cover. **Left explicit, not silently dropped:** this closes only the next-action-ranking half of "Extraction and next-action ranking also receive no active thread state" — the extraction half (biasing what counts as a GOAL-shaped follow-up, or resolving a vague reply against the active thread without repeating the goal statement) is a materially different, larger change to the pre-filter/routing layer, not attempted here.

**(3) [P2] Goal processing failures were permanently, silently dropped.** `persistCandidates` used to mark the triggering `ASK_EXTRACTION_REQUESTED` event PROCESSED (overwriting its own payload) INSIDE its transaction, then call `processGoalCandidate` for each GOAL candidate AFTER that transaction committed — a design the code's own comment explicitly called "accepted, precedented loss." A DB error or crash during that post-transaction call had no retry path at all: the triggering event was already PROCESSED, and its payload (the message needed to re-extract) was already gone. Fixed with the SAME durable-DomainEvent pattern this exact review round's own earlier finding (Phase 5's Radar migration, `radarNotificationMaterializationReconciliation.service.ts`) already established in this codebase: a new `ASK_GOAL_CANDIDATE_ATTACH_REQUESTED` DomainEvent type (additive schema change), carrying the already-parsed candidate content (no re-extraction needed on retry) — created ATOMICALLY inside `persistCandidates`'s own transaction (`requestGoalCandidateAttachment`), committed in the same all-or-nothing unit as the FACT/EVENT/WARRANTY rows and the triggering event's own PROCESSED marking. A new `processGoalCandidateAttachEvent` (exported, self-completing — mirrors `processAskExtractionRequestedEvent`'s own shape exactly) does the actual attachment work and marks ITS OWN event PROCESSED only once thread attachment and the child `AskExecution` both fully succeed; failure leaves it PENDING/FAILED for the worker's standard backoff/dead-letter poller, never silently lost. The inline path (`claimAndProcessGoalAttachEvent`, called right after `persistCandidates`'s transaction commits) is now a pure responsiveness optimization, not the only chance a goal candidate gets — a lost claim race, a failure, or simply running past the outer ~1.5s budget all leave it durably retryable rather than dropped. New consumer registered in `processDomainEvents.job.ts`'s per-type switch (`ProcessDomainEventsDeps.goalCandidateAttach`, mirroring the Radar consumer's own registration exactly). The old batch function `processGoalCandidates` (dead code once this landed) was removed, not left orphaned. FACT/EVENT/WARRANTY rows remain independently, atomically committed by the original transaction regardless of what happens to any goal-attach event afterward — unchanged from before this fix, per the reviewer's own explicit instruction.

**Verification:** `tsc --noEmit` clean on backend and workers (workers needed its own `npm run prisma:generate` re-sync). `conversationalCapture.test.js` (62 tests, all green, including new direct tests for `isValidGoalCandidate`/`MIN_GOAL_EXTRACTION_CONFIDENCE`/`filterValidCandidates`'s GOAL-confidence drop with WARRANTY-index-remap coverage, a real `GoalCandidateAttachPayloadSchema.parse` round-trip test, and source-governance tests for the new durable-event wiring's ordering — goal-attach events created inside the transaction, before the PROCESSED marking). `askNextActions.test.js` (33 tests, all green) and `askEnvelopeAndRankingBoundary.test.js` re-run alongside, unaffected. Workers' `processDomainEventsJob.test.js` (37 tests, all green) covers the new switch case's happy path and its retry/dead-letter path. `extractionCandidateSchema.test.js`'s own pre-existing standalone TDZ fragility (same `capturePropertyFact.ts` circular-import issue this phase's original session already documented) reconfirmed via `git stash` against unmodified `main` — identical, not caused by this round's changes; the new GOAL-confidence logic is instead verified directly through `conversationalCapture.test.js`, which requires the same module successfully via the full test runner.

---

## 13. Phase 7 — Additional Capability Exposure

Only after Phases 1–2 prove stable (the request's own §36 criterion) — **softened this revision:** an earlier draft required "production use, not just tested" as this phase's gate. That specific bar is this document's own addition, not the request's: the request's §36 asks only that "the core architecture proves stable," and its own §41 states there are currently no production users at all, which a hard production-use gate sits awkwardly against. Demonstrated stability under this program's own test/eval suites (§23/§24) is the actual gate; a genuine production rollout, once users exist, only strengthens that signal rather than being required to reach it. Candidates, prioritized by the Three Jobs (Stage 1/2), not code availability: Seller Prep (needed by Phase 6, effectively co-scheduled), Home Event Radar query capability (blocked on §4.5/§4.6), Home Renovation Advisor (naming ambiguity resolved, §4.10 — unblocked), coverage/insurance, personalization-supported ranking (explicitly deferred per Stage 2 §23 — core next-actions works without it), documents, additional maintenance intelligence.

**Status (2026-09-13): Slice 1 (Seller Prep, read-only) shipped.** Chosen first per this section's own priority order ("needed by Phase 6, effectively co-scheduled") and because it's the only unblocked candidate at this pass (Home Event Radar's §4.5/§4.6 preconditions remain genuinely unmet, confirmed unchanged from the Phase 5 review round's own read).

**What shipped:** a new `SELLER_PREP_CHECKLIST` Ask operation and its owning `seller-prep` Skill, following this program's own established ~8-file registration ritual (mirrored from the `sell-hold-rent` Skill as template, verified fresh rather than assumed): `askOperationRegistry.ts` (AskOperationId union entry, routing regex + cascade check, `ASK_OPERATION_DEFINITIONS` entry, `CAPABILITY_CONTINUITY_OPERATIONS` membership), `askAudiencePolicy.ts` (`OWNER_LIFECYCLE_MODES` policy, matching `SELL_HOLD_RENT_ANALYSIS`'s own), `askOperationSemanticPackages.ts` (all 4 required maps + the optional 5th), `askTrustCertificationCorpus.ts` (a certification row + a certified direct answer), `askOrchestrator.service.ts` (the `sellerPrepChecklistResult` handler, capability-handler registration, `captureFallbackHref` case), and the new `services/skills/seller-prep/` package (manifest, evaluation fixture, index, SKILL.md). The handler reads `PropertySaleCaseService.getCase` directly -- the exact same read Phase 6's `buildSellerPrepInlineBlock` already performs for the `SELL_HOLD_RENT_GOAL_CAPTURE` addendum -- now exposed as its own, richer, category-grouped, cost-estimated, directly-askable answer, closing a real gap: `MAJOR_EVENT_ENTRY`'s own "selling" branch (owned by the pre-existing `seller-preparation` Skill, confirmed distinct from this new one before building anything) already suggests "Check sale readiness" as a follow-up, but nothing answered that message before this operation existed.

**Three real registration points found and closed, not assumed from the template alone:** the `sell-hold-rent` template's own manifest/evaluation shape covered most of the ritual, but three SEPARATE registries -- each with its own fail-fast startup validator -- required their own explicit entries, discovered by actually running the validators rather than trusting tsc (which stayed clean throughout and caught none of these):
1. `services/skills/adapters/skillAdapterRegistry.ts` -- a registry distinct from `capabilityHandlerRegistry.ts` (the one Phase 1 built and every prior phase in this program has registered against). `resolveHierarchicalSkillRouting` resolved `UNAVAILABLE`/`SKILL_OPERATION_UNAVAILABLE` until `adapter('seller-prep.checklist', 'PropertySaleCaseService', 'SELLER_PREP_CHECKLIST')` was added here too.
2. `skillEvaluationRegistry.ts`/`skillRegistry.ts`'s own registration maps (import + map entry) -- the evaluation-fixture and skill-definition registries a new Skill package must be added to, same shape as every prior Skill's own registration.
3. Three hardcoded operation/skill-count assertions in test files that this program's own established convention (Phase 6's own memory note) already anticipated bumping: `capabilityHandlerRegistry.test.js` and `askGovernance.test.js` (71→72 Ask operations), `skillCatalogMultiSurface.test.js`/`skillTaxonomyExpansion.test.js` (20→21 Skills, the latter's hardcoded taxonomy map also needed the new skill's operation list added).

**Routing collision checked and resolved deliberately, not accidentally:** `MAJOR_EVENT_ENTRY`'s existing `majorEventPattern` already matches broad "help/prepare/plan/checklist ... selling my home" phrasing. The new `sellerPrepChecklistPattern` is deliberately narrower (targets "seller prep" / "sale readiness" / "am I ready to sell" / "check ... readiness" phrasing specifically) and is checked earlier in the routing cascade, so a real, data-backed checklist answer wins for the phrasing that most directly asks for it, while the broader generic entry point is left completely untouched for vaguer phrasing -- verified with dedicated routing tests for both directions (a Seller-Prep-specific message still resolving to `SELLER_PREP_CHECKLIST`, a generic life-event message still resolving to `MAJOR_EVENT_ENTRY`), not just the new operation's own positive cases.

**Verification:** `tsc --noEmit` clean throughout. Full targeted sweep, not just the new test file: `intelligenceRegistries`, `capabilityHandlerRegistry`, `confirmCapabilityHandlerRegistry` (unaffected -- the new operation is read-only, not confirmation-gated), `skillEvaluationRegistry`, `askAudienceContext`, `askGovernance`, `skillTaxonomyExpansion`, `skillCatalogMultiSurface`, the new `sellerPrepChecklist.test.js`, and `askEnvelopeAndRankingBoundary` -- 85/85 green. The pre-existing `askRoutingCalibration`/`askTrustArchitecture` cluster (this program's own long-standing, documented calibration-drift/count-scaling gap, unrelated to any specific phase) reconfirmed via `git stash` against the pre-Phase-7 state: the exact same fixture mismatch and the exact same constant 4-operation certification gap reproduce at both operation counts (67-of-71 before, 68-of-72 after) -- genuinely pre-existing, not introduced here.

**Deliberately scoped out of this slice, not silently dropped:** `PropertySaleCaseService.setItemDecision` (the real WAIVE/PURSUE/REOPEN/UNPURSUE write path) is a genuine, separate, confirmation-gated follow-up capability -- this slice is read-only by design, matching this program's own "smallest coherent architectural capability" principle rather than shipping the full read+write surface at once. Not deployed/browser-verified, same standing caveat as every phase. The remaining Phase 7 candidates (Home Renovation Advisor -- now unblocked per §4.10 -- coverage/insurance, documents, additional maintenance intelligence, and Home Event Radar once §4.5/§4.6 are resolved) remain unstarted.

**Status (2026-09-13, same session): Slice 2 (Seller Prep, write path) shipped -- the write path named above as Slice 1's own explicit follow-up.** New `SELLER_PREP_ITEM_DECISION` operation exposes `PropertySaleCaseService.setItemDecision` (WAIVE/PURSUE/REOPEN/UNPURSUE on a `SaleReadinessItem`) as a confirmation-gated Ask command, added to the SAME `seller-prep` Skill (version bumped `1.0.0`→`1.1.0`, evaluation package's `skillVersion` bumped to match) rather than a new skill -- both operations are the same checklist's read and write halves.

**Built by mirroring `INSPECTION_FINDING_UPDATE`'s own propose/confirm shape exactly**, the closest existing template for "resolve an exact target + action from free text, propose a confirmation card, apply the write only on confirm": `exactEntityMatch` (already generic over any `{id, title}` row -- reused verbatim, no changes needed) resolves the target `SaleReadinessItem` from free text or `launchContext.entityId`; a new `sellerPrepItemAction` parser resolves WAIVE/PURSUE/REOPEN/UNPURSUE, with REOPEN/UNPURSUE deliberately checked before WAIVE/PURSUE so "undo the waive" (which contains the literal word "waive") isn't misclassified. Registered through every layer Slice 1's own session already mapped out -- `askOperationRegistry.ts` (union, routing pattern, `ASK_OPERATION_DEFINITIONS`), `askDomainCommandRegistry.ts` (the confirmation-required-command registry Slice 1 didn't need), `askAudiencePolicy.ts`, `askOperationSemanticPackages.ts` (5 maps), `askTrustCertificationCorpus.ts`, `services/skills/adapters/skillAdapterRegistry.ts`, the skill manifest/evaluation package, and both `askOrchestrator.service.ts` capability-handler registries (propose-time `registerCapabilityHandler` and confirm-time `registerConfirmCapabilityHandler` -- the second dispatch surface Slice 1's read-only operation never touched).

**One real registry rule found and fixed, not assumed:** `validateSkillDefinitions()` rejects any skill with a `MUTATION_PREPARATION` adapter or a `WRITE`/`EXTERNAL_TRANSMISSION` risk effect at `autonomyLevel` below 2 (confirmed by reading `skillRegistry.ts`'s own validator, then cross-checked against the `maintenance`/`incident-claim` skills' own `autonomyLevel: 2`) -- caught by actually running `skillCatalogMultiSurface.test.js` after adding the write operation to the manifest, not assumed safe from the read-only slice's own `autonomyLevel: 1`. Fixed by bumping the skill's `autonomyLevel` to 2 and adding `WRITE` to `riskPolicy.effects`.

**Routing collision checked deliberately again, same discipline as Slice 1:** `sellerPrepItemDecisionPattern` requires an explicit decision verb (waive/pursue/reopen/unpursue) bidirectionally near "seller prep"/"sale readiness"/"checklist" + "item" phrasing, and is checked BEFORE `sellerPrepChecklistPattern` in the cascade -- "waive the seller prep item for the roof" contains the literal phrase "seller prep" and would otherwise match the bare checklist-read pattern first. Verified both a decision-bearing message and a bare "check my sale readiness" message resolve to the correct, different operations.

**A safety property genuinely different from the read slice, verified by reading `PropertySaleCaseService.setItemDecision`'s own implementation before relying on it:** the write is fully idempotent and unconditional (re-applying the same action is a safe no-op re-write; it does not throw on an already-matching status) -- unlike `INSPECTION_FINDING_UPDATE`'s own confirm handler, `confirmSellerPrepItemDecision` therefore has no `alreadyApplied` staleness-check bypass, since skipping the `contextVersion` check here would have no idempotent-no-op safety net protecting a stale confirm from silently overwriting a decision (and its stored reason) made by someone else in the meantime -- the staleness check is unconditional, always enforced.

**Verification:** `tsc --noEmit` clean throughout. Full targeted sweep -- `intelligenceRegistries`, `capabilityHandlerRegistry` (73 ops now), `confirmCapabilityHandlerRegistry` (29 confirmation-required commands now -- the first count change to this registry since Phase 3's warranty writer), `skillEvaluationRegistry`, `askAudienceContext`, `askGovernance`, `skillTaxonomyExpansion`, `skillCatalogMultiSurface`, the extended `sellerPrepChecklist.test.js` (18 tests total, 10 new), `askEnvelopeAndRankingBoundary` -- 95/95 green. The pre-existing `askRoutingCalibration`/`askTrustArchitecture` cluster reconfirmed once more at the new operation count (69-of-73 certified, same constant 4-operation gap; the same `ask-routing-calibration-v2-016-competitor` fixture mismatch reproduces byte-for-byte) -- unrelated, not introduced here.

**Deliberately left open, not silently dropped:** `reason` (the durable `waivedReason`) is only captured from an explicit "because X"/"since X" trailing clause in the homeowner's own message -- no dedicated capture/edit affordance for it, matching this slice's own proportionate scope. Not deployed/browser-verified. Phase 7 now has two committed, shipped slices for Seller Prep (read + write); the next candidate is not yet chosen.

**Status (2026-09-13, same session): a third Phase 7 candidate, Home Renovation Advisor, was researched and rejected before writing any code.** Following this section's own priority order (after Seller Prep and the still-blocked Home Event Radar), Home Renovation Advisor was next. A dedicated research pass (reading `homeRenovationAdvisor.service.ts`, `engine/evaluationEngine.service.ts`, its routes, and the `HomeRenovationAdvisorSession` Prisma model in full) found it genuinely unfit for exposure right now, not merely inconvenient: both `createSession` and `evaluateSession` set an HTTP `Deprecation: true` header with a `Link: rel="successor-version"` pointing at `/api/properties/:propertyId/renovation-cases`, return `advisoryOnly: true`, and call `assertProjectComplianceApplicable(...)`, which can throw a 409 depending on governed compliance-context state -- i.e. the module can be actively blocked at runtime in favor of the newer `RenovationCase` system, not just soft-deprecated. Worse for this program's own "ship the read slice first" pattern: there is no non-persisted read path at all -- the only route to the pure evaluation engine (`runEvaluation`) is `evaluateSession`, which unconditionally persists a full output-tree transaction, so even a first read-only slice would have to be a WRITE. Presented to the user as an explicit choice (skip it / build against the successor `RenovationCase` system instead / build on the deprecated module anyway); the user chose to skip it. §4.10's "naming ambiguity resolved -- unblocked" note above is now superseded by this finding: the operation was never blocked by naming, but by being actively steered away from at the HTTP layer.

**Status (2026-09-13, same session): Slice 3 (Coverage Comparison status, read-only) shipped, added to the existing `coverage` Skill.** Following the priority order past the now-rejected Home Renovation Advisor, the next candidate was coverage/insurance. Research confirmed the existing `COVERAGE_GAPS` operation only covers per-inventory-item warranty/insurance gap review (`coverage.review` adapter, `getCoverageReviewItems`) and identified two genuinely distinct, unexposed candidates: Coverage Comparison (per-policy current-vs-alternative comparison, `coverageComparison.service.ts`) and Insurance Handoff readiness (a separate human-quote-request feature, MFA-gated writes). Presented to the user as an explicit choice; the user chose Coverage Comparison status, read-only, extending the existing `coverage` Skill rather than creating a new one -- the closest match to this section's own "coverage/insurance" candidate and its established read-first pattern.

**What shipped:** a new `COVERAGE_COMPARISON_STATUS` operation reading `getOrCreateCoverageComparison` (`coverageComparison.service.ts`) -- the exact same call `getCoverageComparisonWorkspace`'s own existing GET route already makes, confirmed by reading the controller before relying on it. Registered through the same ritual as Seller Prep's read slice: `askOperationRegistry.ts` (union, routing pattern, `ASK_OPERATION_DEFINITIONS`, `CAPABILITY_CONTINUITY_OPERATIONS`), `askAudiencePolicy.ts`, `askOperationSemanticPackages.ts` (5 maps), `askTrustCertificationCorpus.ts`, `services/skills/adapters/skillAdapterRegistry.ts` (the third registry), `askOrchestrator.service.ts` (handler, capability-handler registration, `captureFallbackHref` case), and the existing `coverage` Skill's manifest/evaluation package extended (version `1.0.0`→`1.1.0`) rather than a new package created, mirroring Seller Prep's write slice's own precedent for adding a second operation to an existing Skill.

**A genuine access-control gap found and worked around, not assumed away:** unlike `PropertySaleCaseService.getCase`'s own `requireAccess`, which calls the same `resolvePropertyAccess` Ask's `ensurePropertyAccess` already uses (confirmed by reading both, which is why Seller Prep's handler could skip a redundant call), `coverageComparison.service.ts`'s own `authorizeProperty` is a raw `homeownerProfile.userId === userId` check with no household-role gradient at all. Calling `getOrCreateCoverageComparison` directly (mirroring Seller Prep's no-redundant-check pattern blindly) would have thrown a raw 404 for any legitimate non-owner household VIEWER, contradicting this operation's own declared `VIEWER` role floor. Fixed by calling `ensurePropertyAccess` first (same as `coverageResult`/`COVERAGE_GAPS` in the same file, whose own underlying read has no auth check of its own either), then catching `authorizeProperty`'s narrower `PROPERTY_NOT_FOUND` `APIError` and mapping it to a disclosed `BLOCKED` result (`COVERAGE_COMPARISON_OWNER_ONLY`) rather than a raw 404 -- a real, disclosed limitation of the underlying service, not silently absorbed or silently over-permissioned.

**Routing collision checked and resolved deliberately:** `coveragePattern`'s own bare "insurance coverage" phrasing is a literal substring of several natural comparison phrasings (e.g. "compare my insurance coverage"). The new `coverageComparisonPattern` requires a compare/switch/shop/equivalent verb and is checked earlier in the cascade, so verb-bearing phrasing resolves to the more specific comparison operation while bare gap phrasing still falls through untouched -- verified with dedicated bidirectional tests. A second, unplanned collision was caught the same way against `quoteComparisonReviewPattern` (checked earlier in the cascade, contractor quotes/bids/proposals/estimates): an early draft positive example containing the word "quote" ("Compare my insurance policy to this new quote") was swallowed by that pre-existing pattern before reaching the new one at all -- caught by tracing the full cascade order rather than assuming the new check would win, and fixed by rewording the example, not the router.

**Verification:** `tsc --noEmit` clean throughout. Full targeted sweep -- `intelligenceRegistries`, `capabilityHandlerRegistry` (74 ops now), `confirmCapabilityHandlerRegistry` (unaffected -- read-only, no domain command), `skillEvaluationRegistry`, `askAudienceContext`, `askGovernance`, `skillTaxonomyExpansion`, `skillCatalogMultiSurface` (unaffected -- a 2nd operation on an existing Skill, not a new Skill), the new `coverageComparisonStatus.test.js` (12 tests), `askEnvelopeAndRankingBoundary` -- all green. A full `tests/ask/*.test.js` sweep (run once, not this program's routine practice -- see the "prioritize functionality over test runs" convention -- but warranted here to catch anything the targeted list missed) surfaced one real gap this slice's own targeted list hadn't caught: `skillAdapterRegistry.test.js`'s own hardcoded `SKILL_ADAPTER_DEFINITIONS` count was already stale at 64-vs-62 BEFORE this slice (confirmed via `git stash`, a 2-adapter drift left over from an earlier phase never closing this specific count assertion), then 65-vs-62 after this slice's own addition; fixed to the correct 65 in the same edit, closing both the pre-existing drift and this slice's own increment. The sweep's remaining 7 failures (3× `askLaunchContextCapability.test.js` `ASK_CAPABILITY_UNIQUE_OPERATION` export gap, 2× `extractionCandidateSchema`/`extractionContract.test.js` crash on an unrelated `capturePropertyFact.ts` TDZ bug, 1× `skillDecisionFinancialRegistration.test.js` HVAC_SPECIALIST_ENGAGE gap, plus the pre-existing `askRoutingCalibration`/`askTrustArchitecture` 4-test cluster) were each individually reconfirmed via `git stash` to fail identically with this slice's changes stashed out -- genuinely pre-existing, not introduced here, and out of this slice's own scope to fix.

**Deliberately scoped out of this slice, not silently dropped:** `addCoverageComparisonOption` (attach a quote document or another verified policy term) and `recordCoverageDecision` (KEEP/CHANGE/SHOP/DEFER/PROFESSIONAL_REVIEW) are both document-dependent, multi-step writes -- a genuinely separate, larger follow-up capability, not attempted here. `getOrCreateCoverageComparison` itself lazily creates a `DRAFT` `CoverageComparison` + `CURRENT_POLICY` option row on first call when a verified baseline policy exists; this is treated as part of the read (matching the existing GET route's own behavior, verified by reading the controller, not assumed), not as this slice's own write. Not deployed/browser-verified. Phase 7 now has three shipped slices (Seller Prep read + write, Coverage Comparison status); the next candidate is not yet chosen.

**Status (2026-09-13, same session): Slices 4 and 5 shipped in parallel -- Documents (`DOCUMENT_LOOKUP`, a new `documents` Skill) and additional maintenance intelligence (`MAINTENANCE_FORECAST`, added to the existing `maintenance` Skill).** The user asked for both remaining named Phase 7 candidates at once ("documents, additional maintenance intelligence"). Two parallel research forks investigated each before any code was written.

**Documents:** confirmed `DOCUMENT_PROMOTION_REVIEW`/`DOCUMENT_PROMOTION_CONFIRM` only ever read `pendingDocumentPromotionCandidates` (a queue of pending extraction candidates), never `prisma.document` directly -- the document vault itself was a genuine, unexposed gap. `DOCUMENT_LOOKUP` reads `prisma.document.findMany({ propertyId, deletedAt: null })` directly (the same query the existing `GET /api/documents` route already runs, confirmed by reading the route first), grouped by `DocumentType` with verification status. New `documents` Skill (not folded into `document-promotion`, a genuinely different capability: the vault vs. a review queue). Access control matches the shared `resolvePropertyAccess`/`ensurePropertyAccess` pattern with no gap this time (confirmed by reading the route, unlike Coverage Comparison's own narrower check).

**Additional maintenance intelligence:** the research fork explicitly ruled out duplicating `HOME_ACTIONS` (the FRD's own single governed action/intelligence surface, "no competing action source") and instead found `maintenancePrediction.service.ts` -- a fully standalone, previously-unconnected rule-based forecast engine (`MaintenancePrediction` rows, HVAC/ROOF/WATER_HEATER interval rules against verified inventory items) with zero references anywhere in `homeActions.service.ts` or the Personalization pipeline. `MAINTENANCE_FORECAST` reads `listForecast`, and -- since it never lazily generates on its own, unlike Coverage Comparison's `getOrCreateCoverageComparison` -- materializes once via `generateForecast` (confirmed cheap, deterministic, local-only, dedup'd via an in-flight map) when the forecast has never been run, then re-lists, rather than answering a stale empty state. Added to the existing `maintenance` Skill (version `1.0.0`→`1.1.0`) alongside `MAINTENANCE_STATUS`/`HOME_DEADLINE_MONITOR`, not a new Skill -- flagged to the user first that the underlying frontend component (`MaintenanceForecast.tsx`) has zero importers anywhere in the app (built but never mounted), so this operation's own href falls back to the generic `/maintenance` page rather than a forecast-specific one that doesn't exist.

**A second access-control gap found by direct comparison, not assumed:** `maintenancePrediction.service.ts`'s own `listForecast`/`generateForecast` have no internal auth check at all (same pattern as `getCoverageReviewItems`/`COVERAGE_GAPS`), so `maintenanceForecastResult` calls `ensurePropertyAccess` itself, matching that established precedent rather than Seller Prep's no-redundant-check one.

**Three real hardcoded-count/version gaps found by running the fail-fast validators and full test sweep, not assumed:** `skillPlatformFoundation.test.js`, `skillExecutionBinding.test.js`, and `skillLineageRegistry.test.js` each hardcode the `maintenance` Skill's exact operation list and/or version string (`1.0.0`) independently of `skillCatalogMultiSurface`/`skillTaxonomyExpansion`'s own counts -- none of these were touched by Coverage Comparison's own slice (which only added an operation to `coverage`, not `maintenance`) and so went unexercised until this slice actually expanded `maintenance` for the first time this program. All three fixed (operation list gains `MAINTENANCE_FORECAST`; version bumped `1.0.0`→`1.1.0` including the `retiredMaintenance()` lineage-test fixture's own `supersededByVersion` default). `maintenance` was also deliberately kept OUT of `skillTaxonomyExpansion.test.js`'s `EXPANDED_SKILLS` list despite gaining an operation: it has a legitimate, pre-existing direct import (`MAINTENANCE_TASK_CONTEXT_PROVIDER`/`SEASONAL_CHECKLIST_CONTEXT_PROVIDER` from `../skills/maintenance/skill.manifest`, predating this phase) that test's own "no per-Skill router coupling" check would otherwise -- correctly -- flag.

**Two real, self-caught semantic-routing regressions, found only by running the full `tests/ask/*.test.js` sweep (not the targeted list) and fixed before shipping:** `DOCUMENT_LOOKUP`'s own semantic package initially caused two existing, previously-stable golden queries to tie with it at HIGH confidence band and fall into ambiguous `CLARIFICATION`/`GROUNDED_GUIDANCE` instead of their correct, previously-confident answers -- `askLanguageCompatibility.test.js`'s TA6 fixture ("What information does this house still need?" → should stay `PROPERTY_SUMMARY`) and `askEnvelopeAndRankingBoundary.test.js`'s own fixture ("What derived intelligence does this property have?" → should stay `INTELLIGENCE_ENVELOPE_QUERY`). Root-caused by reading `askSemanticRouter.ts`/`askSemanticEmbedding.ts` directly rather than guessing: (1) the job/intent text "look up documents **on file** for this home" put "file" into the shared, very broad `record` synonym concept group (`record`, `recorded`, `profile`, `documentation`, `dossier`, `file`, `details`, `information`) that `PROPERTY_SUMMARY`'s own vocabulary already saturates, and (2) a positive example ending "...for this property?" structurally mirrored the failing query's own trailing shape closely enough to score high on trigram/character-gram similarity regardless of topic. Fixed by removing "on file"/"for this home"/"for this property" phrasing from `DOCUMENT_LOOKUP`'s job, intent-driving text, and positive examples (replacing generic templates with concrete document-type nouns -- invoices, permits, contracts), plus adding both exact failing queries as hard-negative examples. Also closed, in the same pass: a corpus-integrity failure from copying an existing `DOCUMENT_PROMOTION_REVIEW` certification message verbatim into `DOCUMENT_LOOKUP`'s own hard negatives (paraphrased instead, since "no complete normalized sentence is indexed as a positive or negative example twice" is enforced). Each fix was verified with a direct routing-candidate dump (`resolveAskRoutingCascade`/`retrieveAskOperationCandidates`) before and after, not just re-running the test blind.

**Verification:** `tsc --noEmit` clean throughout, after every edit. Full targeted sweep -- `capabilityHandlerRegistry` (76 ops now), `confirmCapabilityHandlerRegistry` (unaffected -- both read-only), `skillAdapterRegistry` (67 adapters), `skillCatalogMultiSurface`/`skillTaxonomyExpansion` (22 Skills now, `documents` added), `skillEvaluationRegistry`, `skillExecutionBinding`, `skillLineageRegistry`, `skillPlatformFoundation`, the new `documentLookup.test.js` and `maintenanceForecast.test.js`, `askLanguageCompatibility`, `askEnvelopeAndRankingBoundary` -- all green. Multiple full `tests/ask/*.test.js` sweeps (not routine practice, but this slice's own two self-introduced regressions above justified the extra passes) converged on exactly the same pre-existing 10-failure cluster this program has now reconfirmed via `git stash` at every operation count this session (`askLaunchContextCapability.test.js`'s `ASK_CAPABILITY_UNIQUE_OPERATION` export gap, the `extractionCandidateSchema`/`extractionContract.test.js` `capturePropertyFact.ts` TDZ crash, `skillDecisionFinancialRegistration.test.js`'s `HVAC_SPECIALIST_ENGAGE` gap, and the constant-4-operation `askRoutingCalibration`/`askTrustArchitecture` certification cluster) -- genuinely pre-existing, not introduced here.

**Deliberately left open, not silently dropped:** `updateForecastStatus` (mark a prediction `COMPLETED`/`DISMISSED`) is a real write follow-up, not attempted here -- this slice is read-only, matching the program's established pattern. `MaintenanceForecast.tsx`'s orphaned-frontend status (flagged to the user, not silently worked around) means this operation's "open it" action lands on the generic maintenance page today, not a forecast-specific view, until product decides whether to mount that component or build a new one. Not deployed/browser-verified. Phase 7 now has five shipped slices (Seller Prep read + write, Coverage Comparison status, Documents, Maintenance Forecast); Home Event Radar's own §4.6 code blocker is closed later this session (see below) -- only personalization-supported ranking (explicitly deferred) and §4.5's environmental live-data precondition remain.

**Status (2026-09-13, same session): §4.6 (the real code gap blocking Home Event Radar) fixed; §4.5 (seed data) remains a genuine environmental limitation, unchanged.** The user asked to "look into unblocking Radar." A dedicated research pass -- which, disclosed plainly, exceeded its own read-only research scope and wrote a working implementation directly rather than only a plan (logged as product feedback; every one of its factual claims and the resulting code were independently re-verified against the actual source before being kept, not trusted on the fork's own say-so) -- confirmed §4.6 was still real: `matchesQuery` (`intelligenceEnvelopeQuery.service.ts:572-583`) ANDs `domains` and `entityRefs` independently, and `matchesEntityScope` (`:557-570`) returns `false` immediately whenever `item.subject.entityRef` is absent -- verified by reading both directly. `PropertyRadarMatch`/`PropertyRadarCompoundInsight`'s own envelope adapters never set `entityRef` before this fix (confirmed by reading `propertyRadarEnvelopeAdapter.ts` as it stood), so no Radar-sourced item could ever satisfy a component-scoped query (FRD §31 Scenario 8.4, "is my roof at risk because of the storms?") regardless of domain correctness.

**Fixed by deriving a real `entityRef` from data the system already computes and persists, not by guessing one:** `PropertyRadarMatch.matchedSystemsJson` already stores a `{systems: [{type, relevance}]}` array (`radarImpactRules.ts:135,143,943` -- confirmed present on every row) using a fixed vocabulary (`roof`, `plumbing`, `water_heater`, `hvac`, `drainage`, `sump_pump`, `foundation`, `electrical`, `insurance` -- confirmed exhaustive by reading every `systems.push`/`systems:` site in the file). Only `roof`/`foundation`/`drainage` map onto this program's `PropertyComponentKind` taxonomy (`ROOF`/`FOUNDATION`/`SITE`); the appliance/system-level types (`hvac`, `plumbing`, `electrical`, `water_heater`, `sump_pump`, `insurance`) have no component equivalent and are deliberately left without an `entityRef` rather than guessing one. `PropertyRadarCompoundInsight.ruleCode` is directly attributable for exactly 3 of its 5 registered rule codes (`SEVERE_WEATHER_OPEN_ROOF_ISSUE`→ROOF, `HEAVY_RAIN_UNRESOLVED_GUTTER_DRAINAGE`→SITE, `HEAVY_RAIN_OUTAGE_SUMP_BACKUP`→FOUNDATION -- confirmed against `radarCompoundRules.ts`'s own `RULE_CODES` list and every `ruleCode:` site, all 5 accounted for); the remaining 2 (`SMOKE_HVAC_FILTER`, `FREEZE_OUTAGE_ELECTRIC_HEAT`) are HVAC-only and stay unset. New `radarMatchEntityRef`/`radarCompoundEntityRef` functions in `intelligenceEnvelopeQuery.service.ts` compute these at read time (highest-relevance mappable system wins for matches); both `EnvelopeAdapter` row types gained an optional `entityRef` field threaded straight into `buildMappedItem`'s existing, already-general `entityRef` parameter (`adapterSupport.ts:109,127` -- a mechanism other producers, e.g. inventory items, already used; no new plumbing needed there). `askEnvelopeQueryScope.ts`'s own domain-list also actually implements the per-component-allowlist decision this document's own Phase 0 revision-note-3 had already recorded (line 8) but the code itself never carried out until now (`ROOF`/`FOUNDATION`/`EXTERIOR`/`SITE` widen to `['ASSET_LIFECYCLE', 'WEATHER']`; `INTERIOR` stays `['ASSET_LIFECYCLE']` only, since it has no plausible WEATHER-domain rule).

**§4.5 reconfirmed unchanged, checked directly rather than assumed stale:** `grep -c` against `prisma/seed.ts` for `PropertyRadarMatch`/`RadarEvent`/`PropertyRadarCompoundInsight` still returns 0; Docker isn't running in this environment (`docker ps` fails to reach the daemon) and the configured Postgres port doesn't respond (`pg_isready` on `127.0.0.1:5433`). This is a live-testing/seed-data gap only -- per this document's own line 396, it "only gates Phase 7's live demonstration, not any Phase 0-6 code path" -- and remains exactly that: an environmental precondition for a live demo, not a code defect, unchanged from Phase 0's own finding, not something resolvable from this environment.

**No new Ask operation needed -- this was a bug fix to an already-shipped one, not a new Phase 7 slice:** `INTELLIGENCE_ENVELOPE_QUERY` (Phase 9A/9B) already exists and already reads `PropertyRadarMatch`/`PropertyRadarCompoundInsight` as registered producers; the gap was purely that Radar's own rows could never pass the entity-scope filter. With this fix, a component-scoped query like "is my roof at risk?" now correctly surfaces Radar-sourced WEATHER items once real data exists to query (§4.5), with zero new registration-ritual touch points (no new `AskOperationId`, no new Skill, no new adapter registration) -- so "Home Event Radar query capability" as named in this section's own candidate list is now code-complete, pending only the same live-verification precondition every phase in this program already carries.

**Verification:** `tsc --noEmit` clean. Every factual claim above re-derived independently from the actual source (`matchesQuery`/`matchesEntityScope`, `radarImpactRules.ts`'s full `systems` vocabulary, `radarCompoundRules.ts`'s full 5-rule-code list, `adapterSupport.ts`'s existing `entityRef` plumbing) rather than trusted from the fork's own report. Targeted tests: `tests/unit/intelligenceEnvelopeQuery.test.js` (22 tests, including 5 new ones covering `radarMatchEntityRef`/`radarCompoundEntityRef` derivation, adapter forwarding, and end-to-end query matching), `tests/ask/askEnvelopeAndRankingBoundary.test.js` (component-scope routing including the new WEATHER-domain widening for ROOF/FOUNDATION/EXTERIOR/SITE vs. INTERIOR) -- all green. Broader Radar-domain regression check (`homeEventRadarQuery`, `homeEventRadarImpactRules`, `homeEventRadarCompoundInsight`, `homeEventRadarCompoundRules` -- 42 tests) confirms nothing in the matching/rules layer this fix reads from was disturbed. A full `tests/ask/*.test.js` sweep was attempted for extra confidence but was killed by the local environment running low on memory mid-run, before completing; not repeated, given the change's narrow, well-understood blast radius (no operation registry, Skill, or adapter-count changes) and the 64 directly-relevant tests already green -- not deployed/browser-verified, same standing caveat as every phase.

**Deliberately left open, not silently dropped:** §4.5's own seed-data/live-DB precondition remains exactly as open as Phase 0 left it -- establishing it (real ingestion pipeline or a purpose-built fixture script) is still a prerequisite for actually demonstrating Scenario 8.4 end-to-end, not something this pass could resolve from an environment with no reachable database. `matchedSystemsJson`'s 6 unmapped system types and `ruleCode`'s 2 unmapped HVAC-only rules remain without an `entityRef` by design, not oversight -- guessing a `PropertyComponentKind` for an appliance-level system would misrepresent what Radar's own data actually knows.
