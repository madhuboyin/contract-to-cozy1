...
**Financing writer — end-to-end integrity contract (specified this revision, not left as a one-line mention):**
- **Model, not the generic path.** `capturePropertyFact`'s existing pattern (`capturePropertyFact.ts:283-311`) is the template to follow, not a function to extend: inside one `prisma.$transaction`, it (1) writes the canonical value, (2) supersedes the prior active `PropertyFactEvidence` row for that `factKey` (`updateMany({ supersededAt: null }, { supersededAt: observedAt })`), (3) creates the new evidence row, (4) emits `PropertyChange` — all atomically. The new writer (call it `capturePropertyFinancingFact`) follows the same four steps, substituting an upsert of `PropertyFinancingProfile.interestRateBps` (by `propertyId`, `schema.prisma:19375-19399`) for step 1. `capturePropertyFact`'s own `!definition.writable` gate (`:270-271`) stays exactly as-is for its existing generic-UI caller; this is a new, separate function for the conversational-capture caller specifically, not a bypass flag threaded through the old one.
- **Replay/idempotency ordering, not just "atomic."** The evidence-create must gate the profile upsert, never the reverse: attempt `tx.propertyFactEvidence.create(...)` keyed on the new `captureExecutionId` + its `@@unique([propertyId, factKey, captureExecutionId])` constraint (Phase 2's addition) *before* touching `PropertyFinancingProfile`. A `P2002` on that create means this exact execution already committed — return the existing row's result and skip the upsert entirely (mirroring the established `PropertyFactEvidence` idempotent-create pattern used elsewhere in this program). Only a first-time create proceeds to the upsert, inside the same transaction. This ordering is what stops a retried confirmation from reapplying a stale rate over a newer one written by a *different*, later execution in between — the same failure class Stage 2's second-round correction already named generically ("a stale replay must not resurrect a value a later, unrelated correction already superseded"), applied here to a mutable 1:1 model instead of an append-only evidence log.
