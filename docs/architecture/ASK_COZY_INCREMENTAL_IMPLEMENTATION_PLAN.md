...
**Not yet built:** the actual correction/EDIT path (both new commands declare `correctionModes: ['EDIT']` as stated intent, but nothing dispatches to it yet — §4.1's supersession-chain design remains unimplemented for these two operations); the bidirectional `linkedExecutionId` pairing and `ASK_CAPTURE_LINK_RECONCILE` consumer for the event+warranty case (the schema fields exist, no writer or worker consumer uses them); claim-token re-verification for the not-yet-existing extraction job; `GroundedAskProposal` kind mapping.
