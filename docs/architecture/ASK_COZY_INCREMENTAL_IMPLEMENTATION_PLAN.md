...
**Not yet built:** the bidirectional `linkedExecutionId` pairing and `ASK_CAPTURE_LINK_RECONCILE` consumer for the event+warranty case (the schema fields exist, no writer or worker consumer uses them); claim-token re-verification for the not-yet-existing extraction job; `GroundedAskProposal` kind mapping.
