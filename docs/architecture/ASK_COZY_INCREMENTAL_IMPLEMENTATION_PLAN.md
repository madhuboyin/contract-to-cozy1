...
**Status (2026-09-13): shipped, code-review-clean, and now complete against this section's own Work list**, behind `askConversationalCaptureEnabled` (default OFF) — the last two acknowledged gaps (warranty capture writer + paired-confirmation scenario, then edit-before-confirm) both closed this same day, `e331083a` and `2e59e5a5`. "Complete against the Work list" is still not the same claim as "deployed and browser-verified" (it is not — see below). Ten commits, each independently tsc-clean and targeted-test-verified:
