...
**Status:** Complete — all 8 acceptance scenarios (F01–F08) evaluated. **F02's gap has since been fixed** (see the update note at the end of the F02 section), **though its own scored verdict was revised 2026-09-17 after external review** — the fix is real and closes the FAIL, but doesn't meet the same "revision shown alongside the original" bar the FRD's HVAC comparator (D02) sets.
**Verdict: 5 full passes, 3 partial passes, 0 fails.** Originally scored 5 full/2 partial/1 fail, then briefly 6 full/2 partial/0 fail after the F02 fix — external review found the fix itself is real (a genuine, isolated, non-persisting scenario mechanism now exists where none did) but doesn't fully match "recalculated revision; original retained" as HVAC's `HVAC_DECISION_SCENARIO` (Phase 7's D02) implements it: HVAC renders the new scenario and the existing recommendation *together, side by side, in one response*; the refinance fix returns the scenario alone, with the original recoverable only by asking a different question. F02 is corrected to PARTIAL to reflect that gap; see the F02 section.
