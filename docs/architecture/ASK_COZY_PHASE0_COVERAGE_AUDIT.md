...
| Field | Status |
| --- | --- |
| Audit coverage | Complete — all 77 registered operations classified (Stage 1) and traced (Stage 2), 0 `PENDING` |
| Static (code-read) acceptance | N/A — this is a classification/coverage audit, not a per-scenario acceptance verification; see the per-track acceptance documents for PASS/PARTIAL/FAIL scoring |
| Runtime/browser acceptance | Not evaluated this session — no live database or browser exercised |
| Phase 0 exit criterion | **Met** — see §7's own self-check table |
...
`BOUNDARY_RESPONSE` (safetyClass ends `_BOUNDARY`) → `CONFIRMED_MUTATION` (a domain-command entry exists) → `DIRECT_MUTATION` (listed in `ASK_DIRECT_MUTATION_OPERATION_IDS`, a recorded IW-CONF-001 exception; added 2026-09-22 for `HOME_EVENT_RADAR_STATE`) → `INTERNAL_CAPTURE` (not message-routable) → `NAVIGATION_HANDOFF` (`CAPABILITY_DISCOVERY` family) → `CONVERSATIONAL_CONTINUATION` (`GENERAL_HOME_GUIDANCE` family) → explicit per-operation overrides (below) → `WORKFLOW_CONTINUATION` (bound to a `decision-platform.*` DecisionThread, or `WORKFLOW_GUIDANCE` family) → `PROACTIVE_INSIGHT` (Track 6's own attention/forecast surfaces) → `READ_RESULT` (default).
...
- **Reconciliation is opt-in per handler, not automatic — and mostly unused.** `ConfirmCapabilityResult.refreshedExecutions` is the only reconciliation mechanism (passed through as `childExecutions`). Grepping every call site that actually populates it found exactly one consumer as of this original pass: `refreshMaintenanceSourceExecution`, used only by `confirmMaintenanceTaskComplete`/`confirmMaintenanceTaskUpdate` (the Completed-reference track). **None of Records and capture's 5 confirmation-gated operations populate it.** Until each track's Stage 2 pass proves otherwise, assume XREC-001 is unmet everywhere outside maintenance. **(Update 2026-09-17: the later Buyer B04 remediation added a 2nd consumer, `confirmBuyerTaskUpdate`, via the same mechanism generalized as `refreshAskSourceExecution` — see §4.15's own update note below for the current count.)**
...
| `MAJOR_EVENT_ENTRY` | NAVIGATION_HANDOFF | `major-event.entry` | VIEWER | Y | N | — |
...
- **`MAJOR_EVENT_ENTRY`:** The table above already reflects the corrected classification. Stage 1's static default guessed `WORKFLOW_CONTINUATION`; reading the actual handler in Stage 2 found it literally shares `capabilityResult()` with `CAPABILITY_DISCOVERY` -- a navigation/handoff, not a durable-workflow binding. See §5's cross-cutting findings and the coverage matrix's own `MAJOR_EVENT_ENTRY` note for the full reasoning; this table cell was corrected here after external review found it had drifted back to the Stage-1 guess.
...
| `INCIDENT_CONTINUATION` | READ_RESULT | `incident-claim.continuation` | VIEWER | Y | N | — |

- **`INCIDENT_CONTINUATION`:** The table above already reflects the corrected classification. Stage 1's static default guessed `WORKFLOW_CONTINUATION`; see §5's cross-cutting findings (below) for the full reasoning — a plain, stateless list with no `DecisionThread`, no durable workflow identity, and no resumption logic. This table cell was corrected here after external review found it had drifted back to the Stage-1 guess.
...
- **`INVENTORY_LOOKUP`:** real per-read `contextVersion` (sha256 of all items' `{id,updatedAt}`); filtering is message-text-derived server-side (category/specific-item/token search), not a structured, round-trippable filter — a handoff to `/inventory` loses that filter context. **Update (2026-09-18, ASK_COZY_INLINE_WORKSPACE_FRD Phase 3):** the `inventory-results` block now renders through a bespoke `InventoryResultList`/`InventoryItemDetail` pair — item titles open canonical detail inline (direct `GET .../inventory/items/:itemId`, re-validated on open) instead of navigating away. **Second update, same day:** `inventory-entity-selection` is now also routed through the same component (selecting an ambiguous match opens inline detail rather than a bare href). **Third update, same day:** `inventory-history` (a different entity type, `HomeEvent`) is now inline too, via a new `HomeEventResultList`/`HomeEventDetail` pair reusing the existing `homeEventsApi.getHomeEvent` client function — all three `inventory-*` blocks are now inline-complete. No item `actions` exist yet on any of them (read-only slice).
- **`DOCUMENT_LOOKUP`:** ~~**no `contextVersion` at all** — relies solely on the shared `observedAt` wrapper, which can't detect that documents actually changed.~~ **Correction (2026-09-18), verified directly against source:** this was wrong even when originally written — `documentLookupResult` has set a real per-read `contextVersion` (sha256 of every document's `{id, verificationStatus, updatedAt}`) since 2026-09-13 (commit `621837fb`), the same shape as `INVENTORY_LOOKUP`'s. `GROUPED_LIST` still declares `filters: []`, and each type-group is still capped at 20 items (`docs.slice(0, 20)`) with no truncation disclosure — both still-open gaps, unrelated to the corrected claim above. **Also same day (ASK_COZY_INLINE_WORKSPACE_FRD Phase 3):** the `document-lookup-groups` block now renders through a bespoke `DocumentResultList`/`DocumentDetail` pair (the fourth bespoke `GROUPED_LIST` exception) — item titles open canonical detail inline via a new property-scoped, VIEWER-floor `GET /api/documents/property/:propertyId/:documentId`, deliberately not the existing `GET /api/documents/:id` (`requireDocumentOwnership`, CONTRIBUTOR floor for a non-uploaded document), which would 404 for every VIEWER-role household member opening a document they didn't personally upload. Reconciliation is also closed: `DOCUMENT_LOOKUP` is now a declared `ASK_MUTATION_IMPACT_MAP` sibling of both `DOCUMENT_PROMOTION_CONFIRM` and `CAPTURE_EVIDENCE_CONFIRM`.
...
- **`CAPTURE_FACT_CONFIRM` / `CAPTURE_EVENT_CONFIRM` / `CAPTURE_WARRANTY_CONFIRM` / `CAPTURE_EVIDENCE_CONFIRM`:** all four share the platform idempotency/UI/freshness layers above, plus real per-operation specifics: `CAPTURE_EVENT_CONFIRM`'s correction path adds a bespoke `ask-correction:{executionId}` key guarding `updateHomeEvent` (which has no idempotency of its own); `CAPTURE_EVIDENCE_CONFIRM` delegates to `HomeEventsService.attachDocument` (upsert-idempotent on `[eventId, evidenceKey]`) and enforces a real ordering constraint — it fails clearly with `EVIDENCE_SIBLING_EVENT_NOT_CONFIRMED` if its paired `CAPTURE_EVENT_CONFIRM` hasn't completed yet. ~~**All four confirmed to have the reconciliation gap** from §2.5 — none populate `refreshedExecutions`.~~ **Update (2026-09-18):** `CAPTURE_FACT_CONFIRM` and `CAPTURE_EVENT_CONFIRM` now call `reconcileAskExecutionSideEffects` (siblings: `PROPERTY_SUMMARY`, `INVENTORY_LOOKUP`) — `confirmCaptureEvent`'s fix applies at all five of its return paths (idempotent replay, both concurrent-write-race recoveries, correction, and normal create) via a shared `captureEventConfirmResult` helper. **Second update (2026-09-18, later same day):** `CAPTURE_WARRANTY_CONFIRM` (siblings: `PROPERTY_SUMMARY`, `INVENTORY_LOOKUP`) and `CAPTURE_EVIDENCE_CONFIRM` (siblings: `DOCUMENT_LOOKUP`, `PROPERTY_SUMMARY`) are now fixed too — all 4 of 4 `CAPTURE_*_CONFIRM` operations populate `refreshedExecutions`, closing this gap completely. All four still hand off via a bare `href` (`/edit` or `/timeline`) with no return-anchor or field-level target — none would pass FRD §17's "no handoff completion from `href` alone" on their own; that gap is unrelated to reconciliation and remains open.

**Track-level assessment:** UI surface and idempotency are solid (shared, well-tested platform mechanisms). Freshness is inconsistent — `PROPERTY_SUMMARY`/`DOCUMENT_PROMOTION_CONFIRM`/`INVENTORY_LOOKUP` have real version sources, `DOCUMENT_PROMOTION_REVIEW`/`DOCUMENT_LOOKUP` don't. ~~**Reconciliation is the track's biggest gap**: 5 of 5 confirmation-gated operations in this track fail XREC-001 as currently implemented.~~ **Update (2026-09-18, later same day): 5 of 5 now populate `refreshedExecutions`** (`DOCUMENT_PROMOTION_CONFIRM`, `CAPTURE_FACT_CONFIRM`, `CAPTURE_EVENT_CONFIRM`, `CAPTURE_WARRANTY_CONFIRM`, `CAPTURE_EVIDENCE_CONFIRM`) — this track's reconciliation gap is fully closed. Handoff still exists everywhere as a bare `href` but nowhere carries return context, which the FRD explicitly says doesn't count as complete -- that gap is unrelated to reconciliation and remains open. Separately, `INVENTORY_LOOKUP` (this track's one read now genuinely reconciled by three of the five above) also gained a bespoke inline-detail component (ASK_COZY_INLINE_WORKSPACE_FRD Phase 3, same day) -- see its own §2.4 entry.
...
~~**Reconciliation gap confirmed for the one confirmation-gated operation** (`REFINANCE_RATE_MONITOR` — grep-verified no `refreshedExecutions`), consistent with every other track~~. **FIXED (2026-09-18):** `confirmRefinanceRateMonitor` now calls `reconcileAskExecutionSideEffects` (`REFINANCE_ANALYSIS` declared as the sibling) — its inline pause/stop/edit actions remain, so both mechanisms apply together rather than one substituting for the other.
...
**`MAINTENANCE_STATUS`/`TASK_CREATE`/`TASK_COMPLETE`/`TASK_UPDATE`** — the FRD's own predecessor reference slice — were traced directly rather than left assumed-correct. They hold up: `MAINTENANCE_STATUS`'s `MaintenanceViewState` (a stable `resultId`, minted once and carried across every filter/refresh of the same interactive result, distinct from `executionId`) is the literal architectural ancestor of ROLL-002's "refining one dimension preserves compatible dimensions" requirement. As of this document's original tracing (2026-09-16), `MAINTENANCE_TASK_UPDATE` was **the only operation in all 77 with a real `editableFields` implementation**, and `MAINTENANCE_TASK_COMPLETE`/`UPDATE` were **the only two operations in the platform that actually populate `refreshedExecutions`** — the reconciliation mechanism this audit found missing everywhere else was not missing here; it was the one place it had been built and wired. **Update (2026-09-17, historical baseline vs. current state):** the later Buyer B04 remediation (see [[project_ask_cozy_cross_domain_rollout_frd]] / `ASK_COZY_PHASE6_BUYER_ACCEPTANCE_VERIFICATION.md`) added `BUYER_TASK_UPDATE` as a 2nd operation with a real `editableFields` entry (after `MAINTENANCE_TASK_UPDATE`) and a 3rd operation populating `refreshedExecutions` (after `MAINTENANCE_TASK_COMPLETE`/`UPDATE`), reusing the exact same generalized mechanism (`refreshAskSourceExecution`, renamed from `refreshMaintenanceSourceExecution`). The counts above describe this document's own original Stage 2 pass, not current platform state — as of 2026-09-17, current state is 2/77 with real editable fields and 3/30 confirmation-gated operations populating `refreshedExecutions`, not 1/77 and 2/30 (confirmed by direct grep of every non-empty `editableFields:` literal in `askOrchestrator.service.ts` — exactly 2 conditional assignments exist). B04's own reconciliation is itself scoped to one prior list, not "every affected result" — see the Buyer document's own re-scored B04 verdict for that separate, narrower limitation.
...
**Reconciliation gap final tally**: of the 30 confirmation-gated operations traced this session (2026-09-16), exactly **2** (`MAINTENANCE_TASK_COMPLETE`, `MAINTENANCE_TASK_UPDATE`) populated `refreshedExecutions`. The other 28 did not. **Update (2026-09-17):** the Buyer B04 remediation added `confirmBuyerTaskUpdate` as a 3rd operation populating `refreshedExecutions` via the same generalized mechanism — current state is 3/30, not 2/30, as of that fix. ~~This tally describes the original Stage 2 tracing pass, not current platform state.~~ **Correction (2026-09-18), verified directly against source (`grep -n 'await reconcileAskExecutionSideEffects(' askOrchestrator.service.ts` plus the two `refreshAskSourceExecution` maintenance call sites), not by trusting this document's own running tally**: the 2026-09-17 update above undercounted. The pre-2026-09-18 total was actually **6/30**: `MAINTENANCE_TASK_COMPLETE`/`MAINTENANCE_TASK_UPDATE` (via `refreshAskSourceExecution`), `BUYER_TASK_COMPLETE`/`BUYER_TASK_UPDATE` (B03/B04, `ASK_COZY_PHASE6_BUYER_ACCEPTANCE_VERIFICATION.md`), and `CLAIM_FILE`/`CLAIM_TRANSITION` (P05, `ASK_COZY_PHASE8_PROTECTION_ACCEPTANCE_VERIFICATION.md`) — the Claims pair was never folded into this running count even after that document shipped. This session (2026-09-18) added 5 more (`DOCUMENT_PROMOTION_CONFIRM`, `CAPTURE_FACT_CONFIRM`, `CAPTURE_EVENT_CONFIRM`, `BUYER_LIFECYCLE_UPDATE`, `REFINANCE_RATE_MONITOR`) — current state is **11/30**. This tally describes the original Stage 2 tracing pass, not current platform state; re-derive by grepping the live source rather than trusting any running count in prose, including this one, indefinitely.
...
1. **XREC-001 reconciliation is real but essentially unbuilt.** 2 of 30 confirmation-gated operations used it as of this document's original tracing (both maintenance). **Update (2026-09-18, verified directly against source): current state is 11/30** — `MAINTENANCE_TASK_COMPLETE`/`MAINTENANCE_TASK_UPDATE` (original), `BUYER_TASK_COMPLETE`/`BUYER_TASK_UPDATE` (B03/B04), `CLAIM_FILE`/`CLAIM_TRANSITION` (P05), and `DOCUMENT_PROMOTION_CONFIRM`/`CAPTURE_FACT_CONFIRM`/`CAPTURE_EVENT_CONFIRM`/`BUYER_LIFECYCLE_UPDATE`/`REFINANCE_RATE_MONITOR` (this session). The other ~19 — including at least `CAPTURE_WARRANTY_CONFIRM`/`CAPTURE_EVIDENCE_CONFIRM` (Records and capture) and `BUYER_TASK_CREATE`/`BUYER_FINDING_DISPOSITION` (Buyer), plus whatever this document's other tracks (Decisions and projects, and the remaining Financial and ownership operations beyond `REFINANCE_RATE_MONITOR`) haven't since been separately fixed — still leave every other still-visible Ask result unrefreshed after a successful mutation, even when their own confirmation copy sometimes promises reconciliation happens (`BUYER_FINDING_DISPOSITION`) — that reconciliation is real, but happens inside the owning domain service's own write, invisible to Ask's own result surfaces. (`CLAIM_TRANSITION`'s own version of this same promise is no longer an example of the gap — see its P05 fix, folded into the 11/30 above.)
2. **The undisclosed item-cap pattern (`slice(0, N)`/`take: N` with count shown, no "showing X of Y" or continuation control) recurred in essentially every list-shaped read across every track.** `CAPITAL_RESERVE_PLAN`'s `TABLE` block (§4.13) and `INTELLIGENCE_ENVELOPE_QUERY`'s cursor pagination (§4.9) were the two genuine, disclosed exceptions found during this document's own Stage 2 pass. **Correction (external review, 2026-09-17):** the later, deeper Completed-reference re-verification (`ASK_COZY_PHASE1_COMPLETED_REFERENCE_REVERIFICATION.md`) found a third: `MAINTENANCE_STATUS`'s `GROUPED_LIST` also discloses its cap (`"Showing up to N items per section"` plus an always-first "View all in Maintenance" link for genuine full-result access) — missed here because this document's own Completed-reference tracing (§4.15) was comparatively shallow, consistent with ROLL-010's warning against treating a handler's mere existence as verified. Three exceptions, not two, as of the later pass; this document's own count was accurate only as of when it was written.
...
2. **XREC-001 reconciliation**: ~~28 of 30 confirmation-gated operations don't refresh other still-visible Ask results after a successful mutation. `refreshMaintenanceSourceExecution`/`refreshedExecutions` already exists and works (2 operations use it)~~ **Partially closed, ongoing rollout, current state 11/30 (see §4.15/§4.10/§4.6 update notes above)** — this is a rollout gap, not a missing mechanism, and continues to be closed operation-by-operation rather than all at once.
...
**Stage 1 and Stage 2 of Phase 0 are both complete: all 77 registered operations are classified and traced.** §4.5–§4.15 record the evidence per track; §5 records the cross-cutting findings; §6 records what's genuinely still open. **Correction (external review, 2026-09-17):** this sentence originally said the §4.8 (§22) decision was "awaiting the user before Phase 4 can proceed further," contradicting this document's own opening status line, written and updated afterward, which already states the decision is DECIDED and IMPLEMENTED (Option B, read-attach only, 2026-09-17 — see `docs/architecture/ASK_COZY_PHASE4_SELL_HOLD_RENT_UX_COMPLETION_VERIFICATION.md` for the Phase 4 work built on top of it). This closing sentence was never updated after that decision was made later in the same engagement; it's corrected here to match the opening rather than left as stale drift.
