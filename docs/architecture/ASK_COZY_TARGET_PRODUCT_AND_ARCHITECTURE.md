...
**Revision note 4:** a fourth review pass found two more gaps, both verified before correcting: (1) "shared lease ownership" stopped two attempts from *starting* concurrently but didn't stop a lease from being legitimately reclaimed while the original, still-alive-but-slow attempt kept running — if that stale attempt later finished, its candidate-set commit had no way to know its claim had moved on, and deterministic candidate identities only prevent duplicate rows, not a stale result silently overwriting or adding to a newer, already-persisted one; fixed by making the candidate-set transaction's "mark processed" step a conditional update gated on the claim token observed at claim time, so a stale attempt's entire commit — candidates included — rolls back if its lease was reclaimed out from under it; (2) `linkedExecutionId` was set only on the warranty candidate, so if the warranty completed first its reconciliation worked, but the event's later completion had no outgoing sibling reference to act on at all — fixed by setting the field symmetrically on both candidates at creation time, so either side's completion always carries a usable pointer.
...
**[CORRECTION — P1]** "Shared lease ownership" (above) prevents two attempts from *starting* concurrently — it does not prevent a lease from expiring and being legitimately reclaimed by a second attempt while the *first* one is still running, just slow (not crashed). A slow-but-alive first attempt has no way to know its lease was reclaimed, and if it eventually finishes and reaches the atomic-persist transaction, it can commit a candidate set derived from a stale run — and deterministic candidate identities (above) only prevent that stale set from duplicating rows; they don't prevent it from **overwriting or adding to** whatever the newer, already-completed claimant already wrote, since an `upsert` against the same deterministic keys will happily apply an older result on top of a newer one.

**[DECISION]** The atomic candidate-set transaction must re-verify lease ownership as part of the same commit, not just at claim time: the claiming `updateMany` (the shared-lease step above) records a claim token on the `DomainEvent` row — its current `attempts` count or an equivalent generation marker, consistent with how this document's other lease-based recoveries already track re-claims (`HomeEvent`/`AskConfirmationReceipt`'s own attempt-count pattern). The candidate-set transaction's write to mark the event `PROCESSED` is a **conditional** update — `UPDATE ... WHERE id = event AND attempts = <the value observed at claim time>` — inside the same transaction as the candidate creates. If that conditional update affects zero rows (because a later claimant already bumped the count, meaning this caller's lease was reclaimed out from under it), the whole transaction throws and rolls back, including the candidate creates — a stale, slow-finishing attempt can never persist anything once its claim has moved on, regardless of what it computed.

Together these four close both failure modes: shared leasing prevents two concurrent *starts*; the commit-time claim-token check prevents a stale, slow-finishing attempt from persisting after its lease was legitimately reclaimed; atomic all-or-nothing persistence prevents a partial candidate set from ever being visible or mistaken for a complete one; stable identities make a legitimate retry (by the *current* claimant, not a stale one) converge instead of duplicating.
...
- **`Warranty`**: has no `idempotencyKey` field of its own (**[FACT — this pass]**, confirmed absent from its schema).
  ...
  **[DECISION]** Linking, independent of confirmation order: `AskExecution` gains a second self-referencing field, `linkedExecutionId: String?` — distinct from `parentExecutionId` (§7, "which turn produced this").

  **[CORRECTION — P2]** An earlier draft set `linkedExecutionId` only on the warranty candidate, pointing at the event — but the reconciliation design below needs *either* side's completion to carry a usable sibling reference, and only one side had one. If the warranty completes first, its reconciliation event correctly carries `linkedExecutionId` and can check the event's status. But when the event *later* completes, the event's own execution row has no `linkedExecutionId` of its own — its reconciliation event has nothing to point at, so the one completion that's actually in a position to close the loop (the second one, per the reconciliation design's own logic) has no reference to act on.

  **[DECISION]** set `linkedExecutionId` **on both** candidates, symmetrically, at the moment §7's atomic candidate-set transaction creates them — extraction already knows the two are related at creation time (that's the only reason a link is being made at all), so populating it on both sides costs nothing extra and isn't a separate step. Every completion, event's or warranty's, then carries a valid outgoing `linkedExecutionId` in its reconciliation event, and the reconciliation handler's logic (check the sibling's status, act if it's done, no-op otherwise) works correctly regardless of which side happens to complete first — no direction-dependent special case, no reverse lookup required.
