...
2. *Then*, as a latency optimization only, attempt the extraction call inline with a short, strict timeout (illustrative: ~1.5s — the exact figure is a Stage 3 tuning question). If it completes in time, merge its candidate items into the same response, exactly as before — but **also** mark the just-emitted `DomainEvent` processed (or have the outbox consumer's handler recognize, via `parentExecutionId`, that a child `CAPTURE_*_CONFIRM` execution already exists for this event and no-op) so the durable job doesn't redundantly re-run the same extraction a second time.
3. If the inline attempt doesn't complete in time (or the process restarts before it can), the primary routed response is returned unaffected, and the **already-durable** event is picked up by the existing outbox consumer on its normal poll cycle — nothing new needs to happen for the async path to work, because the event was never conditional on the inline attempt succeeding. Results are delivered via `notifyWithAskContinuation` exactly as described previously.
...
| `@@unique([propertyId, factKey, sourceEntityId])` | `PropertyFactEvidence` | New constraint (`sourceEntityId` itself is reused, not new — see below) | §16 |
...
| `sourceEntityId` (populated, not added) | `PropertyFactEvidence` | **Reused** — already exists; `capturePropertyFact`'s internal idempotency check is new logic, not a new field (the new unique constraint above is what's actually new) | §16 |
...
- **`Warranty`**: has no `idempotencyKey` field of its own (**[FACT — this pass]**, confirmed absent from its schema).
  ...
  **[DECISION]** Linking, independent of confirmation order: `AskExecution` gains a second self-referencing field, `linkedExecutionId: String?` — distinct from `parentExecutionId` (§7, "which turn produced this") — set on the warranty candidate to point at its sibling event candidate's `executionId` when extraction determines the two are related (both from `§14`'s split of the same statement). At confirm time:
  - If the **event** confirms first: its execute phase upserts `HomeEvent` as already designed. When the **warranty** later confirms, its execute phase creates the `Warranty` row (idempotent per above) and — because it can resolve its sibling via `linkedExecutionId` — reads that sibling execution's recorded `artifactId` (on its own now-`COMPLETED` `AskConfirmationReceipt`) to get the `HomeEvent` id, and sets `HomeEvent.warrantyId` in the same transaction as the `Warranty` create.
  - If the **warranty** confirms first: its execute phase still creates the `Warranty` row (it does not need the `HomeEvent` to exist to do this — it's an independent record with its own required fields, `propertyId` included), just without `HomeEvent.warrantyId` set yet. When the **event** later confirms, its execute phase checks whether its own execution is the `linkedExecutionId` target of any already-`COMPLETED` sibling (a warranty), and if so, sets `HomeEvent.warrantyId` to that warranty's id in the same transaction as its own upsert.
  - Either order, the link ends up set exactly once, each write is independently idempotent against its own replay, and neither write depends on the other having already happened — only on being able to *look up* the other if it has.
- **Scalar capture (`PropertyFactEvidence` via `capturePropertyFact`)**: **[FACT — this pass, and the specific mechanism Stage 1 originally flagged]** `capturePropertyFact` currently always `create`s a new `PropertyFactEvidence` row — it has no existing idempotency guard, and this is precisely the failure mode Stage 1's audit identified in the current `GroundedAskProposal` path ("a retry calls `capturePropertyFact` again and can create a second `PropertyFactEvidence` row for the same claim"). Converging onto `AskConfirmationReceipt` does not fix this by itself.
  ...
  **[DECISION]** add `@@unique([propertyId, factKey, sourceEntityId])` to `PropertyFactEvidence` (consolidated into §14's schema-change inventory) and change the write to attempt a `create` directly, catching the unique-constraint violation (the same `P2002`-catch-and-re-read pattern `AskConfirmationReceipt`'s own claim phase already uses — Stage 1's finding, reused here rather than invented) rather than checking first. On a violation, fetch and return the existing `(propertyId, factKey, sourceEntityId)` row **regardless of its current `supersededAt` value** — a replay of execution A must always resolve to A's own original write, never re-evaluate whether that write is still the *current* fact, which is an entirely separate question already correctly handled by supersession semantics whenever a *different* execution (like B) writes a correction. Idempotency-per-execution and current-value-per-fact are two different questions; the fix is keeping them from being answered by the same query.
...
| `PropertyFactEvidence` | **EXTEND** | Gains `captureChannel`, `attribution`, and `extractionConfidence` fields plus a `(propertyId, factKey, sourceEntityId)` unique constraint (§15, §16); `confidence` keeps its existing meaning unchanged — it is deliberately **not** repurposed (§15 correction) |
...
6. **[CORRECTION — cleanup]** Apply §14's consolidated schema-change inventory as direct edits to `prisma/schema.prisma`, not a migration script — an earlier draft of this item said "produce the detailed schema migration," which reads as instructing exactly the artifact this repository's convention prohibits. **[CORRECTION — a second, narrower overreach in the first correction]** that first fix then stated `npx prisma db push` as *the* rollout convention — this repository's `CLAUDE.md` authorizes `db push` as the local-development apply step for schema edits (the command a developer runs against their own dev database after editing the schema), which is a real and correctly-cited convention, but neither `CLAUDE.md` nor anything in Stage 1's audit says anything about how schema changes reach a production database — that is a separate, unestablished question this document has no evidence for. **[DECISION]** Stage 3 edits `prisma/schema.prisma` directly (no migration script) and applies it locally with `npx prisma db push` during development, per the repo's stated dev-workflow convention — and separately confirms, from whoever owns deployment, what the actual production rollout mechanism is, rather than assuming `db push` is also that. Resolve the two flagged enum-vs-string questions in §14's inventory (`DecisionThread.goalCode`, `AskExecution.operationId`) before editing the schema, since the answer determines whether those two rows need a schema change at all.
