...
### Intelligence Readiness Score: **46 / 100**
...
| Attention Engine | 42 | A real, deterministic, two-layer scoring engine (`scoreHomeAction` in `homeActions.service.ts`, fed by `orchestration.service.ts`'s upstream aggregation) with genuine two-pass entity-level dedup (fuzzy text key, then durable work-item key) and a safety-aware lifecycle (hard-blocks deferring `SAFETY_EMERGENCY` items) — a real asset, not a toy. Capped by: (a) it's one of **three parallel systems** answering "what does this property need," (b) the weight table (40/32/28/12, 30/22/12/4, 12/10/8...) is hand-tuned with no documented model behind it, (c) richer continuous urgency computed upstream is discretized into 4 buckets before the ranker sees it, losing resolution, and (d) explanation is a single flattened string — the structured, per-source `DecisionTraceStep[]` explainability that would make this a real "why" engine exists only in the dead `orchestration.service.ts` path. |
...
| Evaluation & Observability | 34 | Instrumentation is more mature than typical pre-launch: a real activation score (`computeSetupStatus`), real timestamped time-to-first-value fields, event-driven cohort retention computed from genuine activity rows, a centralized 37-event analytics taxonomy with a curated 9-stage north-star lineage, and — genuinely good news — a **real, general-purpose, cohort-based feature-flag/rollout system** (`config/featureFlags.ts` + `rollout.middleware.ts`) already exists. Two things still pull the score down hard: (1) three separate "was this helpful" write paths (Ask up/down, home-action usefulness, personalization feedback) feed **no downstream aggregate** — they're write-sinks, not measured signals; (2) 539 backend test files include two genuine golden-fixture suites (10 property archetypes, 17 savings-benefits scenarios) but **zero** evaluate LLM-generated content quality — the one AI-generation test checks JSON well-formedness, never correctness. |
...
The clearest evidence for "not yet one system": the single component most explicitly designed to deliver this audit's target experience — *"here's what needs your attention, here's why, here's what I recommend"* with full explainability — is `OrchestrationDecisionTrace` and its supporting `orchestration.service.ts`. It is backend-complete, well-built, and **has zero live frontend consumers**; every one of its 7 frontend call sites is dead code or a discarded computation (see [§18, Blocker #1](#18-top-10-intelligence-blockers)). Meanwhile, two *other* independent implementations of "what does this property need" — the canonical Home tab feed and the separate Fix-hub resolution center — are both live, both real, and share no code with each other. A homeowner using C2C today gets answers to "what needs my attention" from two different, non-communicating backends depending which tab they're on, and the one component built to explain *why* isn't reachable from either.
...
2. `orchestration.service.ts` → the upstream aggregator *and* a separate, more sophisticated system that additionally builds a structured, per-source `DecisionTraceStep[]` explainability trace and persists it (`orchestrationDecisionTrace.upsert`) — genuinely better explainability than anything in the live path, **currently unreachable** by any homeowner.
3. `resolutionCenter.service.ts` → the Fix hub's independent implementation, **shares no code with either of the above**.

A homeowner's sense of "what does C2C think I should do" today depends on which of two live surfaces (Home tab vs. Fix hub) they're looking at. The single-string `explanation` field `scoreHomeAction` produces for the live feed is a materially worse "why" than the structured trace sitting unused one layer up.
...
**A related, confirmed gap: "was this recommendation good" feedback is fragmented into three independent write-only paths that feed no aggregate anywhere.** Ask execution feedback (helpful/not-helpful) increments a Prometheus counter (`askFeedbackTotal`) — an ops metric, not a product signal. Home-action usefulness feedback (per-item USEFUL/NOT_USEFUL) writes to its own service with no evidence of downstream consumption. Personalization's explicit/implicit feedback writes to its own suppression repository. None of the three feed the admin-analytics product dashboards or each other. This means C2C currently has **no way to know, in aggregate, whether its recommendations are actually good** — a direct blocker to the "did the homeowner find this useful" instrumentation goal (§15 of the original audit brief), distinct from the LLM-content-evaluation gap in §18 Blocker #4.
...
**Why compelling:** genuine cross-source correlation (Home Event Radar + Maintenance Prediction) that `homeActions.service.ts`'s confirmed scoring mechanics (§8) treat as two independently-scored, independently-deduped items — dedup only collapses candidates representing the *same* underlying issue, not two different issues that compound each other. This is a real, confirmed gap, not a hypothetical.
**Existing assets:** both signal sources already feed the same feed via `orchestration.service.ts`'s aggregation; `PropertyRadarCompoundInsight` model exists in the schema.
**Missing pieces:** whether `PropertyRadarCompoundInsight` is actually populated by any job was not confirmed in this audit — check that first, since it may already have a data source waiting for a consumer, which would make this even lower-effort than estimated.
**Effort:** Low–Medium.
...
**1. Two competing "what does this property need" ranking authorities, with the explainability layer unreachable from either.**
*Evidence:* `orchestration.service.ts`/`OrchestrationDecisionTrace` fully built, 7/7 frontend call sites dead (`docs/wiki/00-introduction.md` reachability audit, independently structurally consistent with this audit's own reading). `homeActions.service.ts` and `resolutionCenter.service.ts` share no code.
*Consequence:* the product cannot deliver "one thing that matters most, with a trustworthy explanation" — its own two live implementations disagree, and the one built to explain itself is invisible.
*Fix:* pick one ranking authority (recommend `homeActions.service.ts` — it's live, deterministic, and reasonably extensible per the ranking deep-dive). Port `orchestration.service.ts`'s decision-trace generation to run *against* `homeActions.service.ts`'s output instead of its own parallel aggregation. Reconnect `DecisionTraceDrawer` to `UnifiedHomeSurface.tsx`'s cards. Decide whether the Fix hub becomes a filtered view of the same feed or is retired.
*Effort:* Medium-High (real integration work, not a rewrite). *Risk:* Medium (touches the most-used surface in the app) — mitigate with a feature flag and a shadow-mode comparison period.
...
**4. No evaluation exists anywhere for LLM-generated content quality — and no aggregation exists for the "was this useful" feedback C2C already collects.**
*Evidence:* zero of 539 backend test files assert on generated-content correctness (the one AI-generation test, `diyAiGuideGeneration.test.js`, checks JSON well-formedness only); the only evaluation infrastructure found (Ask Trust, calibration releases) evaluates deterministic routing/scoring, not generated text. Separately, three real "was this helpful" feedback paths (Ask up/down, home-action usefulness, personalization feedback) each write successfully but feed no downstream aggregate — confirmed via direct code read, not inference.
*Consequence:* every Gemini-backed feature ships and evolves with no regression signal, **and** even the feedback C2C already asks homeowners for goes nowhere — a double gap in the same "does this actually work" loop.
*Fix:* see §25, Synthetic Evaluation Suite (build on the two golden-fixture suites that already exist rather than starting from zero). Separately, route the three existing feedback write-paths into `adminAnalytics` as a quick win — this is wiring, not new collection.
...
| Recommendation contract | **New** (thin) | A read-model/adapter view over existing tables — additive, not a schema rewrite |
...
1. **Confirm and remove dead AI surfaces.** Grep-confirm zero live callers of `/api/gemini/chat` (`groundedAsk.service.ts`), `taxAppeal.routes.ts`, and the Home Renovation Risk Advisor frontend page; remove or formally 410-deprecate. *Files:* `apps/backend/src/routes/gemini.routes.ts`, `taxAppeal.routes.ts`, `apps/backend/src/homeRenovationAdvisor/*`, `apps/frontend/src/app/(dashboard)/dashboard/home-renovation-risk-advisor/`. *Completion criteria:* routes return 410 or are removed; no active Gemini spend from these paths.
...
3. **Fix or remove Property Appreciation's silently-broken "live web search" grounding.** Either implement a real search call or remove the dead `declare const google` block and stop implying grounding that doesn't happen. *Files:* `apps/backend/src/services/propertyAppreciation.service.ts`. *Completion criteria:* the code path either does what it claims or the UI copy is corrected to not imply live grounding.
