...
The Prisma schema is far ahead of checked-in migration history: only five `migration.sql` files are present under `apps/backend/prisma/migrations/`, while the schema contains hundreds of models/enums, and `database/migrations/` is empty apart from `.gitkeep`. This makes schema-parity verification, additive migration rehearsal, and backward-compatible rollout a Phase 0 requirement; the schema alone cannot prove production database state.
