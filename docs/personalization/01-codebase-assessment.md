...
The Prisma schema is far ahead of checked-in migration history: only five `migration.sql` files are present under `apps/backend/prisma/migrations/`, while the schema contains hundreds of models/enums, and `database/migrations/` is empty apart from `.gitkeep`. During greenfield internal validation, the user applies the desired schema to a disposable database and no backfill/rehearsal machinery is required. Conventional migrations and rehearsal become necessary only after deployed data must survive schema changes.
