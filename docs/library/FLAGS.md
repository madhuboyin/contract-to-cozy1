...
2. **Authority gaps are now narrower.** Guidance Engine and ~7 overlapping April pre-launch plans still lack a complete declared winner (C3, C10). Ask scoped reading order is now recorded in `AUTHORITY.md`; requirement atomization and delivery verification remain incomplete (C6).
...
| C6 | Ask family (`product/ASK_COZY_*`, `AI_HOME_CONCIERGE_ASK_*`, `architecture/ASK_COZY_*`) | Scoped lineage and precedence are now recorded in `AUTHORITY.md`: specific audience/trust addenda retain their scope, Cross-Domain governs rollout, and Inline Workspace overrides only routine handoff and richer-workspace exclusions. Delivery remains incomplete at requirement level: three Ask FRDs need stable IDs; phase verification is mainly static and contains explicit partials; the Incremental FRD remains proposed unless separately evidenced. | [read] requirements and verification records · [executed] focused trust/audience tests |
