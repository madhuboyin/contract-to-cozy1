...
This is a document-level authority screen for the 28 files classified as FRD/PRD in `catalog.csv`. All 313 source documents received inventory, metadata, date, and cited-path checks; the 28 documents below were also checked for their stated status, supersession, and overlap with nearby requirement families. Targeted code paths were traced where a conflict was found. **This does not certify every requirement sentence or runtime behavior.** `catalog.csv` records each file's `review_basis` so a future change does not mistake an unflagged row for a full semantic review.
...
| [Ask Intelligence Incremental](../product/AI_HOME_CONCIERGE_ASK_INTELLIGENCE_INCREMENTAL_FRD.md) | Earlier Ask addendum | Proposed status and older naming need reconciliation with the September Ask Cozy family (`C6`). |
| [Ask Redo](../product/AI_HOME_CONCIERGE_ASK_REDO_FRD.md) | Earlier Ask requirements | “Implementation in progress”; no explicit lineage to the newer Ask Cozy FRDs (`C6`). |
...
| [Ask Cozy Inline Workspace](../product/ASK_COZY_INLINE_WORKSPACE_FRD.md) | Governing completion target | Explicitly defines precedence and tracks partial implementation by requirement. The source file is changing concurrently; recheck its latest version before work. |
...
| [Ask Cozy Message First](../product/ASK_COZY_MESSAGE_FIRST_FRD.md) | Stage 3 intent | No top-level status was found by the catalog; reconcile with the Inline Workspace FRD and its paired implementation plan before treating it as delivered. |
...
## Decisions for the other 285 files
...
- The 16 files classified as plans describe sequencing, not necessarily current behavior. The April v1 plans explicitly superseded by v2 are deletion candidates; other plans contain open or partially implemented work and need family-level reconciliation.
- The other 259 files are audits, feature references, status records, runbooks, and assets. Their dates, references, and detected conflicts are in `catalog.csv`. Historical `H` and wiki `W` flags are snapshot warnings, not invalidations of every contained statement.

The unresolved families with material requirement ambiguity are Guidance (`C3`), Ask (`C6`), Property Context (`C7`), and the overlapping April plans (`C10`). The [flag review](FLAGS.md) records the exact disagreement and [deletion candidates](FLAGS.md#deletion-candidates). Resolve a behavioral difference from the specific FRD, directly affected contract/schema/code, and any later explicit supersession; do not pick a winner from filename or modification date alone.
