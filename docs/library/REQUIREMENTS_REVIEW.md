...
| [Ask Redo](../product/AI_HOME_CONCIERGE_ASK_REDO_FRD.md) | Early platform baseline | Retains adopted routing, operation, confirmation, and governance constraints where later scoped requirements do not revise them. It still needs stable requirement IDs. |
...
| [Ask Cozy Inline Workspace](../product/ASK_COZY_INLINE_WORKSPACE_FRD.md) | Governing completion target | Explicitly overrides routine handoff and richer-workspace exclusions and tracks partial implementation by requirement. Recheck its current version before work. |
...
| [Ask Cozy Message First](../product/ASK_COZY_MESSAGE_FIRST_FRD.md) | Message-first architecture target | Complements the later interaction documents but has no top-level delivery status or stable requirement IDs; do not treat it as delivered from architecture prose. |
...
The unresolved families with material requirement ambiguity are Guidance (`C3`), Property Context (`C7`), and the overlapping April plans (`C10`). Ask reading precedence is now recorded in `AUTHORITY.md`; its remaining work is requirement atomization and delivery verification (`C6`). The [flag review](FLAGS.md) records the exact disagreement and [deletion candidates](FLAGS.md#deletion-candidates). Resolve a behavioral difference from the specific FRD, directly affected contract/schema/code, and any later explicit supersession; do not pick a winner from filename or modification date alone.
