...
| C14 | `docs/README.md` | Index lists `api/`, `deployment/`, `development/` - each holds only `.gitkeep` - and omits `audit/`, `audit-gemini/`, `audits/`, `acquisition/`, `personalization/`, `wiki/` and the seven root-level Pass/dashboard docs. | [executed] `find` |
