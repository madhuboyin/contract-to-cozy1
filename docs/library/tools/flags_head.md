...
2. **Authority gaps are now narrower.** Guidance Engine and ~7 overlapping April pre-launch plans still lack a complete declared winner (C3, C10). Ask scoped reading order and stable requirement IDs are now recorded; requirement-level delivery verification remains incomplete (C6).
...
| C6 | Ask family (`product/ASK_COZY_*`, `AI_HOME_CONCIERGE_ASK_*`, `architecture/ASK_COZY_*`) | Scoped lineage and precedence are now recorded in `AUTHORITY.md`: specific audience/trust addenda retain their scope, Cross-Domain governs rollout, and Inline Workspace overrides only routine handoff and richer-workspace exclusions. Delivery remains incomplete at requirement level: phase verification is mainly static and contains explicit partials; the Incremental FRD remains proposed unless separately evidenced. The Ask FRDs now have stable IDs in the generated ledger. | [read] requirements and verification records · [executed] focused trust/audience tests |
