...
**Still open:** the cutover itself (work items 7-9) remains the largest gap and was not attempted in this pass — it is a genuine cross-surface rewrite (`resolutionCenter.service.ts`'s legacy discovery/ranking, the frontend's independent `resolutionCenterViewModel.ts` sort, and Cozy's priority-list consumption), not a contained bug fix.
