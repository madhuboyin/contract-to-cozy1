...
**Phase 5 is now fully complete.** All 6 work items landed: Radar compound-insight promotion (work item 1); all 7 HI-CMP-002 cross-domain rules (work item 2); the common `ExtractionEnvelope` contract and `documentPromotionAdapterRegistry` (work item 3); real envelope integration across Home Records/Warranty, Material Spec, and Loan Estimate extraction (work item 4); legacy inspection extraction adapted rather than retired, after verifying no separate finding truth exists to retire (work item 5); and document-promotion conflict detection extended from InsurancePolicy to Warranty and Expense (work item 6). Genuinely deferred, not silently dropped: candidate-mapping migration for the 3 `ExtractedFactCandidate`-based domains to consume the envelope's `fields` directly (regression risk judged to outweigh the marginal value); brand-new extraction pipelines for `INVENTORY`, `PROPERTY_TAX`, and `CLAIM` (no existing path to "adapt" — a materially larger, new-feature-scale undertaking); and wiring the Loan Estimate envelope into `refinanceRadar.controller.ts`'s live response (needs a coordinated frontend contract change this backend-only phase didn't make).
