...
**Status: complete for the registry-and-ownership modeling this phase scopes; the exit criterion's "outcome owner" trace resolves to a verified, documented "none" rather than a populated adapter — see the outcome-observation paragraph below before treating this as end-to-end outcome tracing.** `apps/backend/src/services/intelligence/` now holds the Home Action producer ownership registry (`homeActionProducerOwnership.ts`, one declared row per actual producer function — 23 of them, not just per source kind), the Home Action adapter ownership registry (`homeActionAdapterOwnership.ts`, now derived from the producer registry rather than hand-maintained independently), the capability/skill/guidance bridge registry (replacing an untyped, unvalidated map previously inline in `askOrchestrator.service.ts`, and now covering every capability with a non-empty `recommendation.sourceKinds` — 23 of 46 total capabilities — not just the 15 originally hand-picked), the completion evidence policy registry, and contract-only shells for the intelligence consumer registry and compound rule registry (intentionally unpopulated — Phase 2 and Phase 5 populate them respectively, since nothing yet invokes their handlers). All six are validated at process boot in `apps/backend/src/index.ts`, following the existing Ask/Decision Platform registry pattern; `validateHomeActionProducerKindConsistency` additionally catches a producer whose completion or outcome ownership silently disagrees with its source kind's default, and the two previously-undeclared id-prefix carve-outs in `executeHomeActionCommand` (`ownership-cost-change:`, `activation:`) are now sourced from named constants declared on their producer registry entries instead of inline string literals. A CI-time completeness test (`homeActionProducerOwnership.test.js`) reads the real producer source files and fails if a loader is added without a registry row for it — loaders aren't dynamically enumerable at boot, so this guardrail runs in the test suite rather than at startup.
...
**Outcome observation reality, verified by code read:** `hasOutcomeAdapter` is `false` for all 23 producers and all 11 source kinds today. `OutcomeObservationSourceType` (`prisma/schema.prisma`) already declares all 9 source types HI-OUT-005 calls for, but `outcomeObservationService.ts` implements creation for only 2 — `recordHomeownerReportedOutcome` (reachable only from Ask/Cozy chat, `askOrchestrator.service.ts`, never from a Home Action command) and `recordCompletedMaintenanceOutcome` (implemented, zero callers anywhere in the codebase). No producer's `COMPLETE` path creates an `OutcomeObservation`. This is real, unbuilt HI-OUT-005 functionality — expanding outcome creation to the remaining 7 source types and wiring it into `executeHomeActionCommand` is later-phase implementation work, not a Phase 0 registry gap — but the exit criterion asked for the trace to exist, and it now does: §6 of the registry report states this "0 of 23" fact plainly rather than leaving it a silent gap behind a hardcoded `null`. The functional-exit report at [`HOME_INTELLIGENCE_PHASE0_REGISTRY_REPORT.md`](./HOME_INTELLIGENCE_PHASE0_REGISTRY_REPORT.md) is genuinely generated — `scripts/generate-home-intelligence-phase0-report.ts` (`npm run report:home-intelligence-phase0`) builds every table directly from the registries above, including the new fact/signal, command, and outcome columns and a dedicated §6 on outcome-observation reality, and a parity test (`homeIntelligencePhase0Report.test.js`) fails CI the moment the committed file stops matching the generator's output, closing the staleness gap that let three Phase 1 loaders go undocumented after the report was first written. It also carries work items 3 and 4 (the 10-system independent-priority-calculation inventory, and the canonical-read-boundary decision), which remain hand-maintained prose inside the generator script — no registry in the codebase enumerates either of those today. Capability bridge completeness has a documented, narrower remaining gap: a capability reachable only via an Ask operation (empty `sourceKinds`) has no independent canonical signal marking it "Ask-reachable," so completeness there still depends on `capabilitySkillGuidanceBridge.registry.ts`'s own entries being kept current by hand rather than a mechanical check — see that file's header comment. No user-visible behavior changed in this phase, except the `activation-context:` `COMPLETE`-control removal above, which fixes a previously-broken control rather than changing an already-correct one.
...
**Still open:** the cutover itself (work items 7-9) remains the largest gap and was not attempted in this pass — it is a genuine cross-surface rewrite, not a contained bug fix. `resolutionCenter.service.ts`'s legacy discovery/ranking and the frontend's independent `resolutionCenterViewModel.ts` sort are the two most direct instances, but they are 2 of the full 10-system independent-priority-calculation inventory §2 of the Phase 0 registry report already catalogues — including `DashboardHeroSection.tsx`'s `rankHeroCandidates()` (re-ranks a third time on top of guidance-engine data already scored twice) and `notification.service.ts`'s `resolveAttentionPriority()` fallback. Cutting over Fix/Cozy alone would still leave the dashboard hero and notification paths independently ranked — the full work item 9 scope, not an incidental side effect of the Fix cutover.
...
**Still open:** work item 3's fuller bounded/pageable dynamic resolution scope (only the entity-identity threading `recommendation-snapshots` needed was added, not general pagination for a consumer with many dynamic targets); the live-browser verification of the new UI badge.
...
6 (remainder). The broader architectural request — unbounded `findMany`/no cursor pagination contract for a `DYNAMIC` consumer with many targets — is still real. The `MANUAL_REFRESH` fix above is bounded to 500 Decision Threads, not truly paginated; a property with more than that would still under-resolve. No consumer in this registry is close to that scale today, so this is deliberately deferred rather than building unused pagination machinery.
