...
- `IntelligenceRecomputeRun` records the triggering canonical change, context versions, optional source-health state for health-triggered runs, overall status, timing, and failure summary.
...
> **Update (2026-08-29, C2C Intelligence & Agentic Evolution Phase 4A):** a seventh family, `APPLIANCE_REPAIR_REPLACE`, is now registered — a snapshot-backed adapter (`applianceDecisionFamilyAdapter.ts`) over the authoritative non-HVAC `ReplaceRepairAnalysis`. `loadRepairReplaceDecisionActions` and `resolveWorkItemDecisionFamilyRefs` became category-aware: an HVAC item keeps the `repair-replace:` lineage prefix (`HVAC_REPAIR_REPLACE`); every other category uses `appliance-repair-replace:` (`APPLIANCE_REPAIR_REPLACE`). Before this, a non-HVAC repair/replace action resolved to the HVAC family and failed closed at `NOT_APPLICABLE`. See `docs/architecture/C2C_INTELLIGENCE_AGENTIC_EVOLUTION_IMPLEMENTATION_PLAN.md` §9.1.

The final acceptance-boundary closure is domain-aware rather than HVAC-only. `assertDecisionLineageSatisfiedForAcceptance()` resolves all attached decision-required work-item provenance through `OperationalWorkSource`: repair/replace guidance resolves via `ReplaceRepairAnalysis.inventoryItemId` (and, since Phase 4A, its `inventoryItem.category` picks the HVAC vs. appliance family), while coverage work resolves via `CoverageReview` and its current primary evidence-based high-priority question key. Acceptance requires `LINKED` lineage, `contextStatus=CURRENT`, and a non-null `currentRecommendationSnapshotId`; source-backed adapters project a changed or missing current source as stale during read-only lineage resolution, so an old snapshot cannot satisfy acceptance. A CoverageReview with no current primary question fails closed. Registry governance verifies that every work-item-eligible `DECISION_REQUIRED` producer has a supported source resolver, preventing a future producer from silently bypassing commitment enforcement.
...
- `sourceRegistry.ts` declares the owner, capability consumers, exact recompute-consumer keys, SLA, configuration, retry, fallback, degradation copy, and runbook for platform external and AI sources. The unified projection also includes reviewed Property Intelligence sources. Radar, Service Price Benchmark, Property Intelligence run, and operator pause/resume transitions emit `SOURCE_HEALTH_CHANGED` Property Changes for affected properties. The recompute registry has a distinct source-health dependency declaration, source health is carried durably into the recompute run, and successful validation leaves affected currentness `STALE`/`UNAVAILABLE` until a successful `CURRENT` recovery transition. An unmapped source-health event fails its run with an error summary instead of reporting a zero-target success.
...
- **Canonical attention and Fix projection:** Home, dashboard attention widgets, the sidebar, and Resolution Center/Fix now read `getHomeActionFeed()` through `GET /api/properties/:propertyId/home-actions`. Resolution Center preserves the canonical order and adds presentation grouping only. The independent property Resolution Center backend/service, browser view model, and global/property Fix route shims were deleted; every internal Fix link now targets the canonical surface directly. Disconnected dashboard-hero/Morning-Pulse ranking components and registrations for those deleted presentation owners were removed. The remaining `guidance-priority` registration explicitly describes its bounded use for Guidance-only journey deduplication, conflict resolution, suppression, and portfolio ordering; it does not re-rank canonical Home Actions. The raw orchestration service remains only as an internal source adapter used by `homeActions.service.ts`; its public route, controller, client APIs, DTOs, and presentation components were removed.
