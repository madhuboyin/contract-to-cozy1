...
| History data contract | Selected-home pagination/title search plus a bounded all-home scope implemented in code; focused contract/component tests and static verification passed | The authorized recent-session endpoint returns up to 20 conversations at a time with a validated opaque keyset cursor, ordered by activity and bounded by configured raw-conversation retention, session expiry, current user, and current property access. The desktop rail and mobile drawer append older pages and preserve loaded rows on a page-fetch failure. A separate authenticated POST searches retained session titles with the query in the request body, not a URL. The rail now offers “This home” and “All homes”; the latter enumerates current ownership/household membership, includes only property-scoped sessions whose stored executions all belong to accessible homes, and displays each property's label. Cross-home selection rechecks session access and reloads Ask under that home's URL context instead of silently retargeting the active canvas. This deliberately excludes unscoped/deleted-property sessions until their provenance is distinguishable, and does not resolve legacy sessions that span multiple *authorized* homes. Indexed user-visible content search, a single-property session invariant, privacy-safe indexing, lifecycle controls, and browser acceptance execution remain required before IW-HIST-002–018 or Phase 1 can be claimed complete. |
