...
| Contextual information panel | Foundations implemented for `EVIDENCE`, `ASSUMPTIONS`, `LIMITATION`, exact claim-to-source mapping, and one authoritative `OUTPUT_ARTIFACTS` producer; focused contract/component tests and static verification passed | Current response context collapses into one execution-scoped control rather than separate evidence, assumption, and artifact cards. It opens beside the conversation as a right contextual panel when sufficient desktop width is available and as a dismissible right sheet on constrained/mobile layouts, while keeping the Ask route, conversation, and composer intact. The surface groups evidence provenance, assumptions, limitations, and schema-validated output artifacts. Ownership-cost evidence carries server-declared mappings to exact table row identities; dangling block or item references are rejected. Confirmed maintenance-task creation now declares the exact canonical task ID, type, relationship, status, creation time, and optional traditional-navigation target as an output artifact instead of asking the client to infer a record from workflow copy or a URL. Completion status and task details remain in the conversation; only the optional canonical-record transition is progressively disclosed in context. Open panel identity remains bounded per session/property, and archived or superseded responses retain inline disclosures. Related-record context and broader governed output-artifact producers remain required before IW-SHELL-006 can be claimed complete. Browser acceptance scenarios are present but were not executed in this environment. |
| Runtime verification | Not claimed | Browser acceptance scenarios exist for the full-window Ask shell and single desktop conversation sidebar, persistent desktop history, active-session URL state, browser-back restoration, the mobile drawer, adaptive table/card view switching, bounded comparison strip/show-all switching, desktop/mobile contextual sources/claim mappings/assumptions/limitations surfaces with persisted open identity and focus restoration, authoritative maintenance output-artifact context while completion state remains conversational, inline maintenance task detail, maintenance-detail access-loss redaction, inline maintenance creation, and full-scope maintenance paging without route navigation, but were not executed in this documentation/code review environment. |
