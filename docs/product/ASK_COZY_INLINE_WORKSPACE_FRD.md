...
| Contextual information panel | Foundations implemented for `EVIDENCE`, `ASSUMPTIONS`, `LIMITATION`, and exact claim-to-source mapping; focused contract/component tests and static verification passed | Current response context collapses into one execution-scoped Sources and context control rather than separate evidence and assumption cards. It opens beside the conversation as a right contextual panel when sufficient desktop width is available and as a dismissible right sheet on constrained/mobile layouts, while keeping the Ask route, conversation, and composer intact. The surface groups evidence provenance, assumptions, and limitations; required limitation warnings remain visible in the conversation so primary-answer cautions are never hidden behind an optional panel. Ownership-cost evidence now carries server-declared claim mappings to exact schema-validated table row identities; dangling block or item references are rejected by the response contract, and the client displays the mapped claim with its source without inferring relationships from labels or URLs. Open panel identity is persisted as bounded per-session/per-property UI state without storing response content, and explicit close clears that state and restores trigger focus. Superseded/original responses retain inline disclosures as a safe non-interactive fallback. Expansion to related records and workflow/output artifacts remains required before IW-SHELL-006 and the full contextual-surface requirement can be claimed complete. Browser acceptance scenarios are present but were not executed in this environment. |
| Runtime verification | Not claimed | Browser acceptance scenarios exist for persistent desktop history, active-session URL state, browser-back restoration, the mobile drawer, adaptive table/card view switching, bounded comparison strip/show-all switching, desktop/mobile contextual sources/claim mappings/assumptions/limitations surfaces with persisted open identity and focus restoration, inline maintenance task detail, maintenance-detail access-loss redaction, inline maintenance creation, and full-scope maintenance paging without route navigation, but were not executed in this documentation/code review environment. |
