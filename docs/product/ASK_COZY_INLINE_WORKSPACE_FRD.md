...
| Runtime verification | **Executed and updated September 22, 2026** — the full `e2e/ask` Playwright suite (mocked Ask acceptance API, never a real backend/DB) was run in this session, not just claimed to exist; household collection/detail coverage added the same day. | `npx playwright test --config=playwright.ask.config.ts` (both `desktop-chromium` and `mobile-chromium` projects, against a fresh production build) — **64 passed, 0 failed** (56 in `ask.spec.ts` + 5 in `ask.mobile.spec.ts` + 3 in `decisionThreadAccessibility.spec.ts` = 64, cross-checked against the run's own reported total, not just its exit code; two new household scenarios added and mutation-checked — see the "Property Summary inventory and household inline collection/detail" row). This covers, by actual test title (not paraphrase): the full-window Ask shell and desktop conversation sidebar/rail (history paging, search, all-home switching, new/restored conversations, draft continuity), the mobile drawer, adaptive table view switching, bounded-comparison strip/show-all switching on desktop and mobile, desktop/mobile response-context (sources) panels, authoritative maintenance and quote-workspace output-artifact context, the document-to-home-event related-record surface, inline maintenance task detail/access-loss redaction/creation/paging, inline Property Summary timeline-event/room/document/**household** detail (including the household-specific "no longer a member" data-absence state), the accessibility checks for `DECISION_PROGRESS`/`WHY_NOW`/`SCENARIO_COMPARISON`, and every correction/add/completeness-capture scenario listed in the "Property records: correction browser acceptance" row. **Inline inventory item detail** is exercised only indirectly, as the entry point several correction tests open ("Systems and inventory" block, e.g. install-date/condition/purchase-cost corrections) — there is no dedicated "open inventory detail" scenario the way rooms/documents/timeline events/household members each have one. **Confirmed still NOT covered by any scenario in the current spec files** (grepped, not assumed): the inventory-detail item-not-found-vs-access-loss distinction; the inline inventory-disambiguation-list detail; and, per its own row, evidence upload. **What this run does and does not prove:** it verifies the frontend against fixture responses on a fresh build — rendering, dispatch, request bodies — exactly as the other browser-acceptance rows already state; it proves nothing about the server's real role gating, freshness checks, or writes, and nothing has run against a real backend or database. |
