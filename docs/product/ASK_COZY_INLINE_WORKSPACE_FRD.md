...
| Runtime verification | Not claimed | Browser acceptance scenarios exist for persistent desktop history, active-session URL state, browser-back restoration, the mobile drawer, inline maintenance task detail, inline maintenance creation, and full-scope maintenance paging without route navigation, but were not executed in this documentation/code review environment. |
