...
| History data contract | Selected-home pagination and title search implemented in code; focused contract/component tests and static verification passed | The authorized recent-session endpoint returns up to 20 conversations at a time with a validated opaque keyset cursor, ordered by activity and bounded by the configured raw-conversation retention window (30 days by default), session expiry, current user, and selected-property access. The desktop rail and mobile drawer append older pages and preserve loaded rows on a page-fetch failure. A separate authenticated POST searches retained session titles across pages for the selected home; the query travels in the request body, not a URL, and search results are separately paginated. The UI labels this as title search, not conversation-content search. Indexed user-visible content search, all-home scope, privacy-safe indexing, lifecycle controls, and browser acceptance execution remain required before IW-HIST-002–018 or Phase 1 can be claimed complete. |
