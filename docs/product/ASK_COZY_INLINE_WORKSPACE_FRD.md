...
| Runtime verification | Not claimed | Browser acceptance scenarios exist for persistent desktop history, active-session URL state, browser-back restoration, the mobile drawer, adaptive table/card view switching without a new request, inline maintenance task detail, maintenance-detail access-loss redaction, inline maintenance creation, and full-scope maintenance paging without route navigation, but were not executed in this documentation/code review environment. |
