...
| Property records — remaining write/correction gap (Phase 3) | **Open; Phase 3 exit criterion unmet** | Every Property Records inline slice above is read-only (OPEN_INLINE_ENTITY, not MUTATE_RECORD). Per entity: **Inventory item** — first write slice implemented for the three date fields only (see the "Property records: Inventory date corrections" row); condition, brand, model, serial, costs, room, notes remain uncorrectable inline. **Home event (timeline)** — `CAPTURE_EVENT_CONFIRM` has a correction path (`updateHomeEvent`, `ask-correction:{executionId}` guard) but is reachable only from extraction-created candidates; the detail view offers no correction entry. **Warranty** — `capture.warranty.confirm` is an unrouted stub; no renew/edit/delete. **Document** — `CAPTURE_EVIDENCE_CONFIRM` attaches evidence only; no metadata correction. **Room** — no operation; needs a domain-contract scoping pass. **Household member** — role/removal changes are governed by household authorization policy; must be classified as a §9 exception or scoped separately, not treated as a plain record correction. The four `capture.*.confirm` handlers are registered as `captureNotDirectlyRoutableResult` stubs (`askOrchestrator.service.ts`); no detail surface can start them. Phase 3 must not be claimed complete until each entity is either delivered inline (typed action, confirmation, canonical writer, receipt, reconciliation) or recorded as a documented §9 exception. No browser acceptance exists for the inventory/household/warranty slices. |
