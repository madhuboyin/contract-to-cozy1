...
| History data contract | Selected-home pagination implemented in code; focused contract/component tests and static verification passed | The authorized recent-session endpoint now returns up to 20 conversations at a time with a validated opaque keyset cursor, ordered by activity and bounded by the configured raw-conversation retention window (30 days by default), session expiry, current user, and selected-property access. The desktop rail and mobile drawer append older pages with an explicit control, preserve loaded rows on a page-fetch failure, and group records older than seven days separately. The text box is explicitly labeled as a filter of loaded conversations, not full-history search. Server-side title/content search, all-home scope, privacy-safe indexing, lifecycle controls, and browser acceptance execution remain required before IW-HIST-002–018 or Phase 1 can be claimed complete. |
