...
| Runtime verification | **Executed and updated September 22, 2026** — the full `e2e/ask` Playwright suite (mocked Ask acceptance API, never a real backend/DB) was run in this session, not just claimed to exist; household, inventory not-found/access-loss/disambiguation, then evidence-upload coverage added across the day. **Every Property Records browser-coverage gap named in earlier versions of this row is now closed.** | `npx playwright test --config=playwright.ask.config.ts` (both `desktop-chromium` and `mobile-chromium` projects, against a fresh production build) — **69 passed, 0 failed** (61 in `ask.spec.ts` + 5 in `ask.mobile.spec.ts` + 3 in `decisionThreadAccessibility.spec.ts` = 69, cross-checked against the run's own reported total, not just its exit code; seven new scenarios added across three rounds, each mutation-checked against a real rebuild — see the "Property Summary inventory and household inline collection/detail", "Inventory inline detail", and "Evidence upload" rows). This covers, by actual test title (not paraphrase): the full-window Ask shell and desktop conversation sidebar/rail (history paging, search, all-home switching, new/restored conversations, draft continuity), the mobile drawer, adaptive table view switching, bounded-comparison strip/show-all switching on desktop and mobile, desktop/mobile response-context (sources) panels, authoritative maintenance and quote-workspace output-artifact context, the document-to-home-event related-record surface, inline maintenance task detail/access-loss redaction/creation/paging, inline Property Summary timeline-event/room/document/**household** detail (including the household-specific "no longer a member" data-absence state), inline inventory item detail's **item-not-found-vs-access-loss distinction** and the **inventory-entity-selection disambiguation list's** inline detail, **inline evidence upload** (out-of-band file upload via `setInputFiles`, exact `launchContext` identity, a zero-editable-field confirmation, and client-side file-type rejection), the accessibility checks for `DECISION_PROGRESS`/`WHY_NOW`/`SCENARIO_COMPARISON`, and every correction/add/completeness-capture scenario listed in the "Property records: correction browser acceptance" row. **Confirmed still NOT covered by any scenario in the current spec files** (grepped, not assumed): nothing left within Property Records; more broadly, no scenario anywhere in this suite runs against a real backend or database, and Phase 3's non-entity gaps (adaptive presentation, the capability-card audit) remain their own separate rows, untouched by this work. **What this run does and does not prove:** it verifies the frontend against fixture responses on a fresh build — rendering, dispatch, request bodies — exactly as the other browser-acceptance rows already state; it proves nothing about the server's real role gating, freshness checks, or writes. |
