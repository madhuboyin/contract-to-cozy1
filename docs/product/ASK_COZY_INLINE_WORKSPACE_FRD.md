...
| Adaptive presentation runtime | Foundation implemented for `TABLE`; focused component/state tests and static verification passed | Existing schema-validated `TABLE` blocks now use a dedicated renderer selected by the validated type discriminator, with a deterministic responsive default: comparison-fit multi-row tables render as tables at wider widths and as fully labeled cards on narrow widths, while single-record or single-dimension results avoid a table that adds no comparison value. When both modes are useful, an Auto/Cards/Table switch changes only the current view, preserves rows, labels, values, total/partial-result meaning, and block actions, and persists as bounded per-result session state. A general component registry plus broader resolver coverage for grouped lists, comparisons, timelines, accessibility preferences, and safe cross-block fallback remain required before IW-PRES-001–012 can be claimed complete. A browser acceptance scenario is present but was not executed in this environment. |
...
| Contextual information panel | Foundation implemented for `EVIDENCE`; focused component tests and static verification passed | Current response evidence now renders as a compact, execution-scoped Sources control. It opens beside the conversation as a right contextual panel when sufficient desktop width is available and as a dismissible right sheet on constrained/mobile layouts, while keeping the Ask route and conversation state intact. The surface preserves evidence-group titles, item labels, source names, observation dates, property scope, incomplete-provenance disclosure, keyboard focus entry/return, and the originating response question. Superseded/original responses retain the prior inline disclosure as a safe non-interactive fallback. Expansion to assumptions, limitations, related records, workflow/output artifacts, exact claim-to-source mapping, and persisted panel restoration remains required before IW-SHELL-006 and the full contextual-surface requirement can be claimed complete. Browser acceptance scenarios are present but were not executed in this environment. |
| Runtime verification | Not claimed | Browser acceptance scenarios exist for persistent desktop history, active-session URL state, browser-back restoration, the mobile drawer, adaptive table/card view switching without a new request, desktop/mobile contextual evidence surfaces with focus restoration, inline maintenance task detail, maintenance-detail access-loss redaction, inline maintenance creation, and full-scope maintenance paging without route navigation, but were not executed in this documentation/code review environment. |
