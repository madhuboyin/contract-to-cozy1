...
**Proposed, NOT built** (surfaced by this round's questions, out of scope for the fixes above):
- **"Add a warranty" suggestion for aging/uncovered appliances.** User's original question implied this should already exist; it doesn't — `projectTransferableWarranties` only surfaces *existing* active warranties as a positive highlight, nothing checks "this item is old and has no `warrantyId`, suggest adding coverage." Would need a new Tier 1(-ish) projector: for `InventoryItem`s already flagged by `projectAgingSystems` (or simply past some materiality bar) with `warrantyId: null`, emit a `PRESENTATION`/`OPTIONAL_IMPROVEMENT` item suggesting a warranty. Reasonably small addition given `projectAgingSystems` and the warranty-add flow (`/dashboard/warranties?action=new&propertyId=&inventoryItemId=`) already exist — mostly a matter of wiring, not new infrastructure. Not started; needs explicit scoping before building (e.g. should it apply to *any* aging item, or only appliances specifically?).
- **Self-reported answer severity should tailor checklist content, not just gate whether an item shows.** Today, `projectSelfReportedConditions` renders the exact same static `SALE_PREP_VALUE_CATALOG` detail text regardless of which "needs attention" value was picked (`NEEDS_REFRESH` vs `NEEDS_WORK` vs `NEEDS_SOME_WORK` vs `NEEDS_SIGNIFICANT_WORK` all produce identical content) — the answer only decides whether an item appears at all, not what it says. Making the content genuinely responsive to severity (e.g. "needs significant work" warranting stronger framing or a different priority/requirementClass than "some wear") would need either several severity-specific catalog variants per category or a lightweight content-templating layer — a real design/content decision, not a quick fix, and not started.
...
**This closes out the value-aware checklist redesign's originally-scoped 3 stages.** Not built, and intentionally out of scope for this redesign: the "add a warranty for aging/uncovered appliances" and "self-report severity tailors content" proposals from §11 remain proposed-only.
