...
| Audience presentation | Canonical Skill results pass through a bounded post-execution presentation policy that adds lifecycle framing and filters unusable mutation CTAs by live household role. Unknown-context results offer contributors/owners a canonical property-journey correction route while viewers remain read-only. It preserves canonical facts, calculations, evidence, rankings, result status, authorization, and confirmation requirements. |
