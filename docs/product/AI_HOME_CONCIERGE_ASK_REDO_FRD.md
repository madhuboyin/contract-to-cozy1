...
| Phase 2 — Inline capture | Repository closure implemented across repair/replace, refinance, savings, ownership costs, sell/hold/rent, inventory, property summary, Home Actions, and coverage | Apply the user-managed inventory date-precision schema change, retain desktop/mobile acceptance evidence, and certify ≥99% resume success plus ≤1% repeated-prompt rate on a material production-like sample |
| Phase 3 — Capability discovery | Repository closure implemented | Retain desktop/mobile acceptance evidence and certify top-1 relevance plus unavailable-tool honesty against a material production-like homeowner-language sample before launch |
| Phase 4 — Confirmed actions and monitors | Repository closure implemented | Apply the user-managed command-idempotency schema changes (`GuidanceJourney.sourceAskExecutionId` and the unique pre-mutation Ask confirmation claim), retain database-backed retry/crash-recovery/concurrency evidence and desktop/mobile acceptance evidence, and complete domain-owner/notification-policy launch sign-off |
...
Required classifications are executable policy, not display metadata. `REQUIRED_SAFETY` is selected before other missing facts and must block rather than accept an unresolved value. `REQUIRED_APPLICABILITY` must be resolved before Ask claims that an operation applies. `REQUIRED_CALCULATION` must be resolved before a personalized material calculation. All three disable “Not sure”; only `ENHANCEMENT_ACCURACY` may continue with a disclosed general estimate.
...
**Implementation status — August 11, 2026:** The evaluator now enforces required-class ordering and disables unresolved bypasses for all `REQUIRED_*` classes. The shared Ask capture card presents distinct safety, applicability, and calculation consequences. Enhancement capture remains explicitly optional. Inline values remain device-local drafts; financial/security consent and command authorization consent are intentionally not persisted.
...
**Implementation status — August 11, 2026:** Appendix B structural-work/inspection evasion, unsupported insurance-coverage and sale-price determinations, cross-user/private-record access, document-borne exfiltration instructions, destructive SQL/database requests, and additional out-of-domain prompts are intercepted before remote generation. The verbatim catalog examples are regression-tested. The remote provider instruction independently refuses safety-control evasion, private-data access, arbitrary execution, and unsupported professional determinations as defense in depth; it is not the primary routing control.
...
**Implementation status — August 11, 2026:** `ASK_PROPERTY_REQUIRED`, `ASK_ENTITY_REQUIRED`, `ASK_OPERATION_UNSUPPORTED`, and `ASK_EXECUTION_EXPIRED` are emitted by live non-test execution paths. Entity/out-of-scope normalization preserves the operation-specific reason in execution parameters for diagnostics without making clients depend on domain-specific strings.
...
- exactly one database-enforced command claim per execution;
- idempotency key, input hash, confirmation version, and `CLAIMED`/`COMPLETED` lifecycle;
- claim is created in the same transaction that moves the execution from `NEEDS_CONFIRMATION` to `RUNNING`, before any domain mutation;
- a short lease prevents simultaneous side effects; after it expires, same-input recovery is allowed (including across devices), while conflicting input fails closed;
- artifact reference and completion timestamp are attached when orchestration finalizes.
...
**Implementation status — August 11, 2026:** Repository closure implemented. Ask now ranks homeowner goals through a canonical, deterministic capability goal matcher built from governed labels, intent aliases, outcomes, trigger families, and reviewed homeowner-language equivalence groups; this path does not require an LLM call. Candidate results are intersected with the live rollout/route/manifest availability adapter before presentation. Property-scoped results use the same readiness policy and property-context metrics as related capability recommendations, and the response contract/UI distinguish ready, needs-property, needs-context, beta, and unavailable states with actionable reasons. Workflow-only, disabled, rollout-blocked, stale-manifest, or broken-route capabilities are never presented as ordinary launch links. Ambiguous goals return bounded choices rather than silently selecting one. Canonical related-capability cards now continue both discovery results and eligible completed Ask answers. The Ask golden suite covers twelve synonymous homeowner goals, ambiguity, deterministic repeatability, routing breadth, top-1 quality, and adversarial negative prompts; desktop and mobile acceptance fixtures cover readiness, related-tool continuity, and honest unavailable-tool rendering. Production launch still requires retained browser-run evidence and measurement of top-1 relevance and unavailable-tool behavior over a material production-like query set and the deployed rollout configuration.
...
**Implementation status — August 11, 2026:** Repository closure implemented. Eight material command adapters are now registered in one governed command registry: maintenance create/complete/update, household invitation, guided-plan creation, quote-comparison creation, refinance-rate monitoring, and maintenance/home-deadline monitoring. The registry owns adapter identity, role floor, artifact type, pre-execution cancellation copy, and supported correction modes; confirmation and cancellation reject any unregistered command. Owner/contributor/viewer floors are enforced both before preparation and again at confirmation. Every confirmation now acquires a unique database-enforced execution claim and moves the execution to `RUNNING` before domain mutation. Same-key/same-input retries can recover a claim; competing confirmations fail closed; completion attaches the artifact to the claim. Domain-level stable keys still provide post-write recovery, including Maintenance completion metadata, guided-plan `sourceAskExecutionId`, stable Maintenance action keys, and serializable quote-workspace get-or-create. Maintenance updates provide reschedule, priority, assignment, unassignment, archive, and reopen paths. A dated Maintenance task can opt into scoped email delivery without creating a duplicate task; expiration monitoring creates a stable action-keyed Maintenance obligation consumed by the same reminder worker and writes scoped email preference state only after explicit consent. When an insurance policy exists without an expiration date, Ask captures it inline, saves it canonically, and resumes into confirmation. Completion cards link to the owning artifact and use the domain workspace for subsequent correction/revocation/dismissal; refinance retains edit/pause/resume/stop controls, while maintenance-backed reminders use edit/archive/reopen. No migration script is included. Production closure still requires the user-managed schema application, retained database concurrency/crash-recovery evidence, desktop/mobile E2E evidence, and domain/notification-policy approval.
...
**Implementation status — August 11, 2026:** Repository closure implemented. Phase 5 now includes deterministic, property-scoped adapters for sell/hold/rent, ownership costs, quote-comparison review, generic repair/replace, capital timeline and reserve planning, property-tax appeal readiness, insurance/coverage review, renovation/permit readiness, and major-event entry. Repair/replace resolves the requested canonical inventory item across appliance and system categories and calls `ReplaceRepairService` rather than using refrigerator-only lifespan copy. Quote review uses homeowner-confirmed proposal readiness and canonical comparability without selecting a provider. Capital planning uses Home Capital Timeline plus Reserve Fund outputs and supports minimum-path Property Context capture. Tax readiness uses reviewed jurisdiction rules, canonical facts, qualified evidence, and sourced tax-at-stake calculations without predicting success. Renovation readiness reads current-scope blockers and Permit Tracker state without treating the app as an authority. Major-event entry is filtered through the live capability registry and creates or shares nothing automatically. Every new decision operation is deterministic, viewer-readable, property-authorized, independently kill-switchable, bounded by typed presentation blocks, and covered by Phase 5 golden routing, canonical-adapter, professional-boundary, emergency, and negative-prompt contracts. No database schema change or migration is required by this slice. Production closure still requires retained database-backed outcome fixtures, desktop/mobile evidence, measured decision accuracy/latency, and relevant domain-owner approval.

**Reviewed-rule coverage decision — August 11, 2026:** The Bronx/NYC Class 1 profile remains the only activated property-tax rule profile. A second profile was not fabricated merely to increase the count. Cook County is a candidate for the next governed release, but official material establishes township-specific filing periods, multiple appeal stages, and evidence rules that require a matching official-source adapter and reviewed calendar rather than one county-wide fixed deadline. Activation therefore requires: official parcel/class matching; Assessor versus Board of Review stage modeling; township deadline ingestion and expiry controls; reviewed residential class applicability; form/rule citations; and domain-owner sign-off. Until then, Cook County and every other uncovered jurisdiction must return the honest unavailable state. Official review inputs: [Cook County Assessor residential appeals](https://www.cookcountyassessor.com/residential-appeals), [official appeal rules](https://www.cookcountyassessor.com/official-appeal-rules-cook-county-assessor), and [Cook County Board of Review residential appeals](https://www.cookcountyboardofreview.com/residential-appeals).
...
**Implementation status — August 11, 2026:** The requested Phase 6 repository slice is implemented. Safety and negative-prompt interception remains the first deterministic stage. Existing high-confidence registered rules remain second. Previously unmatched wording is evaluated by a bounded, in-process lexical classifier with configurable confidence and ambiguity thresholds; confident matches route to the canonical deterministic operation, close competing matches return a typed clarification, and only unresolved questions reach remote grounded guidance. `ASK_LOCAL_ROUTING_ENABLED` independently rolls this stage back without disabling deterministic routing or Ask. Before grounded generation, the question is normalized, direct email/phone/link/account identifiers are redacted, the question is capped at 600 characters, and the property snapshot is reduced to at most 20 lexically relevant facts within a 6,000-character budget; a zero-match question sends no complete-record fallback. Optional `ASK_RESULT_SYNTHESIS_ENABLED` synthesis is off by default and applies only to eligible deterministic answered results. It receives a schema-validated, identifier/action/link-redacted result DTO rather than the homeowner question, property context, raw database records, or command payload. Boundary, monitor, and workflow blocks are ineligible; canonical structured blocks remain authoritative; unsupported numeric claims reject the synthesis; and any provider/schema failure falls back to the original deterministic result. Routing-stage and synthesis-outcome metrics support containment monitoring. Phase 6 is not fully closed: the versioned offline benchmark, pluggable local-model serving proof of concept, shadow comparison, accuracy/latency/privacy/TCO report, regression thresholds, and production adoption decision remain outstanding.
