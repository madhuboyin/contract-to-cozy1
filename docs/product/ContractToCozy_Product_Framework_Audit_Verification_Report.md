...
**Implementation follow-up — July 24, 2026:** The selected findings are now closed. Global Protect, Save, Documents, Maintenance, and Inventory pages are compatibility redirects to property-scoped destinations; Protect and Save resolve into the existing property-hub tabs; Documents and Maintenance retain their detailed workflows under property-scoped routes; and the legacy hubs no longer appear as Cmd+K shortcuts. Route QA now accepts only explicit redirect primitives for `REDIRECT_DUPLICATE` pages, rather than treating any `router.replace` call as proof. Weekly Home Brief notifications persist/derive canonical attention priority and render ordered Soon, Plan, and Consider sections with weekly-specific copy. Live task orchestration now branches on orthogonal operating modes and reports `BUYER_PLAN`/`RECURRING_CARE` task sources instead of permanent homeowner segment labels. The orphaned household notification reader, `notifyOn*` storage, endpoint, client contract, and parallel UI controls were removed; Household now links to the canonical notification preference page. The original findings below are retained as the audit record.
...
| Route-disposition inventory, CI-enforced | CONFIRMED | `apps/frontend/scripts/product-framework/check-route-disposition.mjs`. Live run: **219 routes classified, PASSED** (README claims 210 — stale count). Wired into `frontend-quality-gates.yml` → `npm run qa:gates`, a real blocking CI gate. |
...
4. **RESOLVED July 24, 2026 — Phase 1 permanent segmentation.** Live orchestration now branches on `PURCHASE`/`EXPLORATION`/`OWNERSHIP` operating modes and uses workflow-oriented `BUYER_PLAN`/`RECURRING_CARE` task sources. Stored telemetry and feature names remain historical taxonomy, not eligibility branches.
5. **RESOLVED July 24, 2026 — Phase 4 legacy household notification reader.** The dead reader and parallel `HouseholdMember.notifyOn*` storage/API/UI contract were removed; Household routes users to the canonical notification preferences.
...
4. **Completed July 24, 2026:** Permanent segment labels were removed from live task-routing decisions.
5. **Completed July 24, 2026:** The dead household notification reader and parallel preference storage/API/UI were removed.
6. Add an "Implementation status" line to Phase 5's section in the main audit doc, and do a pass updating §4/§7 gap-register rows to reflect actual phase completion so the document stops contradicting itself.
