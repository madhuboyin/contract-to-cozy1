...
**Implementation status:** Phase 1 code implementation completed July 18, 2026. Trigger selection, orthogonal entry-context persistence, authenticated context and first-value APIs, canonical evidence-bounded actions, manual lookup fallback, multimodal trigger evidence, adapter-specific handoff, bounded 12-month planning, outcome/feedback lineage, pilot reporting, and complete removal of permanent homeowner segmentation are documented in `docs/product/phase1/README.md`. Human approvals do not block internal testing; they become real-user launch gates through `ENFORCE_HUMAN_POLICY_APPROVALS=true`. Owner-applied schema verification and real-pilot target evaluation remain operational activities.
