...
- 219 frontend page routes, including property-scoped compatibility and contextual routes;
...
**Implementation status:** Technical exit criteria completed July 18, 2026. Executable contracts, all declared source adapters, typed runtime lineage, event-backed north-star aggregation, golden fixtures, route-disposition enforcement, feature-brief requirements, and role-based recommendation launch gates are documented in `docs/product/phase0/`. Because there are no real users, human attestations are advisory and non-blocking in internal beta. They become hard gates when `ENFORCE_HUMAN_POLICY_APPROVALS=true` before real-user launch; technical safety contracts remain mandatory in both modes. See `docs/product/governance-modes.md`.
...
**Implementation status:** Phase 1 code implementation completed July 18, 2026, with segmentation closure on July 24, 2026. Trigger selection, orthogonal entry-context persistence, authenticated context and first-value APIs, canonical evidence-bounded actions, manual lookup fallback, multimodal trigger evidence, adapter-specific handoff, bounded 12-month planning, outcome/feedback lineage, and pilot reporting are documented in `docs/product/phase1/README.md`. Live task routing now uses `PURCHASE`/`EXPLORATION`/`OWNERSHIP` operating modes and workflow-oriented task sources instead of permanent homeowner segment labels. Human approvals do not block internal testing; they become real-user launch gates through `ENFORCE_HUMAN_POLICY_APPROVALS=true`. Owner-applied schema verification and real-pilot target evaluation remain operational activities.
...
**Implementation status:** Started July 18, 2026. Increments 1–3 implement the canonical property-scoped action feed, deterministic ranking and cross-source deduplication, safe lifecycle commands, dismissal suppression, household authorization, a shared responsive five-section Home contract/surface, five-destination homeowner navigation, grounded Ask, full action-lineage semantics, frontend client support, promotion of active guidance, incident, recall, coverage, and project sources, lifecycle suppression across promoted sources, guidance-template route cutover, and living canonical CTA, journey, and static notification route contracts. Post-cutover hardening adds a property-scoped Prioritized Action Plan, consistent summary/detail counts, homeowner-readable action labels, duplicate context-action suppression, grouped seasonal checklist promotion, and distinct critical-weather presentation with source, instructions, expiry, safety lifecycle rules, and duplicate incident-guidance suppression. The Phase 2/Phase 4 convergence increment replaces Home's parallel completeness calculation with canonical Property Context completeness/versioning and promotes technically eligible, governed Personalization recommendations into the same ranked, deduplicated, lifecycle-aware Home feed. Navigation closure on July 24, 2026 converts the global Protect, Save, Documents, and Maintenance pages into compatibility redirects, places Protect and Save inside the property Home Record tabs, scopes Documents and Maintenance to property routes, removes the legacy hubs from Cmd+K shortcuts, and makes route CI reject a `REDIRECT_DUPLICATE` page without redirect behavior. First-value environment closure on July 24, 2026 enforces priority buckets before score, separates official critical alerts from computed Environment Action cards, adds a nullable grounded outlook to Unified Home, makes setup secondary to useful evidence, classifies `NONE_EXPLORING` as `CONSIDER`, and incorporates the property area’s immediately preceding seven weather days into season-aware heat/rain interpretation without adding a provider request. The target Prisma schema has been applied. With no separate test/dev/stage database during beta, remaining acceptance is operational smoke and pre-launch evidence rather than a migration requirement. See `docs/product/phase2/README.md`.
...
The Home hierarchy follows these additional presentation rules:
...
- canonical `NOW` and `SOON` actions always precede passive first-value content and lower-priority work;
- official critical alerts, computed action-level environment risks, watch insights, verified quiet outlooks, and setup guidance use distinct descending treatments; and
- when no high-priority action exists, Home leads with grounded area/season evidence before asking the homeowner for more setup data.

Property Context and Personalization are not parallel homeowner surfaces. Property Context is the versioned intelligence substrate for Home completeness, applicability, evidence, correction, and staleness. Reviewed Personalization is one governed producer of canonical Home Actions. It may retain dedicated administration, consent, and explanation surfaces, but eligible recommendations must participate in the default Home ranking, deduplication, lifecycle, and lineage contracts.
...
- A newly created exploratory property receives a grounded area-specific outlook when evidence is available; pleasant conditions produce a verified bounded result rather than a fabricated threat.
- Recent weather dates are treated as history, weather claims remain threshold-gated, and out-of-season hazards require an actual observed or forecast threshold crossing.
...
**Implementation status:** Increments 1–3 implemented July 18, 2026; code-complete pending owner-applied database acceptance. The canonical asset-lifecycle journey spans stages 9–14 with typed lineage and adaptive execution context. Verified closure uses an idempotent serializable transaction to persist explicit verification/outcome state and update HomeEvent, expense, uploaded proof, inventory history, warranty, material specifications, inspection findings, provider review evidence, and future-care actions. Minor work can close without a full project. Guided provider work records fit rationale, commercial disclosure, non-commercial alternatives, jurisdiction/category credential readiness, and platform eligibility. Approved provider reviews carry project, journey, scope, schedule, cost, and verified-outcome context. Follow-up tasks record health and create remediation when work needs attention or fails. Analytics cover elapsed/blocked time, price variance, recommendation override, provider result, write-back volume, follow-up plan, and follow-up health. A gated read-only database acceptance harness is included for execution after the owner applies the schema. No migration script is included. See `docs/product/phase3/README.md`.
...
**Implementation status:** Implementation-complete through Increment 8 and its convergence closure hardening on July 20, 2026; owner-applied database acceptance and pilot outcome validation remain. Recommendation definitions persist typed trust tiers and policy versions; the reviewed catalog carries schema-validated professional boundaries and safety fallbacks; and MFA-protected role attestations form an auditable review queue. Missing human attestations are advisory in internal beta and block activation when `ENFORCE_HUMAN_POLICY_APPROVALS=true` before real-user launch. Every canonical Home Action producer receives the shared response contract and degraded material CTAs are withheld centrally. Recommendation feedback and operator intake feed an audited incident lifecycle with automatic high-risk definition pausing and aggregate quality reporting. All notification producers route through a canonical preference policy with scoped category/cadence controls, quiet hours, immediate-versus-weekly routing, homeowner feedback controls, and quality outcomes; pilot configuration is limited to the implemented In-App and Email paths. Weekly Home Brief closure on July 24, 2026 persists a canonical attention priority and renders weekly notifications in ordered Soon, Plan, and Consider groups while retaining a separate flat daily digest. Notification convergence closure on July 24, 2026 removes the orphaned household notification reader and the parallel `HouseholdMember.notifyOn*` storage/API/UI path; Household settings now link to the canonical notification preference surface. Grounded Ask isolates sessions by user/property/context version, returns citation-bounded Living Home Record evidence and limitations, and executes all schema-validated proposal kinds only after confirmation with durable audit linkage. Unified Home now uses canonical Property Context completeness/version lineage, materializes reviewed personalization through the same context, promotes it through the governed Home Action pipeline, and synchronizes canonical feedback, suppression, snooze, terminal orchestration state, and the underlying personalized recommendation in one transaction. Repeated snoozes remain individually traceable and fail-closed materialization status is observable without exposing optional profile data. A gated Phase 4 database acceptance harness is included. No migration script is included. See `docs/product/phase4/README.md`.
...
**Implementation status:** Code-complete July 19, 2026. The specialized path includes automated qualification plus auditable operator cohort admission, a property-scoped plan, punch-list collaboration and verified closure, source-cited warranty rights with worker-driven deadline monitoring, inventory registration, classified commissioning evidence, inspection preparation bundles, first-year recurring handoff and Living Home Record write-back, all six expansion-gate measures, consolidated route classification, complete homeowner/admin workflows, and an opt-in database acceptance harness. The gate remains evidence-bound: without actual pilot and repair-journey outcomes it reports insufficient evidence rather than claiming readiness. The updated Prisma schema must be applied by the repository owner; no migration script is included. See `docs/product/phase6/README.md`.
