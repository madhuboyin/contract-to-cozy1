...
**Implementation status:** Started July 18, 2026. Increment 1 extends the canonical asset-lifecycle journey through stages 9–14, adds typed guidance/inventory project lineage, preserves recommendation version, captures repair/replacement, provider/DIY, funding, and complexity context, validates property-scoped source relationships, advances scope confirmation when the linked project is created, and exposes the adaptive handoff in the project UI. Transactional proof/write-back, future-care automation, commercial-integrity completion, outcome analytics, and database-backed acceptance remain in progress. No migration script is included. See `docs/product/phase3/README.md`.
