...
| TA2 — Hybrid semantic operation retrieval | Implemented | Versioned, cached local subword embeddings are fused with typo-tolerant lexical and phrase similarity over the runtime-, authorization-, household-role-, and lifecycle-discoverable operation catalog; the independent embedding control degrades to lexical-only retrieval |
| TA3 — Constrained classification and calibration | Implemented | Candidate-only classification uses versioned per-language, retrieval-path, operation-effect, and materiality calibration profiles, stricter write/material thresholds, short-query ambiguity handling, bounded calibration provenance, and per-operation quality/reliability reporting |
