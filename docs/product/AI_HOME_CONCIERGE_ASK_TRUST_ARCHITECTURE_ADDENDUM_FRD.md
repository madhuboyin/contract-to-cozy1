...
| TA5 — Semantic answer relevance | Implemented | Default-on, independently controlled semantic relevance validation consumes a bounded homeowner-visible projection of all typed response blocks, requires every registered operation to own non-empty answer positives and answer hard negatives separately from routing examples, records bounded scores/reasons and latency, distinguishes mismatch from uncertainty, and certifies zero passes across the complete cross-operation negative matrix while preserving all positive and seasonal rich-response answers |
...
| TA7 — Production-quality learning loop | Implemented | Protected admin trust analytics aggregate bounded event metadata only; the admin product surface can synchronize hashed failure clusters into durable review candidates, capture de-identified representative wording and expected-operation labels, approve/reject, explicitly promote, and export a versioned regression corpus or governed calibration proposal; raw conversation text is not copied, active calibration is never mutated automatically, and threshold recommendations remain advisory |
...
The functional-gap closure pass added the missing TA7 administration surface for candidate synchronization, filtering, de-identified labeling, approval/rejection, explicit promotion, regression-corpus export, and calibration-proposal export. The proposal derivation combines the active labeled evaluation rows with only explicitly promoted production corrections, publishes traceable expected/competitor observations and empirical profiles, and cannot activate or mutate runtime calibration without code review and release. The pass also fixed the disclosed capital-reserve and general home-deadline routing failures and made non-empty answer hard negatives mandatory for all 39 operations. It requires the previously declared `AskTrustReviewCandidate` schema to be applied, but adds no new database model and no migration script.
