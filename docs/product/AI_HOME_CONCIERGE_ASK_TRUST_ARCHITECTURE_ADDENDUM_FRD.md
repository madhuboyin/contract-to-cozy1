...
| TA5 — Semantic answer relevance | Implemented | Default-on, independently controlled concept/semantic relevance validation runs after deterministic final-presentation validation, records bounded scores/reasons and latency, prevents unknown relevance from being counted as verified coverage, applies operation-owned answer hard negatives to confusable operation pairs, and certifies zero passes across the complete cross-operation negative matrix while preserving all positive direct answers |
