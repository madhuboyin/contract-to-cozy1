...
- The current buyer checklist is a flat, small default task list rather than
  conditional phase guidance; frontend default detection depends on English
  titles that do not match the backend's stable action-key templates.
- There is no dedicated contract/contingency, purchase-financing,
  title/escrow, final-walkthrough, Closing Disclosure/cash-to-close, or
  closing-day tool.
- The general tool catalogs are owner-oriented and do not express pre-close
  eligibility, buyer mode, workflow-only placement, or suppression policy.
- Existing refinance Loan Estimate comparison primitives are useful but are
  coupled to refinance framing, models, and conclusions and cannot be exposed as
  purchase financing without a separate product contract.
...
- no phase may show dozens of equal-priority items at once: show **Next**,
  **Blocking**, and **Coming up**, with the complete checklist expandable;
...
| Slice 8 | In progress | Removed the orphaned global buyer-checklist card, route, and framework redirect; removed the associated duplicate dashboard checklist types; removed a tracked obsolete pre-MFA login-page copy with stale role destinations; corrected active API, Swagger, and integration comments that described buyer or owner as a global user segment; expanded the required source-level route/CTA contract to cover onboarding, authentication artifacts, framework redirects, Buyer Closing Home, Buyer Plan, Documents, Inspection, Ask, Recent Owner handoff, Home Record, Household, and Home Operations; retired the duplicate Moving completion column, persisted JSON completion flags, bulk completion service/route/client, and wired the rich Moving view directly to canonical property-scoped buyer-task status updates | Complete the rendered full-journey and protected homeowner traversals; finish site-wide copy/link, legacy helper, responsive, accessibility, and empty/error-state audits |
...
produce benefit-explained correction questions. Applying a preview persists
rule/version/reason/fact/basis provenance and preserves completion, evidence,
assignment, notes, status decisions, and user-edited dates. The Contract &
Contingency Tracker and rendered checklist customization remain next.
