...
12. Surface confirmation via the existing `confirmation` field on each child execution, delivered
    via the new bounded `childExecutions` array on the turn's own response (§14/§28 — the
    confirmation-card mechanism itself needs no new block; the response contract carries the
    child execution(s) that own it)
...
| `UPLOAD_EVIDENCE` | Links an already-uploaded `Document` as evidence for another proposal | **Resolved (Phase 0, implementation plan §4.2):** evidence on a sibling `HomeEvent` capture candidate writes to `HomeEventEvidence` via the same bidirectional `linkedExecutionId` sibling-link mechanism §22 already specifies for Warranty/HomeEvent pairs — confirmed distinct from `DOCUMENT_PROMOTION_CONFIRM`'s target (canonical-record promotion from a pending-review row, never touching `Document`/evidence). Evidence on any other candidate kind has **no existing schema target** (`PropertyFactEvidence` has no `documentId` column) and is left explicitly open for Phase 3 to scope, not silently dropped. | Partial — common case resolved, remainder explicitly deferred, not assumed |
| `ADD_NOTE` | Creates a `GroundedAskArtifact` with the note text **only in `artifactJson`** — no queryable domain model today | **Resolved (Phase 0, implementation plan §4.2):** `HomeEvent` capture (§20) with `type: NOTE` — a precedented enum value already used in production code (`permitDetection.service.ts`, `claims.service.ts`), `occurredAt` defaulted to confirm time with `datePrecision: UNKNOWN`, `amount`/`summary` null. Requires a `propertyId` going forward, unlike today's property-optional note proposals — a deliberate narrowing Phase 3 must state in its own acceptance criteria. | Full — resolved, no new model |
...
- **Home Event Radar — expose now, but three things must happen first**, per Phase 0's resolution (implementation plan §4.6, §4.5):
  ...
  2. **[NEW — Phase 0]** the domain-list fix alone does not achieve this item's goal. `PropertyRadarMatch`/`PropertyRadarCompoundInsight` — the producers carrying the actual WEATHER-domain radar insights (`SEVERE_WEATHER_OPEN_ROOF_ISSUE`, `heavy_rain`, `flood_risk`, etc.) — never populate `entityRef` on their envelope items, so no Radar-sourced item can ever satisfy a component-scoped query regardless of the domain list. Their adapters need an `entityRef` (e.g. `{entityType: 'PROPERTY', entityId, componentKind}` where a radar rule maps to a specific component) before Scenario 8.4 works end-to-end. Scoped as a Phase 7 precondition.
  3. **[FACT — this pass]** runtime Radar population could not be verified in this environment (no reachable database matching the app's configured connection string) — but the seed script (`apps/backend/prisma/seed.ts`) was confirmed to create **zero** `PropertyRadarMatch`/`RadarEvent` rows for any seeded test property, reconfirmed this pass. Treat "Radar data exists for a demo/test property" as a precondition to establish (via the real ingestion pipeline or a purpose-built fixture), not an assumption — this is a real Phase 7 blocker for any live demonstration of Scenario 8.4, not just a documentation gap.
...
- **Home Renovation Advisor — naming ambiguity resolved, Phase 0 (implementation plan §4.10).** `RENOVATION_PERMIT_READINESS` calling `permitTracker`/`renovationCase` (not `homeRenovationAdvisor/`) is confirmed correct, not a wiring bug: the two are legitimately different features. `RENOVATION_PERMIT_READINESS` answers "is my already-started, tracked renovation case ready/blocked on permits?" against a governed `RenovationCase` and its readiness checklist. `homeRenovationAdvisor/` answers a different, earlier-stage question — "what jurisdictional permit/tax/licensing risk would a prospective project of type X carry?" — via an independent evaluation-session model not tied to any `RenovationCase`. No rewiring; `RENOVATION_PERMIT_READINESS` migrates as-is. `homeRenovationAdvisor/` stays unexposed via Ask; if a future phase exposes it, it gets its own distinct `operationId`, never reusing `RENOVATION_PERMIT_READINESS`'s.
