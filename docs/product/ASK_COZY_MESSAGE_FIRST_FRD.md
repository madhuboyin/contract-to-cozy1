...
| `UPLOAD_EVIDENCE` | Links an already-uploaded `Document` as evidence for another proposal | **Approved successor semantics:** evidence is accepted only with a same-batch sibling `HomeEvent` candidate and writes to `HomeEventEvidence`; there is intentionally no generic evidence attachment for fact/task/journey/quote candidates because those models have no canonical document-evidence target. | Intentional scope evolution — supported event evidence is covered; the legacy standalone reference is retired |
| `ADD_NOTE` | Creates a `GroundedAskArtifact` with the note text **only in `artifactJson`** — no queryable domain model today | **Approved successor semantics:** `HomeEvent` capture (§20) with `type: NOTE`, `occurredAt` defaulted to confirm time with `datePrecision: UNKNOWN`, and a required property. | Intentional scope evolution — durable/queryable and property-scoped; legacy propertyless artifacts are retired |

**[REQUIREMENT — amended by the approved retirement decision recorded in the implementation plan, 2026-09-14]** `GroundedAskProposal`/`GroundedAskArtifact` may be deleted only after all seven kinds have either (a) a parity-tested canonical successor or (b) an explicitly approved, tested successor scope. `UPLOAD_EVIDENCE` and `ADD_NOTE` use the two intentional scope evolutions above; retirement tests must prove the legacy models, routes, and client calls are gone and the supported successor paths remain covered.
