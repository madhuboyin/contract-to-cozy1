...
Verified via: live Playwright/Chromium run against `docker compose up postgres redis backend frontend` (local dev stack), two throwaway homeowner accounts. `apps/backend`/`apps/frontend` `tsc --noEmit` re-confirmed clean after the `askMaintenanceIntent.ts` fix. The `askMaintenanceIntent.ts` fix itself is **uncommitted** in the working tree pending the user's decision on whether to include it in this change or land separately.
