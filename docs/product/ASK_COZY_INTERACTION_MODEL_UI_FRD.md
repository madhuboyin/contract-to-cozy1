...
**Correction (2026-09-15):** this section previously read these scenarios as design-validated only, not implemented, and §15 marked their acceptance rows "shared-contract walkthroughs until their subsequent implementations are scheduled." That was stale relative to a parallel, same-day track — `ASK_COZY_MESSAGE_FIRST_FRD.md` §8.5/§20/§21 and `docs/architecture/ASK_COZY_INCREMENTAL_IMPLEMENTATION_PLAN.md`'s Stage 3 Phases 2/3/6 — under which both scenarios below are actually built: real Gemini extraction (not a stub), an editable structured proposal, confirm-time `HomeEvent` persistence with genuine date-precision/uncertainty handling for §12.1; and a `DecisionThread` create-or-resume mechanism keyed on a real database unique constraint (`activeIdentityKey`, session-independent) for §12.2. See §35 for the certification this correction is based on: both scenarios' persistence/non-duplication guarantees are now proven against a real database; the natural-language interpretation step for both remains genuinely unverified in this environment (invalid `GEMINI_API_KEY`), not because the code is unbuilt.

These scenarios validate the shared model; they are not additional maintenance implementation deliverables, but unlike the note this replaced, they are not merely design checks either — see §35.
...
A01–A18, A21, A23–A24 form the maintenance acceptance set where applicable. A22 is a shared-contract walkthrough until its subsequent implementation is scheduled (dismiss/remind-later have no domain policy yet, per §16). **A19 and A20 are corrected as of 2026-09-15** (see §35): both are implemented and certified against a real database for their persistence/non-duplication guarantees; each scenario's own natural-language interpretation step remains unverified live pending a working `GEMINI_API_KEY` in this environment. Existing application safeguards remain mandatory in every slice.
...
## 32. Item-action typed dispatch (2026-09-14)

External feedback on §23 finding 5's disclosure ("dispatch still routes through the generic `ask()` text pipeline rather than a typed per-interaction-type dispatcher") asked for a genuine dispatcher. Traced the actual gap first rather than assuming the whole system needed one: FILTER_RESULT (chips), REFRESH (explicit button) and CONFIRM/EDIT_PROPOSAL (`ConfirmationCard`'s own `/confirm` and `/confirm/edit` endpoints) already had distinct code paths. The one undifferentiated call site was `AskWorkspace.tsx`'s item-action handler (`onItemAction`) — every `GroupedListItemAction`, regardless of its declared `interactionType`, called `ask()` with a forced `operationId`; `interactionType` reached that call site but was never read.

Two scope decisions were confirmed with the user before implementing, both toward the narrower option: (1) `MUTATE_RECORD` keeps routing through the AskExecution/`ask()` pipeline rather than a new endpoint that bypasses conversation-turn creation — the receipt/reconciliation/history/revalidation machinery in §§18–31 is built on AskExecution rows, and a row action should keep producing one; (2) `DISMISS`/`REMIND_LATER` get dispatch plumbing only, not domain semantics — §16's open item (duration/persistence policy) is unchanged and remains a product decision, not something this pass should infer.

**Built:** `apps/frontend/src/features/ask/interactionDispatch.ts` — `resolveItemActionDispatch(interactionType)`, a pure function switching on the full `AskItemActionInteractionType` union (a `never`-check default branch means a new interaction type added to the union fails the build here until given a branch, rather than silently falling through). Four outcomes:
- `ASK_WITH_ENTITY_CONTEXT` (`CONVERSATION_CONTINUE`, `MUTATE_RECORD`) — the existing forced-operation `ask()` call, unchanged behavior.
- `ASK_FILTER_ONLY` (`FILTER_RESULT`) — same shape as a declared filter chip (message + `sourceExecutionId`, no entity/operation forced), for if an item-level filter action is ever declared; none is today.
- `REFRESH` — calls the same `refreshResult()` the explicit Refresh button already uses, no `ask()` turn.
- `UNSUPPORTED` (`DISMISS`, `REMIND_LATER`, `CONFIRM`, `EDIT_PROPOSAL`, `NAVIGATE`) — per ACT-003 ("Unsupported actions fail visibly and safely"), surfaces a specific reason string rather than silently no-oping or falling back to `ask()`. None of these five is declared by any item action today (`askOrchestrator.service.ts` only ever sets `CONVERSATION_CONTINUE`/`MUTATE_RECORD`) — `CONFIRM`/`EDIT_PROPOSAL` belong to `ConfirmationCard`'s own execution and have no meaning as a row click; `NAVIGATE` has no `href` field on `AskGroupedListItemAction` yet; `DISMISS`/`REMIND_LATER` are the still-unresolved §16 item. This branch exists so the day any of these five is wired up without updating this dispatcher, the failure is a visible inline message, not silent inaction.

`AskWorkspace.tsx`'s `ExecutionCard` now calls `resolveItemActionDispatch` in a single `dispatchItemAction` function that both the generic `GROUPED_LIST` renderer and `MaintenanceResultList` (the maintenance-specific path) share — replacing the inline arrow function that previously ignored the `interactionType` argument entirely. A new `itemActionIssue` local state renders the `UNSUPPORTED` reason as an inline `role="alert"` message and clears on the next dispatched action; it is deliberately local to `ExecutionCard` rather than threaded through the parent's `refreshIssues` map, since no live path reaches it yet.

**Not changed:** the server-side operation-selection/authorization/confirmation machinery (ACT-002) — this is client-side routing only, choosing which existing mechanism a click invokes, never a new write path. NAVIGATE items rendered as plain `<a href>` (block-level `actions`, `item.href`) are untouched; this dispatcher only concerns item-level `actions` arrays.

**Tests.** `apps/frontend/src/features/ask/__tests__/interactionDispatch.test.ts` (new): every interaction type's outcome, plus the `never`-exhaustiveness throw for an undeclared type.

Verified via: `apps/backend` and `apps/frontend` full `tsc --noEmit` (clean); `apps/frontend` Jest for `src/features/ask` and `src/components/ask` (21/21 passing, including the 5 new tests). No browser/Playwright run performed; the inline unsupported-action message and the unchanged item-action click behavior for `MUTATE_RECORD`/`CONVERSATION_CONTINUE` are unverified live.

## 33. Browser verification of the item-action dispatch (2026-09-15)

Ran §32's dispatcher live in a local Docker environment (Playwright against Chromium, homeowner login, a fresh property and seeded maintenance tasks) rather than relying on `tsc`/Jest alone. This surfaced one real, unrelated defect (fixed) and then confirmed every reachable dispatch branch.

**Defect found and fixed: `askMaintenanceIntent.ts`'s canonical-answer whitelist was missing `ask-next-actions`.** `MAINTENANCE_STATUS` responses that include a real next-action suggestion (the generic `ask-next-actions` `CAPABILITY_LIST` block from `askNextActions.ts`) fell through `matchesMaintenanceStatusAnswerContract`'s fast path — the whitelist predates that block and never listed it — into `askSemanticAnswerValidator.ts`'s similarity scoring, which returned `UNKNOWN` for a well-formed maintenance answer on a sparse test property and produced a spurious "I need one detail to verify this answer" clarification instead of the actual list. Confirmed by instrumenting the validator directly: `{"status":"ANSWERED","blockIds":[...,{"id":"ask-next-actions","type":"CAPABILITY_LIST"}],"matches":false}`. Fixed by adding `'ask-next-actions'` to the whitelist in `askMaintenanceIntent.ts` (committed separately, see §34). This fix is unrelated to §32's dispatcher change itself; it was blocking verification of it.

**Dispatcher branches confirmed live:**
- **`ASK_WITH_ENTITY_CONTEXT` / `CONVERSATION_CONTINUE`** ("Why is this important?"): clicking it produced a new AskExecution turn with the exact forced question ("Why is \"Annual HVAC inspection\" important?"), article count 1→2.
- **`ASK_WITH_ENTITY_CONTEXT` / `MUTATE_RECORD`** ("Reschedule"): clicking it on the "Clean gutters" row produced a new turn with the exact canned message ("Reschedule this maintenance task."), correctly resolved to that exact task (not a fuzzy match — the follow-up asked "What should change for **Clean gutters**?"), and the row rendered selected. The operation then asked for a specific date (a real, expected clarification — the canned message carries no date) rather than jumping straight to a confirmation, which is why the original test plan's "current due date" text assertion needed correcting, not the dispatcher.
- **`REFRESH`**: clicking it re-read the same execution in place with no new turn (article count unchanged across three separate runs), confirming it never enters `ASK_WITH_ENTITY_CONTEXT`'s `ask()` path.
- **`FILTER_RESULT` chips**: still render and are clickable post-refactor. This exercises the pre-existing `onFilterClick` prop, not `resolveItemActionDispatch` — no item action declares `FILTER_RESULT` today (see §32), so this path remains untouched by the dispatcher and was checked only for a regression, not as new coverage.
- **`UNSUPPORTED`** (`DISMISS`/`REMIND_LATER`/`CONFIRM`/`EDIT_PROPOSAL`/`NAVIGATE`): still only unit-tested (`interactionDispatch.test.ts`) — no live UI declares any of these five as an item action, so there is nothing to click.

No console errors or unhandled page exceptions were observed during any of the confirmed clicks. Test users, properties, and tasks created for this verification were deleted from the local dev database afterward.

Verified via: live Playwright/Chromium run against `docker compose up postgres redis backend frontend` (local dev stack), two throwaway homeowner accounts. `apps/backend`/`apps/frontend` `tsc --noEmit` re-confirmed clean after the `askMaintenanceIntent.ts` fix.

## 34. Same canonical-answer whitelist gap fixed in the other three per-operation contracts (2026-09-15)

§33 disclosed but did not fix the identical `ask-next-actions` gap in `askSavingsIntent.ts`, `askHomeActionsIntent.ts`, and `askOwnershipCostsIntent.ts`. Confirmed the gap actually applies before fixing it: `buildAskNextActionsBlock`'s call site (`askOrchestrator.service.ts`) attaches the `ask-next-actions` block to *any* operation result that is `ANSWERED`/`COMPLETED`/`READY_WITH_LIMITATIONS`, has no outstanding captures or confirmation, and doesn't already carry a `CAPABILITY_LIST` block — nothing about that gate is maintenance-specific, so `SAVINGS_OPPORTUNITIES`, every `HOME_ACTIONS` envelope, and `OWNERSHIP_COSTS` are equally exposed. `askInventoryIntent.ts` was checked too and does **not** share the gap — it matches by specific block title/id pairs with `.some()`, not an exhaustive `.every()`-against-a-whitelist shape, so an extra block never fails it.

Fixed by adding `'ask-next-actions'` to:
- `askSavingsIntent.ts`'s single whitelist.
- `askOwnershipCostsIntent.ts`'s single whitelist.
- `askHomeActionsIntent.ts`'s all three whitelists (`FEED_BLOCK_IDS`, `FOCUSED_BLOCK_IDS`, `BUYER_PLAN_BLOCK_IDS`) — Home Actions has three independent canonical envelopes, and the gap applied identically to each.

Verified via: `apps/backend` `tsc --noEmit` (clean); the 27 tests across `askMaintenanceSuggestedPrompt.test.js`, `askOwnershipCostsSuggestedPrompt.test.js`, `askSavingsSuggestedPrompt.test.js`, `askHomeActionsSuggestedPrompt.test.js`, and `askBuyerPlanPresentation.test.js` (all pass, including each file's "rejects unrelated operation blocks" negative case — confirms the added id doesn't loosen the whitelist into accepting a genuinely wrong block). A subsequent attempt at a full `apps/backend/tests/ask/*.test.js` regression run was abandoned after it hung with no live child test process for over 15s (a lingering open handle in an unrelated file, not a real failure — confirmed by the exit code matching the manual kill signal); the targeted 27-test run above was treated as sufficient rather than re-attempting the full sweep, consistent with this project's own preference for targeted verification over reflexive full-suite runs. No browser verification performed for these three operations specifically; §33's live click-through covered `MAINTENANCE_STATUS` only.

## 35. Certifying the interaction grammar against three vertical slices (2026-09-15)

External feedback: maintenance-only certification isn't enough proof the interaction *grammar* (not just the maintenance feature) generalizes — proposed certifying three deliberately different slices instead: A) retrieval + action (maintenance, already certified in §§18-34), B) conversational capture (roof event), C) persistent decision (selling goal). This section covers B and C.

**Investigation first, not assumption.** Before writing anything, traced actual code state for B and C rather than trusting this FRD's own §12/§15, which turned out to be stale (see §12's correction note above) — both scenarios are substantially built under the Stage 3 track, not merely design-validated. The real gap found: existing test coverage for both is unit-level against mocked Prisma or asserts on function source text ("governance" tests) — no existing test drove a raw message to a real Postgres row for either, and the one genuinely rigorous create/resume certification harness in this repo (`decisionThreadContinuationCertification.db.test.js`, 1,000+ attempts, concurrency, deletion, retry) covers the HVAC repair/replace decision family, never sell-hold-rent specifically, despite both sharing the identical underlying adapter factory.

**A hard environment blocker, found and isolated, not routed around.** This environment's configured `GEMINI_API_KEY` (`.env.local`) is invalid — confirmed independently of the app via a direct `curl` to `generativelanguage.googleapis.com` (`"API key not valid. Please pass a valid API key."`, `API_KEY_INVALID`). Both B and C's natural-language *interpretation* step depends on this key; neither the new DB tests nor a browser walkthrough of that specific step could be completed. Rather than fabricate a workaround or silently skip the whole certification, each new test below is split into two: an LLM-independent test proving the mechanism the FRD actually requires (persistence, uncertainty handling, non-duplication), and a live-extraction smoke test that **skips cleanly with a diagnosed reason** (not a false pass or a false fail) when a preflight ping shows the key can't reach the model. **Not done, and still open:** a live browser walkthrough of either scenario's capture/goal UI, and any proof of live extraction *quality* (does the model actually classify these messages correctly) — both require a working key. A related, separately-discovered infrastructure fact: this host runs an unrelated native Postgres on port 5432 (confirmed via `lsof`) that silently wins over Docker's own Postgres for any `localhost:5432` connection from outside the Docker network — every new test here connects on port 5433, this repo's own second published port for exactly this conflict, instead.

**B — "I replaced my roof last summer for $14,500." (`apps/backend/tests/integration/askConversationalCaptureCertification.db.test.js`, new):**
- *Downstream pipeline* (no LLM, passing): a hand-built `EventExtractionCandidate` matching this exact message — matching `extractionEvaluationCorpus.ts`'s own "pos-1" fixture's expected shape (EVENT, RANGE precision, amount 14500) — run through `buildChildExecutionData` (exported "for direct unit testing (pure, no I/O)"), inserted, then confirmed via the real `confirmAskExecution`. Proves: the proposal requires confirmation before any write; RANGE precision survives to the persisted `HomeEvent` row (not collapsed to a fabricated exact date); `captureChannel` correctly records `ASK_CONVERSATIONAL_CAPTURE`; and — the FRD's own explicit negative requirement — a bare replacement statement with no warranty-shaped detail does not fabricate a `Warranty` record.
- *Live extraction smoke*: would send the literal message through the real `createAskExecution` entry point and poll for the resulting capture execution. Currently skips with `GEMINI_API_KEY cannot reach the model (HTTP 400)`.

**C — "I'm thinking about selling next year." (`apps/backend/tests/integration/askSellHoldRentGoalCertification.db.test.js`, new):**
- *Create-or-resume identity* (no LLM, passing): calls `sellHoldRentDecisionFamilyAdapter.createOrResumeThread` directly, twice, for a property that starts with zero prior Sell/Hold/Rent activity (proving the homeowner does not need to have opened that tool first — matches `ensureCanonicalSellHoldRentAnalysis`'s own documented reason for calling `estimate()` first). The two calls simulate "leave and return in a separate turn" without needing to wait real days, because the identity mechanism (`DecisionThread.activeIdentityKey`, a real unique-constrained column keyed on property/definition/entity, never on `AskSession`) is provably time-independent, not because the test is weaker than a real multi-day gap — asserts the second call resolves to the exact same `DecisionThread.id`, exactly one `SELL_HOLD_RENT` thread exists afterward, and no `Booking` or other consequential domain write was created as a side effect of goal bookkeeping alone.
- *Live smoke*: would send the literal message through `createAskExecution` and poll for the resulting `DecisionThread`. Currently skips for the identical `GEMINI_API_KEY` reason.

**Both new test files are self-cleaning** (delete their fixture rows in a `finally` block, matching `decisionThreadContinuationCertification.db.test.js`'s own convention) and opt-in on a reachable database, never falling back to silently running against whatever `DATABASE_URL` happens to be set. One cleanup bug was found and fixed while writing the goal-certification test: `DecisionThread.createdByUserId` has no cascade-on-delete foreign key (unlike most of this schema's other `User` relations), so deleting the fixture user without first deleting its `DecisionThread`/`RecommendationSnapshot`/`DecisionThreadExecutionLink` rows left orphaned data in the database on every run — fixed by deleting those explicitly first; one pre-existing orphaned fixture from before the fix was cleaned up manually.

**What this does and does not certify.** Certified, against a real Postgres database: capture's edit/confirm/persist pipeline including uncertainty preservation and the no-fabricated-warranty guarantee; the goal thread's create-or-resume non-duplication guarantee specifically for sell-hold-rent (closing the gap the HVAC-only certification left open). Not certified in this pass, and requiring a working `GEMINI_API_KEY` to close: whether the live model actually classifies either example message correctly (extraction quality), and either scenario's UI rendering in a real browser. Maintenance (slice A) remains the only one of the three verified through an actual browser click-through (§33).
