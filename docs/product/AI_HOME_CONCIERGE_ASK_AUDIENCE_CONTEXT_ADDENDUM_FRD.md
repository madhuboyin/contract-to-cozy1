...
The pre-addendum implementation was property-grounded and household-role aware. It correctly distinguished `OWNER`, `CONTRIBUTOR`, and `VIEWER` for property reads, writes, captures, and confirmations, but did not consistently use account role or property ownership lifecycle to determine:
...
### 4.2 Baseline gaps closed by this addendum
...
### Slice 9 — Audience contract, safety, and privacy closure

1. Resolve one versioned effective audience context from authenticated account, current property access, and canonical journey context.
2. Use the declared `PRIMARY_OWNER`, `HOUSEHOLD_MEMBER`, and `NONE` property-relationship values consistently in execution policy and telemetry.
3. Initialize audience telemetry before operation dispatch so safety, clarification, and non-Skill executions retain known bounded dimensions.
4. Run safety and restricted-request routing before property-dependent authorization and detach safety responses from untrusted property context.
5. Remove private preference references for every Ask audience and owner-only references for viewer/contributor audiences.
6. Bring immutable Skill evaluation packages and execution-binding fixtures into parity with the registered journey provider.

**Exit:** Safety guidance cannot be suppressed by stale property access, protected preferences cannot appear in lower-role responses, and every initial execution carries a contract-valid audience snapshot.
...
The delivered implementation and its verification evidence are recorded below:

| Slice | Status | Evidence |
| --- | --- | --- |
| Account-role eligibility | Verified | All `/ask/*` routes use the homeowner account guard after fresh authentication; initial execution and Concierge Home also recheck the service boundary; provider/admin direct Ask routes are redirected before rendering; backend policy tests, frontend policy tests, backend TypeScript, frontend production build, and all 177 Ask tests pass |
| Journey context provider | Verified | `property.journey-context@1.0.0` performs one bounded canonical onboarding read, derives `BUYING` / `OWNING` / `SELLING` / `UNKNOWN`, is registered as optional context on all 14 property Skills, is retained in successful execution parameters, and is exposed by Concierge Home with explicit `AVAILABLE` / `UNKNOWN` / `UNAVAILABLE` state; backend TypeScript, 21 focused Skill Platform tests, and the complete Ask regression suite pass |
| Audience applicability registry | Verified | 30 immutable Skill-operation policies cover the registered property Skill catalog; startup validation rejects missing, conflicting, weak-role, unsupported-version, and journey-provider-invalid policies; execution evaluates policy after context composition and before adapter dispatch, material confirmation rechecks live applicability, and Concierge Home removes inapplicable discovery prompts server-side; backend TypeScript, registry validation, focused policy tests, and the complete Ask regression suite pass |
| Persona-aware landing prompts | Verified | A declarative operation-bound catalog supplies buyer, recent-owner, established-owner, preparing-transfer, and safe-unknown prompts; Concierge preserves Decision Thread → Home Action → inventory → lifecycle → general precedence, caps output at four, deduplicates operations, prefers category diversity, and filters by household role, audience applicability, and runtime availability; desktop and mobile acceptance tests verify capability teaching, degraded personalization, and personalized attention behavior |
| Persona-aware focused responses | Verified | Successful governed Skill results pass through a bounded post-canonical presentation layer that adds lifecycle framing, discloses general guidance when journey context is unknown, removes mutation actions/command suggestions the current household role cannot use, retains read-only navigation, and suppresses private preference references plus owner-only references for viewer/contributor audiences; backend regression and frontend acceptance tests pass without changing canonical facts or authorization ownership |
| Telemetry, controls, and authoring closure | Verified | Every initial Ask execution event initializes bounded account role, household role, operating mode, contract-valid property relationship, eligibility/applicability outcome, policy version/evaluation mode, and journey-provider status before dispatch, then enriches that snapshot with Skill lineage when applicable; independent account-role, policy, discovery, presentation, and provider controls degrade fail-closed; the complete 177-test backend Ask suite, frontend production build, and 16-scenario desktop/mobile Ask acceptance suite pass |
| Operational degradation experience | Verified | The Ask workspace recognizes the stable eligibility-control pause across landing composition, session history, pending work, question submission, and resume flows; it suppresses misleading prompts/composers, preserves the current draft and durable data, renders one calm temporary-unavailable state, and retries all initial reads in place without creating an execution; degraded-personalization and safe-recovery acceptance scenarios pass |
| Canonical journey correction | Verified | The existing property setup route reads and updates the canonical ownership journey through a narrow backend contract; viewers retain read-only visibility while contributors/owners may select one confirmed stage. Ask offers the route only when lifecycle context is unknown and the active household role can update it. Backend audience-policy and authorization regression tests pass, and no schema migration was introduced |
| Audience contract, safety, and privacy closure | Verified | `AskAudienceContext@1.0` composes authenticated account role, current household authorization, canonical lifecycle, operating mode, entry lineage, and `PRIMARY_OWNER` / `HOUSEHOLD_MEMBER` / `NONE` relationship values. Safety routing detaches untrusted property scope before access-dependent work, and presentation enforces preference/reference visibility. Backend TypeScript, all 177 Ask tests, frontend TypeScript, production build, and all 16 desktop/mobile Ask acceptance scenarios pass |
